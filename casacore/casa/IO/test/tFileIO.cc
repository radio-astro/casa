...
//#
//# $Id: tFileIO.cc 20329 2008-06-06 07:59:22Z gervandiepen $
...
int main (int argc, const char* argv[])
{
	...
	FiledesIO file2 (fd, "tFileIO_tmp.dat1");
	timer.mark();
	for (i=0; i<nr; i++) {
	    if (seek  &&  i%3 == 0) {
	...
	FiledesIO file2 (FiledesIO::open ("tFileIO_tmp.dat1"), "tFileIO_tmp.dat1");
	timer.mark();
	for (i=0; i<nr; i++) {
	    if (seek  &&  i%3 == 0) {
		file2.seek (i*leng, ByteIO::Begin);
	...
	LargeFiledesIO file2 (fd, "tFileIO_tmp.dat2");
	timer.mark();
	for (i=0; i<nr; i++) {
	    if (seek  &&  i%3 == 0) {
		file2.seek (i*leng, ByteIO::Begin);
	...
	LargeFiledesIO file2 (LargeFiledesIO::open ("tFileIO_tmp.dat2"), "tFileIO_tmp.dat2");
	timer.mark();
	for (i=0; i<nr; i++) {
	    if (seek  &&  i%3 == 0) {
		file2.seek (i*leng, ByteIO::Begin);
	...
	FiledesIO file2 (FiledesIO::open ("tFileIO_tmp.dat2"), "tFileIO_tmp.dat2");
	timer.mark();
	for (i=0; i<nr; i++) {
	    if (seek  &&  i%3 == 0) {
		file2.seek (i*leng, ByteIO::Begin);
	...
	LargeFiledesIO file2 (LargeFiledesIO::open ("tFileIO_tmp.dat1"),"LargeRegularFileIO small" );
	timer.mark();
	for (i=0; i<nr; i++) {
	    if (seek  &&  i%3 == 0) {
		file2.seek (i*leng, ByteIO::Begin);
