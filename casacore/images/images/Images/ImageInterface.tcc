...
ImageInterface<T>::ImageInterface() :
	regHandPtr_p(0) {
	logSink() << LogOrigin("ImageInterface<T>", "ImageInterface()", WHERE)
			<< LogIO::DEBUGGING << "Creating ImageInterface" << LogIO::POST;
...
ImageInterface<T>::ImageInterface(const ImageInterface& other) :
	MaskedLattice<T> (other), coords_p(other.coords_p), log_p(other.log_p),
			imageInfo_p(other.imageInfo_p), unit_p(other.unit_p), miscInfo_p(
					other.miscInfo_p), regHandPtr_p(0) {
	logSink() << LogOrigin("ImageInterface<T>",
			"ImageInterface(const ImageInterface&)", WHERE) << LogIO::DEBUGGING
			<< LogIO::POST;
...
Bool ImageInterface<T>::setCoordinateInfo(const CoordinateSystem &coords) {
	...
	if (!ok) {
		...
		Int stkcrd = -1;
		while (ok && (stkcrd
				= coords.findCoordinate(Coordinate::STOKES, stkcrd)) >= 0) {
			ok = True;
			...
			if (axis >= 0) {
				...
				if (axislength > nstokes) {
					ok = False;
					errmsg << "Stokes axis " << axis << " is length "
							<< axislength << " but we only have " << nstokes
	...
	if (ok) {
		coords_p = coords;
		logSink() << LogIO::DEBUGGING << "Changing coordinate system:\n"
				...
				<< coords_p.worldAxisUnits() << endl << "linear xfrom = "
				<< coords_p.linearTransform() << LogIO::POST;
	} else {
		// !ok
...
LELCoordinates ImageInterface<T>::lelCoordinates() const {
	return LELCoordinates(new LELImageCoord(coords_p, imageInfo_p, units(),
			miscInfo_p));
}

template<class T>
ImageRegion ImageInterface<T>::makeMask(const String& name,
		Bool defineAsRegion, Bool setAsDefaultMask, Bool initialize, Bool value) {
...
void ImageInterface<T>::defineRegion(const String& name,
		const ImageRegion& region, RegionHandler::GroupType type,
		Bool overwrite) {
...
template<class T>
ImageRegion* ImageInterface<T>::getImageRegionPtr(const String& name,
		RegionHandler::GroupType type, Bool throwIfUnknown) const {
	return regHandPtr_p->getRegion(name, type, throwIfUnknown);
}
template<class T>
void ImageInterface<T>::renameRegion(const String& newName,
		const String& oldName, RegionHandler::GroupType type,
		Bool throwIfUnknown) {
...
Bool ImageInterface<T>::fromRecord(String& error, const RecordInterface& inRec) {
	...
	CoordinateSystem* pCSys = CoordinateSystem::restore(coordinateRecord,
			"coordsys");
