...
Bool GaussianConvert::deconvolve(Quantum<Double>& majorAxisModel,   
...
//
   ...
   if (positionAngleSource.getFullUnit() != radians) {
      String msg("GaussianConvert::deconvolve - source position angle units not angular");
      throw(AipsError(msg));
   }
   if (positionAngleBeam.getFullUnit() != radians) {
      String msg("GaussianConvert::deconvolve - beam position angle units not angular");
      throw(AipsError(msg));
   }
   if (positionAngleModel.getFullUnit() != radians) {
      positionAngleModel.setUnit(positionAngleSource.getFullUnit());
   }
   Unit positionAngleModelUnit = positionAngleModel.getFullUnit();
   positionAngleModel.setUnit(radians);
//
// Check major axis units
//
   if (majorAxisSource.getFullUnit() != radians) {
      String msg("GaussianConvert::deconvolve - source major axis units not angular");
      throw(AipsError(msg));
   }
   if (majorAxisBeam.getFullUnit() != radians) {
      String msg("GaussianConvert::deconvolve - beam major axis units not angular");
      throw(AipsError(msg));
   }
   if (majorAxisModel.getFullUnit() != radians) {
      majorAxisModel.setUnit(majorAxisSource.getFullUnit());
   }
   Unit majorAxisModelUnit = majorAxisModel.getFullUnit();
   majorAxisModel.setUnit(radians);
//
// Check minor axis units
//
   if (minorAxisSource.getFullUnit() != radians) {
      String msg("GaussianConvert::deconvolve - source minor axis units not angular");
      throw(AipsError(msg));
   }
   if (minorAxisBeam.getFullUnit() != radians) {
      String msg("GaussianConvert::deconvolve - beam minor axis units not angular");
      throw(AipsError(msg));
   }
   if (minorAxisModel.getFullUnit() != radians) {
      minorAxisModel.setUnit(minorAxisSource.getFullUnit());
   }
   Unit minorAxisModelUnit = minorAxisModel.getFullUnit();
   minorAxisModel.setUnit(radians);
//
// Get values in radians
//
   Double majorSource = majorAxisSource.getValue(radians);
   Double minorSource = minorAxisSource.getValue(radians);
   Double thetaSource = positionAngleSource.getValue(radians);
   Double majorBeam = majorAxisBeam.getValue(radians);
   Double minorBeam = minorAxisBeam.getValue(radians);
   Double thetaBeam = positionAngleBeam.getValue(radians);
//
// Do the sums
//
...
//
   if(alpha<0.0 || beta<0.0 || s<t) {
      majorAxisModel.setValue(0.0);
      minorAxisModel.setValue(0.0);
...
// Fill in values of beam
      ...
      minorAxisModel.setValue(sqrt(0.5*(s-t)));
      if((abs(gamma)+abs(alpha-beta))==0.0) {
        positionAngleModel.setValue(0.0);
      } else {
...
//
   majorAxisModel.convert(majorAxisModelUnit);
   minorAxisModel.convert(minorAxisModelUnit);
   positionAngleModel.convert(positionAngleModelUnit);
//
