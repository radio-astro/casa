...
//#
//# $Id: blockio.cc 20648 2009-06-29 07:22:00Z gervandiepen $
//
...
BlockIO::~BlockIO() {
	if (m_filename != 0 && strlen(m_filename) > 0) {
	   // For writting node, fits_close_file() damage the output file!
	   int l_status = 0;
...
int BlockIO::close_file( fitsfile *fptr, int *status){
    ...
    if ((fptr->Fptr)->open_count == 0)              // if no other files use structure
    {
        ...
        //if ((*driverTable[(fptr->Fptr)->driver].close)((fptr->Fptr)->filehandle) )
		  //cout<<"[BlockIO::close_file()] Before calling file_close()." << endl;
		  if( file_close( (fptr->Fptr)->filehandle) ){ 
		      //cout<<"[BlockIO::close_file()] file_close() failed." << endl;            
...
int BlockOutput::write(char *addr) {
	...
	if (m_current >= m_blocksize) {	
      ...
      int l_status = 0;
		...
		// record into a buffer. So we call ffflsh() to flush everything onto disk. Otherwise
		// something would go wrong!
		if(ffflsh(m_fptr, TRUE, &l_status)){ errmsg(WRITEERR,"[~BlockOutput()] Error flushing buffer!"); }
