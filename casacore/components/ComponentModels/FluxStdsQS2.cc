...
Bool FluxStdPerleyButler2013::setSourceCoeffs()
{
  ...
  if(!Aipsrc::findDir(stddatapath,"~/"+stdTabName)) {
    if(!Aipsrc::findDir(stddatapath, "data/nrao/standards/"+stdTabName)) {
      ostringstream oss;
      oss << "The coefficient data for Perley-Butler 2013, " <<  stdTabName
          << " is not found in ./ or in ~/data/nrao/standards/";
...
/***
  if (srcEnum != FCQS::THREEC286 && 
      ...
      srcEnum != FCQS::THREEC295 &&
      srcEnum != FCQS::THREEC196 &&
      srcEnum != FCQS::THREEC48 &&
      srcEnum != FCQS::THREEC138 &&
      srcEnum != FCQS::THREEC147) 
***/
  if (srcEnum != FSS::THREEC286 && 
