...
$(document).ready(function () {
    ...
    filterPipeline.addFilter(FILTERS.createMatchFilter('field', '#select-field'));
	...
	<div class="row-fluid">
		...
		<div class="column-fluid span6">
			...
			<fieldset>
				...
				<select id="select-field" class="select2" multiple style="width:100%" placeholder="Show all fields">
					% for field in sorted(list(set([p.parameters['field'] for p in plots]))):
					<option>${field}</option>
					% endfor
		       	</select>
			</fieldset>
...
<div class="column-fluid">
	<ul class="thumbnails">
	% for plot in sorted(plots, key=lambda p: p.parameters['field']):
		<li class="span2">
			<div class="thumbnail">
				<a class="fancybox"
				   href="${os.path.relpath(plot.abspath, pcontext.report_dir)}"
				   title="Antenna ${plot.parameters['field']} spw ${plot.parameters['spw']}"
				   data-thumbnail="${os.path.relpath(plot.thumbnail, pcontext.report_dir)}">
					<img   src="${os.path.relpath(plot.thumbnail, pcontext.report_dir)}"
						 title="${plot.parameters['field']} spw ${plot.parameters['spw']}"
						   alt="">
					</img>
				</a>
					<p class="text-center">${plot.parameters['field']} spw ${plot.parameters['spw']}</p>
			</div>
		</li>
	% endfor
	</ul>
