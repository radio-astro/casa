...
class SDImportDataOldInputs(basetask.StandardInputs):
    ...
    def as_dict(self):
        properties = super(SDImportDataOldInputs,self).as_dict()
        properties['infiles'] = properties.pop('vis')
        return properties


class SDImportDataOldResults(basetask.Results):
    def __init__(self, mses=[], scantables=[]):
        super(SDImportDataOldResults, self).__init__()
        self.mses = mses
    ...
    def __repr__(self):
        return 'SDImportDataOldResults:\n\t{0}\n\t{1}'.format(
            '\n\t'.join([ms.name for ms in self.mses]),
            '\n\t'.join([st.name for st in self.scantables]))


class SDImportDataOld(basetask.StandardTaskTemplate):
    Inputs = SDImportDataOldInputs
    ...
    def prepare(self, **parameters):
        ...
        results = SDImportDataOldResults(observing_run_sd.measurement_sets,
