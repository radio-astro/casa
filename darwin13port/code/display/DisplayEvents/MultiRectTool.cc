//# MultiRectTool.cc: Base class for MultiWorldCanvas event-based rectangle tools
...
namespace casa { //# NAMESPACE CASA - BEGIN
				...
				if ( (*iter)->regionVisible( ) ) {
					...
					if ( viewer::Region::regionSelected(result) ) {
						if ( ev.key() == Display::K_Escape ) {
							rectanglelist::iterator xi = iter;
							++xi;
	...
	void MultiRectTool::set(const Int &x1, const Int &y1,
	...
	void MultiRectTool::get(Int &x1, Int &y1, Int &x2, Int &y2) const {
		if (!itsCurrentWC) return;
		get(x1,y1);
		Vector<Double> pix(2);
	...
	void MultiRectTool::get(Int &x1, Int &y1) const {
		if (!itsCurrentWC) return;
		Vector<Double> pix(2);
