//# tFlagAgentExtension.cc This file contains the unit tests of the FlagAgentBase class.
//#
...
void writeFlags(string inputFile,Record dataSelection,vector<Record> agentParameters)
{
	...
	Int agentNumber = 1;
	FlagAgentList agentList;
	FlagAgentExtension *flaggingAgent = NULL;
	for (vector<Record>::iterator iter=agentParameters.begin();iter != agentParameters.end();iter++)
	{
		...
		iter->define("name","FlagAgentExtension_" + agentName.str());
		flaggingAgent = new FlagAgentExtension(dh,*iter);
		agentList.push_back(flaggingAgent);
		agentNumber++;
	}
