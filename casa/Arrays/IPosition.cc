...
/*
void IPosition::next(
	const IPosition& shape, const Bool doChecks
) {
	next(shape, this->makeAxisPath(size_p), doChecks);
}
*/

void IPosition::next(
	const IPosition& shape, const IPosition& axisPath, const Bool doChecks
) {
	if (doChecks) {
		...
		if (axisPath.size_p != size_p) {
			throw AipsError(
				"IPosition::next(): Sizes of object and axisPath not consistent"
			);
		}
		if (*this > shape) {
			throw AipsError(
		...
		std::vector<uInt> foundAxes(0);
		for (uInt i=0; i<size_p; i++) {
			Bool found = False;
			for (uInt j=0; j<size_p; j++) {
				for (
					std::vector<uInt>::const_iterator iter=foundAxes.begin();
					iter!=foundAxes.end(); iter++
				) {
					if (axisPath[i] == *iter) {
						throw AipsError(
							"IPosition::next(): Axis " + String::toString(i)
					    	+ " found multiple times in axisPath"
						);
					}
				}
				if (axisPath[i] == j) {
					found = True;
					foundAxes.push_back(j);
					break;
				}
			}
			if (! found) {
	...
	uInt xpos = data_p[axisPath[0]];
	xpos++;
	if (xpos < shape[axisPath[0]]) {
		data_p[axisPath[0]] = xpos;
	}
	else {
		data_p[axisPath[0]] = 0;
		for (uInt i=1; i<size_p; i++) {
			if (data_p[axisPath[i]] < shape.data_p[axisPath[i]] - 1) {
				data_p[axisPath[i]]++;
				break;
			}
			else {
