...
# publish_summary.runTest( 'immoments_test' )
#
# To test the script by itself, ie. not with test infrastructure.
# cd CASA_REPOS/code/xmlcasa/tasks/test
# import immoment_test
# immoment_test.run()
# </example>
#
# <motivation>
...
####################################################################
# Support methods for various things
####################################################################
...
def description():
    _momentTest_debug_msg( 0 ) 
    return " Tests the correctness of the immoments task in CASA including:"\
           + "\n\n"\
           + "1. Incorrect input for each paramter.  Incorrect input includes"\
           + "   one input of the incorrect data type, out-of-bounds (where"\
           + "   applicable, and correct data type but non-sensical."\
...
def run():
    ...
    test_list = [ 'input_test()', 'moments_test()', 'pixel_sel_test()', \
                  'region_test()', 'mask_test()' ]
    ...
    # casapy function, update_params() can not be found and you get a
    # the following msg
    #  results = eval( "immoments( 'n1333_both', moments=[0], outfile='input_test_1' )", globals() )
    # traceeback.py:13:   File "<string>", line 1, in <module>
    # traceback.py:13:   File "/casa/active/linux_gnu/python/2.5/immoments_cli.py", line 135, in __call__
    # traceback.py:13:     myf['update_params'](func=myf['taskname'],printtext=False)
    # traceback.py:13: KeyError: 'update_params'
    #
    # but if you call each method directly then there is no issue.
    
    passed = True
    error_msgs = ""
    testResults=[]
    _momentTest_debug_msg( 4 ) 
