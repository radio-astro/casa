	...
	while (dh->nextChunk())
	{
		...
		while (dh->nextBuffer())
		{
					...
					antennaIter != antennaMap->end();++antennaIter)
			{
				...
				// Get visibility cube
				visCubeView = dh->getVisibilitiesView(antennaPair.first,antennaPair.second);

				// Get mapped flag cube (can be modified and the flags will be written in the correct positions of the total flag cube)
				flagCubeView = dh->getFlagsView(antennaPair.first,antennaPair.second);
					...
					subIntegrationIter != subIntegrationMap->end();++subIntegrationIter)
			{
				...
				// Get visibility cube
				visCubeView = dh->getVisibilitiesView(timestep);

				// Get mapped flag cube (can be modified and the flags will be written in the correct positions of the total flag cube)
				flagCubeView = dh->getFlagsView(timestep);
