...
unsigned int SpectralGrid::add(unsigned int numChan, double refFreq, vector<
    double> chanFreq, string freqUnits)
{
...
unsigned int SpectralGrid::getNumSpectralWindow()
{
  return v_transfertId_.size();
}
unsigned int SpectralGrid::getNumChan()
{
  return v_numChan_[0];
}
unsigned int SpectralGrid::getNumChan(unsigned int spwId)
{
  if(wrongSpwId(spwId)) return 0;
  return v_numChan_[spwId];
}

unsigned int SpectralGrid::getRefChan()
{
  return v_refChan_[0];
}
unsigned int SpectralGrid::getRefChan(unsigned int spwId)
{
  if(wrongSpwId(spwId)) return 32767;
  return v_refChan_[spwId];
}

Frequency SpectralGrid::getRefFreq()
{
  return Frequency(v_refFreq_[0], "Hz");
}
Frequency SpectralGrid::getRefFreq(unsigned int spwId)
{
  if(wrongSpwId(spwId)) return 32767.;
  return Frequency(v_refFreq_[spwId], "Hz");
}

Frequency SpectralGrid::getChanSep()
{
  return Frequency(v_chanSep_[0], "Hz");
}
Frequency SpectralGrid::getChanSep(unsigned int spwId)
{
  if(wrongSpwId(spwId)) return 32767.;
  return Frequency(v_chanSep_[spwId], "Hz");
}

Frequency SpectralGrid::getChanFreq(unsigned int i)
{
...
Frequency SpectralGrid::getMinFreq()
{
  return Frequency(v_minFreq_[0], "Hz");
}
Frequency SpectralGrid::getMinFreq(unsigned int spwId)
{
  if(wrongSpwId(spwId)) return 32767.;
  return Frequency(v_minFreq_[spwId], "Hz");
}

Frequency SpectralGrid::getMaxFreq()
{
  return Frequency(v_maxFreq_[0], "Hz");
}
Frequency SpectralGrid::getMaxFreq(unsigned int spwId)
{
  if(wrongSpwId(spwId)) return 32767.;
...
Frequency SpectralGrid::getBandwidth()
{
...
double SpectralGrid::getLoFrequency()
{
  return v_loFreq_[0];
}

double SpectralGrid::getLoFrequency(unsigned int spwId)
{
  if(wrongSpwId(spwId)) return 32767.;
