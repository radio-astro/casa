...
namespace casa {
	...
	class QtRectangle : public QtRegion, public Rectangle {
		...
		bool modified( ) const { return true; }
#if OLDSTUFF
		void clearstats( )
			{ QtRegion::clearstats( ); }
		void addstats( const std::string &name, std::list<std::pair<String,String> > *stats )
			{ QtRegion::addstats( name, stats ); }
#endif
		/* QtRectangle( ) : QtRegion("current rectangle") { } */
		QtRectangle( QtRegionSource *factory, double blc_x, double blc_y, double trc_x, double trc_y );
