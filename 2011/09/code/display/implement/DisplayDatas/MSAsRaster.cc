...
void MSAsRaster::initMSAR_( const viewer::DisplayDataOptions &ddo ) {
    ...
    r[of] = .23f; g[of] = .33f; b[of] = .8f;	// (medium blue)
...
void MSAsRaster::selectVS_( const viewer::DisplayDataOptions &ddo ) {
  ...
  if (!msselValid_) {
    ...
    visStart_=0; visShape_=0; visShapeA_=0;
...
void MSAsRaster::setCS_() {
  ...
  if(!antSort_) {
    ...
    if(axY==BASELN) daxisnames(Y) += " (m)";  }   //  in this case).
...
Bool MSAsRaster::FlagEdit_::appliesTo(Int bsln) {
  ...
  Int firstBsl, endBsl;  getSureRange(BASELN, firstBsl, endBsl);
...
void MSAsRaster::MSpos_::setb(Int b) {
  ...
  if(m->nAnt_==1) feed=b;
  else { ant1=m->a1_(b); ant2=m->a2_(b);  }  }
...
void MSAsRaster::computeBaselineSorts_() {
  ...
  a1A_.resize(nbslA_);  a2A_.resize(nbslA_);
  for(Double bsl=0.; bsl<nbslA_; bsl++) {
    Double a1d, a2d;  a1a2_(a1d,a2d, bsl);
    a1A_(bsl) = Int(a1d);  a2A_(bsl) = Int(a2d);  }
  ...
  a1L_.resize(nbslA_);  a2L_.resize(nbslA_);
...
void MSAsRaster::deleteParameters_() {
  ...
  delete itsUndoAll; delete itsEditEntireMS; delete itsSaveEdits;  }
