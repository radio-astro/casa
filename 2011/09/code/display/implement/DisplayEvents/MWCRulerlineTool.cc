//# MWCRulerlineTool.cc: MultiWorldCanvas rulerline  tool
...
MWCRulerlineTool::~MWCRulerlineTool() { reset();
}
  
void MWCRulerlineTool::keyPressed(const WCPositionEvent &ev) {
...
void MWCRulerlineTool::keyReleased(const WCPositionEvent &ev) {
  if(!itsActive) return;
