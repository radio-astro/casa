...
myrms=spave.stats(stat='rms',mask=rmsmask)                                                                
...
mymax=spave.stats('max',linemask)	#  IF[15] = 0.366
mysum=spave.stats('sum',linemask)	#  IF[15] = 49.949
mymedian=spave.stats('median',linemask) #  IF[15] = 0.094
mymean=spave.stats('mean',linemask)	#  IF[15] = 0.125
											# Chans  bchan    echan      Xmin      Xmax        Ymin     Ymax
...
ori_rms=0.03720
ori_sum=49.95
diff_max = abs((ori_max-mymax[0])/ori_max)
diff_rms = abs((ori_rms-myrms[0])/ori_rms)
diff_sum = abs((ori_sum-mysum[0])/ori_sum)
