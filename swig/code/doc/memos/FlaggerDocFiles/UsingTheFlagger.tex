...
Values are defined as expressions that involve data columns and correlation-selections (for example, ABS_I or REAL_RR,LL, etc).
...
column APPLIED set to True inside the FLAG_CMD sub-table of the MS.

\subsubsection{Managing flag-commands in the flagcmd task}
Flag commands can be given as input to the flagcmd task in several ways. The
default is the 'table' which will have the flag commands inside the FLAG_CMD
...
\paragraph{Using flag-commands}
The example below shows how to setup and run tflagdata in list mode, using a
flag-command file.
...
Similarly, one can use the flagcmd task to execute the same commands using
either the 'file' or the 'cmd' inpmode options. 
...
The FLAG_CMD sub-table is meant to store only meta-data selections such as
...
Keeping the state of the flags stored in the FLAG_CMD sub-table is also not
...
flagging using plotms. Note that plotms does not write to the FLAG_CMD
sub-table.
