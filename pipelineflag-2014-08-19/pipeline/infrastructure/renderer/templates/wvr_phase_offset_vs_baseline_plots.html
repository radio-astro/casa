...
$(document).ready(function () {
    ...
    filterPipeline.addFilter(FILTERS.createMatchFilter('scan', '#select-scan'));
	...
	<div class="row-fluid">
		...
		<div class="column-fluid span6">
			...
			<fieldset>
				...
				<select id="select-scan" class="select2" multiple style="width:100%" placeholder="Show all scans">
					% for scan in sorted(list(set([int(p.parameters['scan']) for p in plots]))):
					<option>${scan}</option>
					% endfor
		       	</select>
			</fieldset>
...
<div class="column-fluid">
	<ul class="thumbnails">
	% for plot in sorted(plots, key=lambda p: int(p.parameters['scan'])):
		<li class="span2">
			<div class="thumbnail">
				<a class="fancybox"
				   href="${os.path.relpath(plot.abspath, pcontext.report_dir)}"
				   title="Scan ${plot.parameters['scan']} spw ${plot.parameters['spw']}"
				   data-thumbnail="${os.path.relpath(plot.thumbnail, pcontext.report_dir)}">
					...
					<p class="text-center">Scan ${plot.parameters['scan']} spw ${plot.parameters['spw']}</p>
			</div>
		</li>
	% endfor
	</ul>
