...
private:
	...
	// TODO the methods which come in seven or eight flavors were written in that way
	// to maximize performance for a particular data set on the basis of it has
	// a mask, weights, and/or ranges, or none of these. It is possible that
	// the performance decline for any one of these is not significant, even
	// for large datasets, so that fewer of these methods are in practice possible
	// with no significant performance degradation. That remains to be investigated.


	// <group>
	// scan through the data set to determine the number of good (unmasked, weight > 0,
	// within range) points. There are seven not eight because the one with no mask, no
	// ranges, and no weights is trivial with npts = nr.

	static void _accumNpts(
		uInt64& npts,
	...
	//vector<AccumType> _quantilesFromBins(const vector<Double>& quantiles, uInt maxArraySize);

	// This is a modified version of quick select developed for the context of this class
	// (minimizing memory usage) based on the algorithm outlined in section 2.1. of
	// http://www.stat.cmu.edu/~ryantibs/papers/median.pdf
	// find the kth largest value.
	//AccumType _quickSelect(const vector<Double>& q, const vector<AccumType>& pivot);

	// no weights, no mask, no ranges
	void _unweightedStats(const InputIterator& dataBegin, Int64 nr, uInt dataStride);
