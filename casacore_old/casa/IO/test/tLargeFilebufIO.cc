...
//#
//# $Id: tLargeFilebufIO.cc 21505 2014-11-21 11:43:02Z gervandiepen $
...
void doIt()
{
  ...
  {
    LargeRegularFileIO file (RegularFile("tLargeFilebufIO_tmp.dat"),
			     ByteIO::New);
    file.write (2000, buf);
  }
  int fd = open ("tLargeFilebufIO_tmp.dat", O_RDONLY);
  check (fd, 2000, buf);
  check (fd, 100, buf);
  check (fd, 1, buf);
  check (fd, 3, buf);
