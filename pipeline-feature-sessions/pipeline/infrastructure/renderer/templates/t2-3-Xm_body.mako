...
<table class="table">
	...
	% for result_type, results_list in topic.results_by_type.items():
		...
		<tr>
			<td><a href="t2-4m.html?sidebar=sidebar_stage${results.stage_number}">${hr.get_task_description(results)}</a><span class="pull-right">${scores[results.stage_number].shortmsg}</span></td>
