...
from types import *
from task_hifa_importdata import hifa_importdata
class hifa_importdata_cli_:
    __name__ = "hifa_importdata"
    rkey = None
    ...
    def __init__(self) :
       self.__bases__ = (hifa_importdata_cli_,)
       self.__doc__ = self.__call__.__doc__

       self.parameters={'vis':None, 'session':None, 'pipelinemode':None, 'asis':None, 'process_caldevice':None, 'overwrite':None, 'nocopy':None, 'bdfflags':None, 'asimaging':None, 'lazy':None, 'dbservice':None, 'ocorr_mode':None, 'createmms':None, 'dryrun':None, 'acceptresults':None, }
    ...
    def __call__(self, vis=None, session=None, pipelinemode=None, asis=None, process_caldevice=None, overwrite=None, nocopy=None, bdfflags=None, asimaging=None, lazy=None, dbservice=None, ocorr_mode=None, createmms=None, dryrun=None, acceptresults=None, ):
...
Imports data into the interferometry pipeline
	Arguments :
		...
		asimaging:	Import measurement sets as imaging measurement sets
		   Default Value: False
...
6. Run in  dryrun mode before running for real:
   ...
   results = hifa_importdata(vis=['uid___A002_X30a93d_X43e.ms'])
	...
	#casalog = casac.casac.logsink()
        self.__globals__['__last_task'] = 'hifa_importdata'
        self.__globals__['taskname'] = 'hifa_importdata'
        ###
	...
	if useLocalDefaults :
	   for item in function_signature_defaults.iteritems():
		   ...
		   if(type(keyVal) == dict) :
            ...
            myparams['asimaging'] = asimaging = self.parameters['asimaging']
            myparams['lazy'] = lazy = self.parameters['lazy']
            myparams['dbservice'] = dbservice = self.parameters['dbservice']
...
#
	...
	trec = casac.casac.utils().torecord(pathname+'hifa_importdata.xml')

        casalog.origin('hifa_importdata')
	try :
          #if not trec.has_key('hifa_importdata') or not casac.casac.utils().verify(mytmp, trec['hifa_importdata']) :
	    #return False

          casac.casac.utils().verify(mytmp, trec['hifa_importdata'], True)
          scriptstr=['']
          saveinputs = self.__globals__['saveinputs']
          if type(self.__call__.func_defaults) is NoneType:
              saveinputs=''
          else:
              saveinputs('hifa_importdata', 'hifa_importdata.last', myparams, self.__globals__,scriptstr=scriptstr)
          tname = 'hifa_importdata'
          spaces = ' '*(18-len(tname))
          casalog.post('\n##########################################'+
          ...
          result = hifa_importdata(vis, session, pipelinemode, asis, process_caldevice, overwrite, nocopy, bdfflags, asimaging, lazy, dbservice, ocorr_mode, createmms, dryrun, acceptresults)
          if (casa['state']['telemetry-enabled']):
...
#            paramgui.setGlobals({})
#
#        paramgui.runTask('hifa_importdata', myf['_ip'])
#        paramgui.setGlobals({})
#
#
#
#
	...
	if not hasattr(self, "__globals__") or self.__globals__ == None :
        ...
        a['pipelinemode'] = {
                    0:{'value':'automatic'}, 
                    1:odict([{'value':'interactive'}, {'asis':'SBSummary ExecBlock Antenna Station Receiver Source CalAtmosphere CalWVR'}, {'ocorr_mode':'ca'}, {'process_caldevice':False}, {'overwrite':False}, {'nocopy':False}, {'bdfflags':True}, {'dryrun':False}, {'acceptresults':True}]), 
                    2:odict([{'value':'getinputs'}, {'asis':'SBSummary ExecBlock Antenna Station Receiver Source CalAtmosphere CalWVR'}, {'process_caldevice':False}, {'overwrite':False}, {'nocopy':False}, {'bdfflags':True}, {'ocorr_mode':'ca'}])}
...
#
#
    def description(self, key='hifa_importdata', subkey=None):
        desc={'hifa_importdata': 'Imports data into the interferometry pipeline',
               'vis': 'List of input visibility data',
               'session': 'List of visibility data sessions',
               'pipelinemode': 'The pipeline operating mode',
               'asis': 'Extra ASDM tables to convert as is',
...
#
    ...
    def itsdefault(self, paramname) :
        ...
        if a.has_key(paramname) :
	      return a[paramname]
hifa_importdata_cli = hifa_importdata_cli_()
