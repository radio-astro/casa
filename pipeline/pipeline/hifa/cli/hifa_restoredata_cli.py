...
from types import *
from task_hifa_restoredata import hifa_restoredata
class hifa_restoredata_cli_:
    __name__ = "hifa_restoredata"
    rkey = None
    ...
    def __init__(self) :
       self.__bases__ = (hifa_restoredata_cli_,)
...
1. Restore the pipeline results for a single ASDM in a single session 
	...
	#casalog = casac.casac.logsink()
        self.__globals__['__last_task'] = 'hifa_restoredata'
        self.__globals__['taskname'] = 'hifa_restoredata'
        ###
...
#
	...
	trec = casac.casac.utils().torecord(pathname+'hifa_restoredata.xml')

        casalog.origin('hifa_restoredata')
	try :
          #if not trec.has_key('hifa_restoredata') or not casac.casac.utils().verify(mytmp, trec['hifa_restoredata']) :
	    #return False

          casac.casac.utils().verify(mytmp, trec['hifa_restoredata'], True)
          scriptstr=['']
          saveinputs = self.__globals__['saveinputs']
          if type(self.__call__.func_defaults) is NoneType:
              saveinputs=''
          else:
              saveinputs('hifa_restoredata', 'hifa_restoredata.last', myparams, self.__globals__,scriptstr=scriptstr)
          tname = 'hifa_restoredata'
          spaces = ' '*(18-len(tname))
          casalog.post('\n##########################################'+
          ...
          result = hifa_restoredata(vis, session, products_dir, copytoraw, rawdata_dir, lazy, bdfflags, ocorr_mode, asis, pipelinemode, dryrun, acceptresults)
          if (casa['state']['telemetry-enabled']):
...
#            paramgui.setGlobals({})
#
#        paramgui.runTask('hifa_restoredata', myf['_ip'])
#        paramgui.setGlobals({})
#
#
#
...
#
#
    def description(self, key='hifa_restoredata', subkey=None):
        desc={'hifa_restoredata': 'Restore flagged and calibration interferometry data from a pipeline run',
               'vis': 'List of input visibility data',
               'session': 'List of sessions one per visibility file',
               'products_dir': 'The archived pipeline data products directory',
               'copytoraw': 'Copy calibration and flagging tables to raw data directory',
...
#
    ...
    def itsdefault(self, paramname) :
        ...
        if a.has_key(paramname) :
	      return a[paramname]
hifa_restoredata_cli = hifa_restoredata_cli_()
