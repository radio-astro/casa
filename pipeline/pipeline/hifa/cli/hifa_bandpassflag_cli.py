...
from types import *
from task_hifa_bandpassflag import hifa_bandpassflag
class hifa_bandpassflag_cli_:
    __name__ = "hifa_bandpassflag"
    rkey = None
	...
	Arguments :
		...
		field:	Field ids to use in data selection
		   Default Value: 

		spw:	Spectral windows to use in data selection, \'\' for all
		   Default Value: 
...
1. run with recommended settings to create bandpass solution with flagging
	...
	#casalog = casac.casac.logsink()
        self.__globals__['__last_task'] = 'hifa_bandpassflag'
        self.__globals__['taskname'] = 'hifa_bandpassflag'
        ###
...
#
	...
	trec = casac.casac.utils().torecord(pathname+'hifa_bandpassflag.xml')

        casalog.origin('hifa_bandpassflag')
	try :
          #if not trec.has_key('hifa_bandpassflag') or not casac.casac.utils().verify(mytmp, trec['hifa_bandpassflag']) :
	    #return False

          casac.casac.utils().verify(mytmp, trec['hifa_bandpassflag'], True)
          scriptstr=['']
          saveinputs = self.__globals__['saveinputs']
          if type(self.__call__.func_defaults) is NoneType:
              saveinputs=''
          else:
              saveinputs('hifa_bandpassflag', 'hifa_bandpassflag.last', myparams, self.__globals__,scriptstr=scriptstr)
          tname = 'hifa_bandpassflag'
          spaces = ' '*(18-len(tname))
          casalog.post('\n##########################################'+
...
#            paramgui.setGlobals({})
#
#        paramgui.runTask('hifa_bandpassflag', myf['_ip'])
#        paramgui.setGlobals({})
#
#
#
#
	...
	if not hasattr(self, "__globals__") or self.__globals__ == None :
        ...
        a['pipelinemode'] = {
                    0:{'value':'automatic'}, 
                    1:odict([{'value':'interactive'}, {'vis':[]}, {'caltable':[]}, {'field':''}, {'intent':''}, {'spw':''}, {'antenna':''}, {'combine':''}, {'refant':''}, {'solnorm':True}, {'antnegsig':4.0}, {'antpossig':4.6}, {'tmantint':0.063}, {'tmint':0.085}, {'tmbl':0.175}, {'antblnegsig':3.4}, {'antblpossig':3.2}, {'relaxed_factor':2.0}, {'niter':2}, {'dryrun':False}, {'acceptresults':True}]), 
...
#
#
    def description(self, key='hifa_bandpassflag', subkey=None):
        desc={'hifa_bandpassflag': 'Compute bandpass calibration with flagging',
               'vis': 'List of input MeasurementSets',
               'caltable': 'List of output caltables',
               'intent': 'Set of data selection intents',
               'field': 'Field ids to use in data selection',
...
#
    ...
    def itsdefault(self, paramname) :
        ...
        if a.has_key(paramname) :
	      return a[paramname]
hifa_bandpassflag_cli = hifa_bandpassflag_cli_()
