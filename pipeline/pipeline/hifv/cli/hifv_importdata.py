...
import task_hifv_importdata
def hifv_importdata(vis=[''], session=[''], pipelinemode='automatic', asis='Receiver CalAtmosphere', overwrite=False, nocopy=False, createmms='automatic', ocorr_mode='co', dryrun=False, acceptresults=True):
...
#
	...
	trec = casac.utils().torecord(pathname+'hifv_importdata.xml')

        casalog.origin('hifv_importdata')
        if trec.has_key('hifv_importdata') and casac.utils().verify(mytmp, trec['hifv_importdata']) :
	    result = task_hifv_importdata.hifv_importdata(vis, session, pipelinemode, asis, overwrite, nocopy, createmms, ocorr_mode, dryrun, acceptresults)
