...
from types import *
from task_hifv_applycals import hifv_applycals
class hifv_applycals_cli_:
    __name__ = "hifv_applycals"
    rkey = None
    ...
    def __init__(self) :
       self.__bases__ = (hifv_applycals_cli_,)
       self.__doc__ = self.__call__.__doc__

       self.parameters={'vis':None, 'field':None, 'intent':None, 'spw':None, 'antenna':None, 'applymode':None, 'flagbackup':None, 'flagsum':None, 'flagdetailedsum':None, 'gainmap':None, 'pipelinemode':None, 'dryrun':None, 'acceptresults':None, }
    ...
    def __call__(self, vis=None, field=None, intent=None, spw=None, antenna=None, applymode=None, flagbackup=None, flagsum=None, flagdetailedsum=None, gainmap=None, pipelinemode=None, dryrun=None, acceptresults=None, ):
...
hifv_applycals()
	...
	#casalog = casac.casac.logsink()
        self.__globals__['__last_task'] = 'hifv_applycals'
        self.__globals__['taskname'] = 'hifv_applycals'
        ###
...
#
	...
	trec = casac.casac.utils().torecord(pathname+'hifv_applycals.xml')

        casalog.origin('hifv_applycals')
	try :
          #if not trec.has_key('hifv_applycals') or not casac.casac.utils().verify(mytmp, trec['hifv_applycals']) :
	    #return False

          casac.casac.utils().verify(mytmp, trec['hifv_applycals'], True)
          scriptstr=['']
          saveinputs = self.__globals__['saveinputs']
          if type(self.__call__.func_defaults) is NoneType:
              saveinputs=''
          else:
              saveinputs('hifv_applycals', 'hifv_applycals.last', myparams, self.__globals__,scriptstr=scriptstr)
          tname = 'hifv_applycals'
          spaces = ' '*(18-len(tname))
          casalog.post('\n##########################################'+
          ...
          result = hifv_applycals(vis, field, intent, spw, antenna, applymode, flagbackup, flagsum, flagdetailedsum, gainmap, pipelinemode, dryrun, acceptresults)
          if (casa['state']['telemetry-enabled']):
...
#            paramgui.setGlobals({})
#
#        paramgui.runTask('hifv_applycals', myf['_ip'])
#        paramgui.setGlobals({})
#
#
#
...
#
#
    def description(self, key='hifv_applycals', subkey=None):
        desc={'hifv_applycals': 'Applycals',
               'vis': 'List of input MeasurementSets',
               'field': 'Set of data selection field names or ids',
               'intent': 'Set of data selection observing intents',
               'spw': 'Set of data selection spectral window/channels',
...
#
    ...
    def itsdefault(self, paramname) :
        ...
        if a.has_key(paramname) :
	      return a[paramname]
hifv_applycals_cli = hifv_applycals_cli_()
