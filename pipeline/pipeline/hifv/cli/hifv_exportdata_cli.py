...
from types import *
from task_hifv_exportdata import hifv_exportdata
class hifv_exportdata_cli_:
    __name__ = "hifv_exportdata"
    rkey = None
    ...
    def __init__(self) :
       self.__bases__ = (hifv_exportdata_cli_,)
       self.__doc__ = self.__call__.__doc__

       self.parameters={'vis':None, 'session':None, 'exportmses':None, 'pprfile':None, 'calintents':None, 'calimages':None, 'targetimages':None, 'products_dir':None, 'gainmap':None, 'pipelinemode':None, 'dryrun':None, 'acceptresults':None, }
    ...
    def __call__(self, vis=None, session=None, exportmses=None, pprfile=None, calintents=None, calimages=None, targetimages=None, products_dir=None, gainmap=None, pipelinemode=None, dryrun=None, acceptresults=None, ):
	...
	Arguments :
		...
		exportmses:	Export MeasurementSets instead of flags and caltables
		   Default Value: False
...
The hifv_exportdata task for the VLA CASA pipeline exports the data defined in the pipeline context
...
The hifv_exportdata task exports the data defined in the pipeline context
...
only the gain calibrator images be saved.
	...
	#casalog = casac.casac.logsink()
        self.__globals__['__last_task'] = 'hifv_exportdata'
        self.__globals__['taskname'] = 'hifv_exportdata'
        ###
...
#
	...
	trec = casac.casac.utils().torecord(pathname+'hifv_exportdata.xml')

        casalog.origin('hifv_exportdata')
	try :
          #if not trec.has_key('hifv_exportdata') or not casac.casac.utils().verify(mytmp, trec['hifv_exportdata']) :
	    #return False

          casac.casac.utils().verify(mytmp, trec['hifv_exportdata'], True)
          scriptstr=['']
          saveinputs = self.__globals__['saveinputs']
          if type(self.__call__.func_defaults) is NoneType:
              saveinputs=''
          else:
              saveinputs('hifv_exportdata', 'hifv_exportdata.last', myparams, self.__globals__,scriptstr=scriptstr)
          tname = 'hifv_exportdata'
          spaces = ' '*(18-len(tname))
          casalog.post('\n##########################################'+
          ...
          result = hifv_exportdata(vis, session, exportmses, pprfile, calintents, calimages, targetimages, products_dir, gainmap, pipelinemode, dryrun, acceptresults)
          if (casa['state']['telemetry-enabled']):
...
#            paramgui.setGlobals({})
#
#        paramgui.runTask('hifv_exportdata', myf['_ip'])
#        paramgui.setGlobals({})
#
#
#
...
#
#
    def description(self, key='hifv_exportdata', subkey=None):
        desc={'hifv_exportdata': 'Prepare interferometry data for export',
               'vis': 'List of input visibility data',
               'session': 'List of sessions one per visibility file',
               'exportmses': 'Export MeasurementSets instead of flags and caltables',
               'pprfile': 'The pipeline processing request file to be exported',
...
#
    ...
    def itsdefault(self, paramname) :
        ...
        if a.has_key(paramname) :
	      return a[paramname]
hifv_exportdata_cli = hifv_exportdata_cli_()
