...
from types import *
from task_hifv_checkflag import hifv_checkflag
class hifv_checkflag_cli_:
    __name__ = "hifv_checkflag"
    rkey = None
    ...
    def __init__(self) :
       self.__bases__ = (hifv_checkflag_cli_,)
       self.__doc__ = self.__call__.__doc__

       self.parameters={'vis':None, 'checkflagmode':None, 'pipelinemode':None, 'dryrun':None, 'acceptresults':None, }
    ...
    def __call__(self, vis=None, checkflagmode=None, pipelinemode=None, dryrun=None, acceptresults=None, ):
	...
	Arguments :
		...
		checkflagmode:	String name of the mode to run checkflag
		   Default Value: 
...
1. Run RFLAG with associated heuristics in the VLA CASA pipeline.
   ...
   hifv_checkflag()
	...
	#casalog = casac.casac.logsink()
        self.__globals__['__last_task'] = 'hifv_checkflag'
        self.__globals__['taskname'] = 'hifv_checkflag'
        ###
	...
	if useLocalDefaults :
	   for item in function_signature_defaults.iteritems():
		   ...
		   if(type(keyVal) == dict) :
            ...
            myparams['checkflagmode'] = checkflagmode = self.parameters['checkflagmode']
...
#
	...
	trec = casac.casac.utils().torecord(pathname+'hifv_checkflag.xml')

        casalog.origin('hifv_checkflag')
	try :
          #if not trec.has_key('hifv_checkflag') or not casac.casac.utils().verify(mytmp, trec['hifv_checkflag']) :
	    #return False

          casac.casac.utils().verify(mytmp, trec['hifv_checkflag'], True)
          scriptstr=['']
          saveinputs = self.__globals__['saveinputs']
          if type(self.__call__.func_defaults) is NoneType:
              saveinputs=''
          else:
              saveinputs('hifv_checkflag', 'hifv_checkflag.last', myparams, self.__globals__,scriptstr=scriptstr)
          tname = 'hifv_checkflag'
          spaces = ' '*(18-len(tname))
          casalog.post('\n##########################################'+
          ...
          result = hifv_checkflag(vis, checkflagmode, pipelinemode, dryrun, acceptresults)
          if (casa['state']['telemetry-enabled']):
...
#            paramgui.setGlobals({})
#
#        paramgui.runTask('hifv_checkflag', myf['_ip'])
#        paramgui.setGlobals({})
#
#
#
#
	...
	if not hasattr(self, "__globals__") or self.__globals__ == None :
        ...
        a['pipelinemode'] = {
                    0:{'value':'automatic'}, 
                    1:odict([{'value':'interactive'}, {'vis':[]}, {'checkflagmode':''}, {'dryrun':False}, {'acceptresults':True}]), 
                    2:odict([{'value':'getinputs'}, {'vis':[]}, {'checkflagmode':''}])}
...
#
#
    def description(self, key='hifv_checkflag', subkey=None):
        desc={'hifv_checkflag': 'Run flagdata in rflag mode',
               'vis': 'List of input visibility data',
               'checkflagmode': 'String name of the mode to run checkflag',
               'pipelinemode': 'The pipeline operating mode',
               'dryrun': 'Run the task (False) or display task command (True)',
...
#
    ...
    def itsdefault(self, paramname) :
        ...
        if a.has_key(paramname) :
	      return a[paramname]
hifv_checkflag_cli = hifv_checkflag_cli_()
