...
from types import *
from task_hifv_circfeedpolcal import hifv_circfeedpolcal
class hifv_circfeedpolcal_cli_:
    __name__ = "hifv_circfeedpolcal"
    rkey = None
    ...
    def __init__(self) :
       self.__bases__ = (hifv_circfeedpolcal_cli_,)
       self.__doc__ = self.__call__.__doc__

       self.parameters={'vis':None, 'Dterm_solint':None, 'refantignore':None, 'leakage_poltype':None, 'mbdkcross':None, 'clipminmax':None, 'pipelinemode':None, 'dryrun':None, 'acceptresults':None, }
...
1. Basic circfeedpolcal task
   ...
   hifv_circfeedpolcal()
	...
	#casalog = casac.casac.logsink()
        self.__globals__['__last_task'] = 'hifv_circfeedpolcal'
        self.__globals__['taskname'] = 'hifv_circfeedpolcal'
        ###
	...
	if useLocalDefaults :
	   for item in function_signature_defaults.iteritems():
		   ...
		   if(type(keyVal) == dict) :
            ...
            myparams['Dterm_solint'] = Dterm_solint = self.parameters['Dterm_solint']
            myparams['refantignore'] = refantignore = self.parameters['refantignore']
            myparams['leakage_poltype'] = leakage_poltype = self.parameters['leakage_poltype']
            myparams['mbdkcross'] = mbdkcross = self.parameters['mbdkcross']
...
#
	...
	trec = casac.casac.utils().torecord(pathname+'hifv_circfeedpolcal.xml')

        casalog.origin('hifv_circfeedpolcal')
	try :
          #if not trec.has_key('hifv_circfeedpolcal') or not casac.casac.utils().verify(mytmp, trec['hifv_circfeedpolcal']) :
	    #return False

          casac.casac.utils().verify(mytmp, trec['hifv_circfeedpolcal'], True)
          scriptstr=['']
          saveinputs = self.__globals__['saveinputs']
          if type(self.__call__.func_defaults) is NoneType:
              saveinputs=''
          else:
              saveinputs('hifv_circfeedpolcal', 'hifv_circfeedpolcal.last', myparams, self.__globals__,scriptstr=scriptstr)
          tname = 'hifv_circfeedpolcal'
          spaces = ' '*(18-len(tname))
          casalog.post('\n##########################################'+
          ...
          result = hifv_circfeedpolcal(vis, Dterm_solint, refantignore, leakage_poltype, mbdkcross, clipminmax, pipelinemode, dryrun, acceptresults)
          if (casa['state']['telemetry-enabled']):
...
#            paramgui.setGlobals({})
#
#        paramgui.runTask('hifv_circfeedpolcal', myf['_ip'])
#        paramgui.setGlobals({})
#
#
#
#
	...
	if not hasattr(self, "__globals__") or self.__globals__ == None :
        ...
        a['leakage_poltype']  = ''
        a['mbdkcross']  = True
        a['clipminmax']  = [0.0,0.25]
        a['pipelinemode']  = 'automatic'

        a['pipelinemode'] = {
                    0:{'value':'automatic'}, 
                    1:odict([{'value':'interactive'}, {'vis':[]}, {'dryrun':False}, {'acceptresults':True}, {'Dterm_solint':''}, {'refantignore':''}, {'leakage_poltype':''}]), 
...
#
#
    def description(self, key='hifv_circfeedpolcal', subkey=None):
        desc={'hifv_circfeedpolcal': 'Base circfeedpolcal task',
               'vis': 'List of input visibility data',
               'Dterm_solint': 'D-terms spectral averaging',
               'refantignore': 'String list of antennas to ignore',
               'leakage_poltype': 'poltype to use in first polcal execution - blank string means use default heuristics',
...
#
    ...
    def itsdefault(self, paramname) :
        ...
        if a.has_key(paramname) :
	      return a[paramname]
hifv_circfeedpolcal_cli = hifv_circfeedpolcal_cli_()
