...
import task_hifv_exportdata
def hifv_exportdata(vis=[''], session=[''], exportmses=False, pprfile='', calintents='', calimages=[''], targetimages=[''], products_dir='', gainmap=False, pipelinemode='automatic', dryrun=False, acceptresults=True):
...
#
	...
	trec = casac.utils().torecord(pathname+'hifv_exportdata.xml')

        casalog.origin('hifv_exportdata')
        if trec.has_key('hifv_exportdata') and casac.utils().verify(mytmp, trec['hifv_exportdata']) :
	    result = task_hifv_exportdata.hifv_exportdata(vis, session, exportmses, pprfile, calintents, calimages, targetimages, products_dir, gainmap, pipelinemode, dryrun, acceptresults)
