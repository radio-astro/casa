...
from types import *
from task_hsd_blflag import hsd_blflag
class hsd_blflag_cli_:
    __name__ = "hsd_blflag"
    rkey = None
    ...
    def __init__(self) :
       self.__bases__ = (hsd_blflag_cli_,)
...
Examples
	...
	#casalog = casac.casac.logsink()
        self.__globals__['__last_task'] = 'hsd_blflag'
        self.__globals__['taskname'] = 'hsd_blflag'
        ###
...
#
	...
	trec = casac.casac.utils().torecord(pathname+'hsd_blflag.xml')

        casalog.origin('hsd_blflag')
	try :
          #if not trec.has_key('hsd_blflag') or not casac.casac.utils().verify(mytmp, trec['hsd_blflag']) :
	    #return False

          casac.casac.utils().verify(mytmp, trec['hsd_blflag'], True)
          scriptstr=['']
          saveinputs = self.__globals__['saveinputs']
          if type(self.__call__.func_defaults) is NoneType:
              saveinputs=''
          else:
              saveinputs('hsd_blflag', 'hsd_blflag.last', myparams, self.__globals__,scriptstr=scriptstr)
          tname = 'hsd_blflag'
          spaces = ' '*(18-len(tname))
          casalog.post('\n##########################################'+
...
#            paramgui.setGlobals({})
#
#        paramgui.runTask('hsd_blflag', myf['_ip'])
#        paramgui.setGlobals({})
#
#
#
...
#
#
    def description(self, key='hsd_blflag', subkey=None):
        desc={'hsd_blflag': 'Flag spectra based on predefined criteria of single-dish pipeline',
               'iteration': 'Number of iteration to perform sigma clipping to calculate threshold',
               'edge': 'Number of edge channels to be excluded from statistic calculation to flag data',
               'flag_tsys': 'Flag data by Tsys value',
               'tsys_thresh': 'Threshold for Tsys flag',
...
#
    ...
    def itsdefault(self, paramname) :
        ...
        if a.has_key(paramname) :
	      return a[paramname]
hsd_blflag_cli = hsd_blflag_cli_()
