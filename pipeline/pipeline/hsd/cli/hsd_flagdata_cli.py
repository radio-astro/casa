...
from types import *
from task_hsd_flagdata import hsd_flagdata
class hsd_flagdata_cli_:
    __name__ = "hsd_flagdata"
    rkey = None
    ...
    def __init__(self) :
       self.__bases__ = (hsd_flagdata_cli_,)
...
2. Do basic flagging on a MeasurementSet flagging additional scans selected
   by number as well.
	...
	#casalog = casac.casac.logsink()
        self.__globals__['__last_task'] = 'hsd_flagdata'
        self.__globals__['taskname'] = 'hsd_flagdata'
        ###
...
#
	...
	trec = casac.casac.utils().torecord(pathname+'hsd_flagdata.xml')

        casalog.origin('hsd_flagdata')
	try :
          #if not trec.has_key('hsd_flagdata') or not casac.casac.utils().verify(mytmp, trec['hsd_flagdata']) :
	    #return False

          casac.casac.utils().verify(mytmp, trec['hsd_flagdata'], True)
          scriptstr=['']
          saveinputs = self.__globals__['saveinputs']
          if type(self.__call__.func_defaults) is NoneType:
              saveinputs=''
          else:
              saveinputs('hsd_flagdata', 'hsd_flagdata.last', myparams, self.__globals__,scriptstr=scriptstr)
          tname = 'hsd_flagdata'
          spaces = ' '*(18-len(tname))
          casalog.post('\n##########################################'+
          ...
          result = hsd_flagdata(vis, autocorr, shadow, scan, scannumber, intents, edgespw, fracspw, fracspwfps, online, fileonline, template, filetemplate, hm_tbuff, tbuff, qa0, qa2, pipelinemode, flagbackup, dryrun, acceptresults)
          if (casa['state']['telemetry-enabled']):
...
#            paramgui.setGlobals({})
#
#        paramgui.runTask('hsd_flagdata', myf['_ip'])
#        paramgui.setGlobals({})
#
#
#
#
	...
	if not hasattr(self, "__globals__") or self.__globals__ == None :
        ...
        a['scan'] = {
                    0:odict([{'value':True}, {'scannumber':''}, {'intents':'POINTING,FOCUS,ATMOSPHERE,SIDEBAND,CHECK'}]), 
                    1:{'value':False}}
        a['edgespw'] = {
                    0:odict([{'value':True}, {'fracspw':'1.875GHz'}, {'fracspwfps':0.048387}]), 
                    1:{'value':False}}
        a['online'] = {
        ...
        a['hm_tbuff'] = {
                    0:{'value':'halfint'}, 
                    1:odict([{'value':'manual'}, {'tbuff':0.0}])}
...
#
#
    def description(self, key='hsd_flagdata', subkey=None):
        desc={'hsd_flagdata': 'Do basic flagging of a list of MeasurementSets',
               'vis': 'List of input MeasurementSets to flag',
               'autocorr': 'Flag autocorrelation data',
               'shadow': 'Flag shadowed antennas',
               'scan': 'Flag specified scans',
...
#
    ...
    def itsdefault(self, paramname) :
        ...
        a['scan']  = True
        a['scannumber']  = ''
        a['intents']  = 'POINTING,FOCUS,ATMOSPHERE,SIDEBAND,CHECK'
        a['edgespw']  = True
        a['fracspw']  = '1.875GHz'
        ...
        if a.has_key(paramname) :
	      return a[paramname]
hsd_flagdata_cli = hsd_flagdata_cli_()
