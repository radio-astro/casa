...
import task_hsd_restoredata
def hsd_restoredata(vis=[''], session=[''], products_dir='../products', copytoraw=True, rawdata_dir='../rawdata', lazy=False, bdfflags=True, ocorr_mode='ao', asis='SBSummary ExecBlock Antenna Station Receiver Source CalAtmosphere CalWVR', pipelinemode='automatic', dryrun=False, acceptresults=True):
...
#
	...
	trec = casac.utils().torecord(pathname+'hsd_restoredata.xml')

        casalog.origin('hsd_restoredata')
        if trec.has_key('hsd_restoredata') and casac.utils().verify(mytmp, trec['hsd_restoredata']) :
	    result = task_hsd_restoredata.hsd_restoredata(vis, session, products_dir, copytoraw, rawdata_dir, lazy, bdfflags, ocorr_mode, asis, pipelinemode, dryrun, acceptresults)
