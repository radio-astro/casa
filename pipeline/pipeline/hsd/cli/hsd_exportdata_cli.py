...
from types import *
from task_hsd_exportdata import hsd_exportdata
class hsd_exportdata_cli_:
    __name__ = "hsd_exportdata"
    rkey = None
    ...
    def __init__(self) :
       self.__bases__ = (hsd_exportdata_cli_,)
       self.__doc__ = self.__call__.__doc__

       self.parameters={'pprfile':None, 'targetimages':None, 'products_dir':None, 'pipelinemode':None, 'dryrun':None, 'acceptresults':None, }
    ...
    def __call__(self, pprfile=None, targetimages=None, products_dir=None, pipelinemode=None, dryrun=None, acceptresults=None, ):
	...
	Arguments :
		pprfile:	The pipeline processing request(PPR) file to be exported
		   Default Value: 

		targetimages:	List of target CASA images to be exported
		   Default Value: 
...
directory
	...
	#casalog = casac.casac.logsink()
        self.__globals__['__last_task'] = 'hsd_exportdata'
        self.__globals__['taskname'] = 'hsd_exportdata'
        ###
...
#
	...
	trec = casac.casac.utils().torecord(pathname+'hsd_exportdata.xml')

        casalog.origin('hsd_exportdata')
	try :
          #if not trec.has_key('hsd_exportdata') or not casac.casac.utils().verify(mytmp, trec['hsd_exportdata']) :
	    #return False

          casac.casac.utils().verify(mytmp, trec['hsd_exportdata'], True)
          scriptstr=['']
          saveinputs = self.__globals__['saveinputs']
          if type(self.__call__.func_defaults) is NoneType:
              saveinputs=''
          else:
              saveinputs('hsd_exportdata', 'hsd_exportdata.last', myparams, self.__globals__,scriptstr=scriptstr)
          tname = 'hsd_exportdata'
          spaces = ' '*(18-len(tname))
          casalog.post('\n##########################################'+
          ...
          result = hsd_exportdata(pprfile, targetimages, products_dir, pipelinemode, dryrun, acceptresults)
          if (casa['state']['telemetry-enabled']):
...
#            paramgui.setGlobals({})
#
#        paramgui.runTask('hsd_exportdata', myf['_ip'])
#        paramgui.setGlobals({})
#
#
#
#
	...
	if not hasattr(self, "__globals__") or self.__globals__ == None :
        ...
        a['pipelinemode'] = {
                    0:{'value':'automatic'}, 
                    1:odict([{'value':'interactive'}, {'pprfile':''}, {'targetimages':[]}, {'products_dir':''}, {'dryrun':False}, {'acceptresults':True}]), 
                    2:odict([{'value':'getinputs'}, {'pprfile':''}, {'targetimages':[]}, {'products_dir':''}])}
...
#
#
    def description(self, key='hsd_exportdata', subkey=None):
        desc={'hsd_exportdata': 'Prepare singledish data for export',
               'pprfile': 'The pipeline processing request(PPR) file to be exported',
               'targetimages': 'List of target CASA images to be exported',
               'products_dir': 'The data products directory',
               'pipelinemode': 'The pipeline operating mode',
...
#
    ...
    def itsdefault(self, paramname) :
        ...
        if a.has_key(paramname) :
	      return a[paramname]
hsd_exportdata_cli = hsd_exportdata_cli_()
