...
% for field in sparsemap_subpage.keys():
    <h4><a class="replace"
           href="${os.path.relpath(os.path.join(dirname, sparsemap_subpage[field]), pcontext.report_dir)}">${field}</a>
    </h4>
    <ul class="thumbnails">
        % for plot in sparsemap_plots[field]:
            ...
            <li class="span3">
                <div class="thumbnail">
                    ...
                    <div class="caption">
                        <h4>
                            <a href="${os.path.relpath(os.path.join(dirname, sparsemap_subpage[field]), pcontext.report_dir)}"
                               class="replace"
                        ...
                        % for (ant, pols) in profilemap_entries[field].items():
                            ...
                            % for pol in pols:
                                <td align="center">
                                <a href="${os.path.relpath(os.path.join(dirname, profilemap_subpage[field]), pcontext.report_dir)}"
                                   class="replace"
                                   data-spw="${plot.parameters['spw']}"
                                   data-ant="${ant}"
                                   data-pol="${pol}">
...
% for plots in plots_list:
    ...
    % for field in plots['subpage'].keys():
        <h4><a class="replace"
               href="${os.path.relpath(os.path.join(dirname, plots['subpage'][field]), pcontext.report_dir)}">${field}</a>
        </h4>
        <ul class="thumbnails">
            % for plot in plots['plot'][field]:
                ...
                <li class="span3">
                    <div class="thumbnail">
                        ...
                        <div class="caption">
                            <h4>
                                <a href="${os.path.relpath(os.path.join(dirname, plots['subpage'][field]), pcontext.report_dir)}"
                                   class="replace"
                                   data-spw="${plot.parameters['spw']}"
                                   data-ant="${plot.parameters['ant']}">
                                   Spectral Window ${plot.parameters['spw']}
