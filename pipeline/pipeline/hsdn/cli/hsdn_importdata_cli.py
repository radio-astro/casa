...
from types import *
from task_hsdn_importdata import hsdn_importdata
class hsdn_importdata_cli_:
    __name__ = "hsdn_importdata"
    rkey = None
    ...
    def __init__(self) :
       self.__bases__ = (hsdn_importdata_cli_,)
       self.__doc__ = self.__call__.__doc__

       self.parameters={'vis':None, 'session':None, 'pipelinemode':None, 'overwrite':None, 'nocopy':None, 'createmms':None, 'dryrun':None, 'acceptresults':None, }
    ...
    def __call__(self, vis=None, session=None, pipelinemode=None, overwrite=None, nocopy=None, createmms=None, dryrun=None, acceptresults=None, ):
...
6. Run in  dryrun mode before running for real
   ...
   results = hsdn_importdata (vis=['uid___A002_X30a93d_X43e.ms'])
	...
	#casalog = casac.casac.logsink()
        self.__globals__['__last_task'] = 'hsdn_importdata'
        self.__globals__['taskname'] = 'hsdn_importdata'
        ###
...
#
	...
	trec = casac.casac.utils().torecord(pathname+'hsdn_importdata.xml')

        casalog.origin('hsdn_importdata')
	try :
          #if not trec.has_key('hsdn_importdata') or not casac.casac.utils().verify(mytmp, trec['hsdn_importdata']) :
	    #return False

          casac.casac.utils().verify(mytmp, trec['hsdn_importdata'], True)
          scriptstr=['']
          saveinputs = self.__globals__['saveinputs']
          if type(self.__call__.func_defaults) is NoneType:
              saveinputs=''
          else:
              saveinputs('hsdn_importdata', 'hsdn_importdata.last', myparams, self.__globals__,scriptstr=scriptstr)
          tname = 'hsdn_importdata'
          spaces = ' '*(18-len(tname))
          casalog.post('\n##########################################'+
          ...
          result = hsdn_importdata(vis, session, pipelinemode, overwrite, nocopy, createmms, dryrun, acceptresults)
          if (casa['state']['telemetry-enabled']):
...
#            paramgui.setGlobals({})
#
#        paramgui.runTask('hsdn_importdata', myf['_ip'])
#        paramgui.setGlobals({})
#
#
#
#
	...
	if not hasattr(self, "__globals__") or self.__globals__ == None :
        ...
        a['pipelinemode'] = {
                    0:{'value':'automatic'}, 
                    1:odict([{'value':'interactive'}, {'overwrite':False}, {'nocopy':False}, {'dryrun':False}, {'acceptresults':True}]), 
                    2:odict([{'value':'getinputs'}, {'overwrite':False}, {'nocopy':False}])}
...
#
#
    def description(self, key='hsdn_importdata', subkey=None):
        desc={'hsdn_importdata': 'Imports Nobeyama data into the single dish pipeline',
               'vis': 'List of input visibility data',
               'session': 'List of visibility data sessions',
               'pipelinemode': 'The pipeline operating mode',
               'overwrite': 'Overwrite existing files on import',
...
#
    ...
    def itsdefault(self, paramname) :
        ...
        if a.has_key(paramname) :
	      return a[paramname]
hsdn_importdata_cli = hsdn_importdata_cli_()
