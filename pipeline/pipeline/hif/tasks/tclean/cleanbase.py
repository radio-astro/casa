    ...
    def _do_clean_cycle (self, scanidlist=None, result=None, iter=1):
        ...
        if (inputs.niter > 0):
            ...
            if ((tclean_result['stopcode'] == 1) and (tclean_result['iterdone'] >= tclean_result['niter'])):
                result.error = CleanBaseError('tclean reached niter limit. Field: %s SPW: %s' % (inputs.field, inputs.spw), 'niter limit')
        ...
        # Keep threshold and sensitivity bandwidth for QA and weblog
