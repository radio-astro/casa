...
import task_hif_uvcontsub
def hif_uvcontsub(vis=[''], field='', intent='', spw='', applymode='', pipelinemode='automatic', dryrun=False, acceptresults=True):

        """Subtract the fitted continuum from the data

Apply the UV continuum fit stored in the callibrary to the data.
...
#
	...
	trec = casac.utils().torecord(pathname+'hif_uvcontsub.xml')

        casalog.origin('hif_uvcontsub')
        if trec.has_key('hif_uvcontsub') and casac.utils().verify(mytmp, trec['hif_uvcontsub']) :
	    result = task_hif_uvcontsub.hif_uvcontsub(vis, field, intent, spw, applymode, pipelinemode, dryrun, acceptresults)
