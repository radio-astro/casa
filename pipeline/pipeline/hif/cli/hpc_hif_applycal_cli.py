...
from types import *
from task_hpc_hif_applycal import hpc_hif_applycal
class hpc_hif_applycal_cli_:
    __name__ = "hpc_hif_applycal"
    rkey = None
    ...
    def __init__(self) :
       self.__bases__ = (hpc_hif_applycal_cli_,)
       self.__doc__ = self.__call__.__doc__

       self.parameters={'vis':None, 'field':None, 'intent':None, 'spw':None, 'antenna':None, 'applymode':None, 'calwt':None, 'flagbackup':None, 'flagsum':None, 'flagdetailedsum':None, 'pipelinemode':None, 'dryrun':None, 'acceptresults':None, 'parallel':None, }
...
hpc_hif_applycal (intent='TARGET')
	...
	#casalog = casac.casac.logsink()
        self.__globals__['__last_task'] = 'hpc_hif_applycal'
        self.__globals__['taskname'] = 'hpc_hif_applycal'
        ###
...
#
	...
	trec = casac.casac.utils().torecord(pathname+'hpc_hif_applycal.xml')

        casalog.origin('hpc_hif_applycal')
	try :
          #if not trec.has_key('hpc_hif_applycal') or not casac.casac.utils().verify(mytmp, trec['hpc_hif_applycal']) :
	    #return False

          casac.casac.utils().verify(mytmp, trec['hpc_hif_applycal'], True)
          scriptstr=['']
          saveinputs = self.__globals__['saveinputs']
          if type(self.__call__.func_defaults) is NoneType:
              saveinputs=''
          else:
              saveinputs('hpc_hif_applycal', 'hpc_hif_applycal.last', myparams, self.__globals__,scriptstr=scriptstr)
          tname = 'hpc_hif_applycal'
          spaces = ' '*(18-len(tname))
          casalog.post('\n##########################################'+
          ...
          result = hpc_hif_applycal(vis, field, intent, spw, antenna, applymode, calwt, flagbackup, flagsum, flagdetailedsum, pipelinemode, dryrun, acceptresults, parallel)
          if (casa['state']['telemetry-enabled']):
...
#            paramgui.setGlobals({})
#
#        paramgui.runTask('hpc_hif_applycal', myf['_ip'])
#        paramgui.setGlobals({})
#
#
#
#
	...
	if not hasattr(self, "__globals__") or self.__globals__ == None :
        ...
        a['pipelinemode'] = {
                    0:{'value':'automatic'}, 
                    1:odict([{'value':'interactive'}, {'vis':[]}, {'field':''}, {'intent':''}, {'spw':''}, {'antenna':''}, {'calwt':[True]}, {'flagbackup':True}, {'dryrun':False}, {'acceptresults':True}, {'parallel':'automatic'}]), 
                    2:odict([{'value':'getinputs'}, {'vis':[]}, {'field':''}, {'intent':''}, {'spw':''}, {'antenna':''}, {'calwt':[True]}, {'flagbackup':True}])}
        a['flagsum'] = {
                    0:odict([{'value':True}, {'flagdetailedsum':True}]), 
                    1:{'value':False}}
...
#
#
    def description(self, key='hpc_hif_applycal', subkey=None):
        desc={'hpc_hif_applycal': 'Apply the calibration(s) to the data',
               'vis': 'List of input MeasurementSets',
               'field': 'Set of data selection field names or ids',
               'intent': 'Set of data selection observing intents',
               'spw': 'Set of data selection spectral window/channels',
...
#
    ...
    def itsdefault(self, paramname) :
        ...
        if a.has_key(paramname) :
	      return a[paramname]
hpc_hif_applycal_cli = hpc_hif_applycal_cli_()
