...
from types import *
from task_h_tsyscal import h_tsyscal
class h_tsyscal_cli_:
    __name__ = "h_tsyscal"
    rkey = None
    ...
    def __init__(self) :
       self.__bases__ = (h_tsyscal_cli_,)
       self.__doc__ = self.__call__.__doc__

       self.parameters={'vis':None, 'caltable':None, 'chantol':None, 'pipelinemode':None, 'dryrun':None, 'acceptresults':None, }
    ...
    def __call__(self, vis=None, caltable=None, chantol=None, pipelinemode=None, dryrun=None, acceptresults=None, ):
...
Derive the Tsys calibration for list of ALMA MeasurementSets.
	...
	Arguments :
		...
		chantol:	Tsys spectral window map channel tolerance
		   Default Value: 1
...
Example
	...
	#casalog = casac.casac.logsink()
        self.__globals__['__last_task'] = 'h_tsyscal'
        self.__globals__['taskname'] = 'h_tsyscal'
        ###
	...
	if useLocalDefaults :
	   for item in function_signature_defaults.iteritems():
		   ...
		   if(type(keyVal) == dict) :
            ...
            myparams['chantol'] = chantol = self.parameters['chantol']
...
#
	...
	trec = casac.casac.utils().torecord(pathname+'h_tsyscal.xml')

        casalog.origin('h_tsyscal')
	try :
          #if not trec.has_key('h_tsyscal') or not casac.casac.utils().verify(mytmp, trec['h_tsyscal']) :
	    #return False

          casac.casac.utils().verify(mytmp, trec['h_tsyscal'], True)
          scriptstr=['']
          saveinputs = self.__globals__['saveinputs']
          if type(self.__call__.func_defaults) is NoneType:
              saveinputs=''
          else:
              saveinputs('h_tsyscal', 'h_tsyscal.last', myparams, self.__globals__,scriptstr=scriptstr)
          tname = 'h_tsyscal'
          spaces = ' '*(18-len(tname))
          casalog.post('\n##########################################'+
          ...
          result = h_tsyscal(vis, caltable, chantol, pipelinemode, dryrun, acceptresults)
          if (casa['state']['telemetry-enabled']):
...
#            paramgui.setGlobals({})
#
#        paramgui.runTask('h_tsyscal', myf['_ip'])
#        paramgui.setGlobals({})
#
#
#
...
#
#
    def description(self, key='h_tsyscal', subkey=None):
        desc={'h_tsyscal': 'Derive a Tsys calibration table',
               'vis': 'List of input MeasurementSets',
               'caltable': 'List of output caltable(s)',
               'chantol': 'Tsys spectral window map channel tolerance',
               'pipelinemode': 'The pipeline operations mode',
