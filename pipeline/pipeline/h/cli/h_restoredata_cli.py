...
from types import *
from task_h_restoredata import h_restoredata
class h_restoredata_cli_:
    __name__ = "h_restoredata"
    rkey = None
    ...
    def __init__(self) :
       self.__bases__ = (h_restoredata_cli_,)
...
The h_restoredata task restores flagged and calibrated MeasurementSets
...
The h_restoredata restores flagged and calibrated data from archived
ASDMs and pipeline flagging and calibration data products. Pending archive
retrieval support h_restoredata assumes that the required products
are available in the rawdata_dir in the format produced by the
h_exportdata task.

h_restoredata assumes that the following entities are available in the raw
data directory
...
1. Restore the pipeline results for a single ASDM in a single session 
	...
	#casalog = casac.casac.logsink()
        self.__globals__['__last_task'] = 'h_restoredata'
        self.__globals__['taskname'] = 'h_restoredata'
        ###
...
#
	...
	trec = casac.casac.utils().torecord(pathname+'h_restoredata.xml')

        casalog.origin('h_restoredata')
	try :
          #if not trec.has_key('h_restoredata') or not casac.casac.utils().verify(mytmp, trec['h_restoredata']) :
	    #return False

          casac.casac.utils().verify(mytmp, trec['h_restoredata'], True)
          scriptstr=['']
          saveinputs = self.__globals__['saveinputs']
          if type(self.__call__.func_defaults) is NoneType:
              saveinputs=''
          else:
              saveinputs('h_restoredata', 'h_restoredata.last', myparams, self.__globals__,scriptstr=scriptstr)
          tname = 'h_restoredata'
          spaces = ' '*(18-len(tname))
          casalog.post('\n##########################################'+
          ...
          result = h_restoredata(vis, session, products_dir, copytoraw, rawdata_dir, lazy, bdfflags, ocorr_mode, asis, pipelinemode, dryrun, acceptresults)
          if (casa['state']['telemetry-enabled']):
...
#            paramgui.setGlobals({})
#
#        paramgui.runTask('h_restoredata', myf['_ip'])
#        paramgui.setGlobals({})
#
#
#
...
#
#
    def description(self, key='h_restoredata', subkey=None):
        desc={'h_restoredata': 'Restore flagged and calibration interferometry data from a pipeline run',
               'vis': 'List of input visibility data',
               'session': 'List of sessions one per visibility file',
               'products_dir': 'The archived pipeline data products directory',
               'copytoraw': 'Copy calibration and flagging tables to raw data directory',
...
#
    ...
    def itsdefault(self, paramname) :
        ...
        if a.has_key(paramname) :
	      return a[paramname]
h_restoredata_cli = h_restoredata_cli_()
