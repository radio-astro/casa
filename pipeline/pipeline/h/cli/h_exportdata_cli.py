...
from types import *
from task_h_exportdata import h_exportdata
class h_exportdata_cli_:
    __name__ = "h_exportdata"
    rkey = None
    ...
    def __init__(self) :
       self.__bases__ = (h_exportdata_cli_,)
...
only the gain calibrator images be saved.
	...
	#casalog = casac.casac.logsink()
        self.__globals__['__last_task'] = 'h_exportdata'
        self.__globals__['taskname'] = 'h_exportdata'
        ###
...
#
	...
	trec = casac.casac.utils().torecord(pathname+'h_exportdata.xml')

        casalog.origin('h_exportdata')
	try :
          #if not trec.has_key('h_exportdata') or not casac.casac.utils().verify(mytmp, trec['h_exportdata']) :
	    #return False

          casac.casac.utils().verify(mytmp, trec['h_exportdata'], True)
          scriptstr=['']
          saveinputs = self.__globals__['saveinputs']
          if type(self.__call__.func_defaults) is NoneType:
              saveinputs=''
          else:
              saveinputs('h_exportdata', 'h_exportdata.last', myparams, self.__globals__,scriptstr=scriptstr)
          tname = 'h_exportdata'
          spaces = ' '*(18-len(tname))
          casalog.post('\n##########################################'+
          ...
          result = h_exportdata(vis, session, imaging_products_only, exportmses, pprfile, calintents, calimages, targetimages, products_dir, pipelinemode, dryrun, acceptresults)
          if (casa['state']['telemetry-enabled']):
...
#            paramgui.setGlobals({})
#
#        paramgui.runTask('h_exportdata', myf['_ip'])
#        paramgui.setGlobals({})
#
#
#
...
#
#
    def description(self, key='h_exportdata', subkey=None):
        desc={'h_exportdata': 'Prepare interferometry data for export',
               'vis': 'List of input visibility data',
               'session': 'List of sessions one per visibility file',
               'imaging_products_only': 'Export the science target image products only',
               'exportmses': 'Export MeasurementSets instead of flags and caltables',
...
#
    ...
    def itsdefault(self, paramname) :
        ...
        if a.has_key(paramname) :
	      return a[paramname]
h_exportdata_cli = h_exportdata_cli_()
