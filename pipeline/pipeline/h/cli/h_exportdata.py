...
import task_h_exportdata
def h_exportdata(vis=[''], session=[''], imaging_products_only=False, exportmses=False, pprfile='', calintents='', calimages=[''], targetimages=[''], products_dir='', pipelinemode='automatic', dryrun=False, acceptresults=True):
...
#
	...
	trec = casac.utils().torecord(pathname+'h_exportdata.xml')

        casalog.origin('h_exportdata')
        if trec.has_key('h_exportdata') and casac.utils().verify(mytmp, trec['h_exportdata']) :
	    result = task_h_exportdata.h_exportdata(vis, session, imaging_products_only, exportmses, pprfile, calintents, calimages, targetimages, products_dir, pipelinemode, dryrun, acceptresults)
