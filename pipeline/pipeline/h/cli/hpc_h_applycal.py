...
import task_hpc_h_applycal
def hpc_h_applycal(vis=[''], field='', intent='', spw='', antenna='', applymode='', flagbackup=True, flagsum=True, flagdetailedsum=False, pipelinemode='automatic', dryrun=False, acceptresults=True, parallel='automatic'):
...
#
	...
	trec = casac.utils().torecord(pathname+'hpc_h_applycal.xml')

        casalog.origin('hpc_h_applycal')
        if trec.has_key('hpc_h_applycal') and casac.utils().verify(mytmp, trec['hpc_h_applycal']) :
	    result = task_hpc_h_applycal.hpc_h_applycal(vis, field, intent, spw, antenna, applymode, flagbackup, flagsum, flagdetailedsum, pipelinemode, dryrun, acceptresults, parallel)
