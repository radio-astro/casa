...
ImageAnalysis::~ImageAnalysis() {
	  ...
	  if((pImage_p->isPersistent()) && ((pImage_p->imageType()) == "PagedImage")){
	    ...
	    if (type == ImageOpener::AIPSPP) {
	      ...
	      if (pip) {
		// SRankin: commented the following line out as it does not appear to be needed
		// in the typical case, and in some cases causes CASA to crash.  This change will 
		// be reviewed after the CASA-3.2.0 release.
