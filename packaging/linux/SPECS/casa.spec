...
%if %{el6}
%define thirdparty_path /usr/lib64/casa/%{thirdparty}
#-DPYTHON_SHARED_LINKER_FLAGS=-lpython2.6 ensures that libcasa_python.so is
#explicitly linked against libpython2.6.so so that we can affect loading of
#libpython2.6 by adjusting the runpath of libcasa_python.so
%define boost_python_loading -DPYTHON_SHARED_LINKER_FLAGS=-lpython2.6
%else
%if %{el7}0
%define boost_python_loading "-Wl,-rpath=%{libdir}:%{thirdparty_path}/lib:%{thirdparty_path}/lib/libsakura/default/lib,--enable-new-dtags"
%else
%define boost_python_loading ""
%endif
%define thirdparty_path /opt/casa/%{thirdparty}
%endif
...
export PATH=`pwd`/linux/bin:%{bindir}:%{topdir}/%{thirdparty}/bin:%{thirdparty_path}/bin:$PATH
export LD_LIBRARY_PATH=`pwd`/linux/lib:%{libdir}:%{topdir}/%{thirdparty}/lib:$LD_LIBRARY_PATH
%if %{el7}0
export LDFLAGS=-Wl,-rpath=%{libdir}:%{thirdparty_path}/lib:%{thirdparty_path}/lib/libsakura/default/lib,--enable-new-dtags
%endif
...
cmake $COMPILERS -DBoost_NO_BOOST_CMAKE=1 -DCASA_BUILD=1 -DBUILD_TESTING=OFF '-DCMAKE_INSTALL_PREFIX=../../linux' -DBUILD_PYTHON=1 -DPYTHON_INCLUDE_DIR=%{thirdparty_path}/include/python2.7 -DPYTHON_LIBRARY=%{thirdparty_path}/lib/libpython2.7.so -DCMAKE_BUILD_TYPE=Release %{boost_python_loading} ..
...
cd $top/asap
mkdir build && cd build
cmake $COMPILERS -DBoost_NO_BOOST_CMAKE=1 -Darch=linux -DPYTHONLIBD=%{pylibdir} -DPYTHONTASKD=%{pylibdir} -DCMAKE_BUILD_TYPE=Release %{boost_python_loading} ..
...
export CASAPATH="`pwd` linux"
export PATH=`pwd`/linux/bin:%{topdir}/%{thirdparty}/bin:%{thirdparty_path}/bin:$PATH
...
ln -s %{thirdparty_path}/bin/mpicasa
popd
...
%if %{el7}0
        chrpath -r "%{libdir}:%{thirdparty_path}/lib:%{thirdparty_path}/lib/libsakura/default/lib" $i
%else
        chrpath -r "%{libdir}:%{thirdparty_path}/lib" $i
%endif
    fi
  fi
done
