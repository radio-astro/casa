...
%define finddata /usr/src/rpmbuild/build/SOURCES/casa/find-data-version
...
%define ver           4.6.0
%define rev           %(/usr/src/rpmbuild/build/SOURCES/%{name}/svn-revision branch=release-4_6)
%if %{?ver:1}0
%define CASAVER       %{ver}
%else
...
Version:       %{CASAVER}
Autoreqprov:   on
Summary:       Symlinks in /usr/bin for running casa (built on RH%{DISTRO_REL}).
...
%package opt
Summary:       Symlinks in /opt/casa/bin for running casa (built on RH%{DISTRO_REL}).
Group:         Applications
Version:       %{CASAVER}
Autoreqprov:   on
Requires:      %{name}-bin = %{version}-%{release}
...
cmake28 $COMPILERS -DBoost_NO_BOOST_CMAKE=1 -DCASA_BUILD=1 -DBUILD_TESTING=OFF '-DCMAKE_INSTALL_PREFIX=../../linux' -DBUILD_PYTHON=1 -DPYTHON_INCLUDE_DIR=/usr/lib64/casa/01/include/python2.7 -DPYTHON_LIBRARY=/usr/lib64/casa/01/lib/libpython2.7.so -DBOOST_ROOT=/usr/lib64/casa/01 ..
gmake 'VERBOSE=1' install

cd $top/code
mkdir build
cd build
cmake28 $COMPILERS -DBoost_NO_BOOST_CMAKE=1 -DEXTRA_C_FLAGS="-DPG_PPU -I/usr/include/wcslib-4.3" -Darch=linux -DSKIP_PGPLOT="yes" -DPYTHONLIBD=%{pylibdir} -DPYTHONTASKD=%{pylibdir} ..
gmake 'VERBOSE=1'

cd $top/gcwrap
mkdir build && cd build
cmake28 $COMPILERS -DBoost_NO_BOOST_CMAKE=1 -Darch=linux -DPYTHONLIBD=%{pylibdir} ..
gmake 'VERBOSE=1'

cd $top/asap
mkdir build && cd build
cmake28 $COMPILERS -DBoost_NO_BOOST_CMAKE=1 -DEXTRA_C_FLAGS="-I/usr/include/wcslib-4.3" -Darch=linux -DPYTHONLIBD=%{pylibdir} -DPYTHONTASKD=%{pylibdir} ..
gmake 'VERBOSE=1'

###
### make-app depends on the workspace having qcasabrowser...
### (we should fix the casabrowser/qcasabrowser indirection "feature"... )
### below we had another crack at fixing this issue...
###
#cd $top
#%{SOURCE60}
...
# --- --- --- --- /opt/casa/bin wrappers --- --- --- ---
mkdir -p $RPM_BUILD_ROOT%{optdir}/bin
cp -p %{SOURCE100} $RPM_BUILD_ROOT%{optdir}/bin
pushd $RPM_BUILD_ROOT%{optdir}/bin
for i in asdm2MS casabrowser casa-config casafeather casalogger casaplotms casaviewer casapy; do
    ln -s casa $i
done
ln -s /usr/lib64/casa/01/bin/mpicasa
...
%{optdir}/bin/casapy
%{optdir}/bin/asdm2MS
%{optdir}/bin/casabrowser
%{optdir}/bin/casa-config
%{optdir}/bin/casafeather
%{optdir}/bin/casalogger
%{optdir}/bin/casaplotms
%{optdir}/bin/casaviewer
%{optdir}/bin/mpicasa
