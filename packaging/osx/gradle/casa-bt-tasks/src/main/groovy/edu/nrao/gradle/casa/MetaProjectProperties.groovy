    ...
    static {
        ...
        KEY=null ->
            if ( ! properties.containsKey(delegate) ) {
                ...
                def os = Sys.os( )
                def init_files = [ "gradle.properties",
                                   ...
                                   "gradle.properties.${hostname}.${username}"
                                 ].inject( [] ) { acc, it ->
                                                    def f = new File("${root}/${it}")
                                                    if ( f.isFile( ) ) acc.add(f)
                                                    return acc }
                def props = defaultProperties( )
                init_files.each {
                    println "loading properties from ${it}"
                    def latest = new java.util.Properties( )
                    latest.load(new FileInputStream(it))
                    // support recursive references to properties loaded earlier...
                    latest.inject(latest) {
                        acc,k,v ->
                            if ( v ==~ /.*\Q[:${k}:]\E.*/ ) {
                                // recursive expansion
                                def last_v = props.getProperty(k)
                                assert last_v != null :
                                "${it}: recursive property reference (${k}) only supported if " +
                                    "property was previously defined"
                                acc.setProperty(k,v.replaceAll(/\Q[:${k}:]\E/,last_v))
                            } else {
                                // immediate (e.g. [!var!]) expansion
                                def immediate = v.findAll(/\Q[!\E(.+?)\Q!]\E/) { full, part -> part }
                                if ( immediate.size( ) > 0 ) {
                                    acc.setProperty( k, immediate.inject(v) {
                                                         str, match ->
                                                             def last_v = props.getProperty(match)
                                                             assert last_v != null :
                                                             "${it}: immediate property reference " + 
                                                             "(${match}) only supported if property " +
                                                             "was previously defined"
                                                             str.replaceAll(/\Q[!${match}!]\E/,last_v)
                                                     } )
                                } else {
                                    acc.setProperty(k,v)
                                }
                    ...
                    latest.each { k,v -> props.setProperty(k,v) }
                }
                    ...
                    props.each{ key,value->
                        props.setProperty(key,orig.inject(value){
                                              acc,k,v ->
                                                  ...
                                                  // that there is no way in java/groovy to have
                                                  // "[:${key}:]" interpreted as a simple string of
                ...
                // sweep through and replace all remaining, unexpanded "[:KEY:]"
                // elements with empty string
                props.each{ key,value-> 
                              def newval = value.replaceAll(/\Q[:\E.+\Q:]\E/,"")
                              if ( value != newval ) props.setProperty(key,newval) }
