    ...
    def runRegression( String tag="smoke",
		...
		return runTestAction( tag, output_root, testing_root, work_root ) {
			test, ver, rev, wrapper, test_dir, output_dir, env ->
				...
				// Resolve pybot location for linux
				def wrapperAbsPath = wrapper.absolutePath
				def binDir = ""
				if  ( distro_os_ == 'linux' ) {
					binDir=wrapperAbsPath.substring(0,wrapperAbsPath.lastIndexOf("/")+1)
				}
				...
				//println "AA>>>>>>-----> cmd: ${wrapper} --exec pybot --outputdir ${output_dir} --logtitle \"${title} Log\" --reporttitle \"${title} Report\" --include ${test} casa"
				def cmd = "${wrapper} --exec pybot --outputdir ${output_dir} --logtitle \"${title} Log\" --reporttitle \"${title} Report\" --include ${test} casa"
				if ( distro_os_ == 'linux' ) {
					cmd = "${wrapper} --exec python ${binDir}pybot --outputdir ${output_dir} --logtitle \"${title} Log\" --reporttitle \"${title} Report\" --include ${test} casa"
				}
				println "runRegression: running pybot with: " + cmd
				return execute( cmd, test_dir, env, 0 )
		}
    ...
    def runTests( List tests,
		...
		return runTestAction( tests, output_root, testing_root, work_root ) {
			test, ver, rev, wrapper, test_dir, output_dir, env ->
				...
				// trying to use single quotes to make the titles a single argument is hopeless...
				def title = "CASA ${ver} (${rev})"
				//println "BB>>>>>>-----> cmd: ${wrapper} --exec python pybot --outputdir ${output_dir} --logtitle \"${title} Log\" --reporttitle \"${title} Report\" ${test_args} casa"
				def cmd="${wrapper} --exec pybot --outputdir ${output_dir} --logtitle \"${title} Log\" --reporttitle \"${title} Report\" ${test_args} casa"
				if ( distro_os_ == 'linux' ) {
					cmd="${wrapper} --exec python ${binDir}pybot --outputdir ${output_dir} --logtitle \"${title} Log\" --reporttitle \"${title} Report\" ${test_args} casa"
				}
				println "Running pybot with: " + cmd
				return execute( cmd, test_dir, env, 0 )
		}
