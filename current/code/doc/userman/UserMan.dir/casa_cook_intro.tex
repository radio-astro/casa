...
cheatsheet, example scripts of processing your data in CASA,
...
The CASA release 3.2.0 is the third release of CASA. The main feature
improvements are listed below. In addition to the new features, many
bugs were addressed and fixed.

We occasionally issue patches and 'stable' versions of CASA. To get
notified, please subscribe to the 'casa-users' mailing list. Releases
will be announced via the 'casa-announce' mailing list. To subscribe,
please visit the mailing lists webpage. For feedback, and help please
go to the CASA webpage {\url http://casa.nrao.edu} or visit the NRAO
helpdesk {\url http://help.nrao.edu}.
...
interferometers can be imported from uvfits or sdfits formats into
measurements sets (ms) or scantables in CASA. Given the variety of
non-standard fits formats, we cannot guarantee that CASA will fully
support data from all telescopes. However, efforts are made to support
data formats from other facilities. 
%%%%%%%%%%%%%
...
\begin{itemize}
    ...
    \item Mac OS
         \begin{itemize}
          \item Mac OS 10.5 (Leopard; 32-bit)
...
\subsection{What's New in Release 3.2.0}
...
\item {\bf Flagging}
    \begin{itemize} 
     \item New task {\tt flagcmd} to apply flags from text files, xml files, the flag\_cmd table (see {\tt importevla}), and python strings  
     \item Selection by scan intents is now available in {\tt flagdata}
   \end{itemize}

\item {\bf Imaging}
    \begin{itemize}
     \item More Stokes selection options in {\tt clean}
     \item Improved handling of flat noise in mosaics
     \item Double-precision frequency grids are now used in {\tt clean} (important for interpolation of narrow channel-width data)
     \item MS-MFS performance improvements ({\tt nterms>1} in the {\tt clean} task) - the
minor-cycle is multi-threaded on scales, and deconvolution is no
longer restricted to the inner-quarter of the image.
...
\item {\bf Data examination/editing}
  \begin{itemize}
    \item {\tt plotms} now has the option to iterate through the data, e.g. through baselines or spectral windows interactively
    \item {\tt plotms} hardcopy outputs now have a white background instead of the former grey color
    \item {\tt plotms:} colorize is now available from the plotms task interface. There are also many smaller updates like allowing grid lines and changes to the title. 
   \end{itemize}

\item {\bf  Calibration}
\begin{itemize}

    \item CASA now supports the calibration of linear polarization feeds
    \item {\tt uvcontsub2}  now offers polynomial continuum fits
    \item Support for ALMA Tsys and EVLA switched power calibration
    \item Support for scan intents in {\tt listobs} and a number of calibration tasks (and tools)
    \item Improved ALMA primary beam tables are now available
    \item Tools to handle solar system object distances and sizes for calibration (including comets)  
    \item {\tt uvcontsub2} output no longer creates scratch columns
    \item A flux standard model for Venus is now implemented
    \item Unification of spectral shift and regridding in {\tt cvel} and {\tt  Clean}
    \item {\tt Applycal} now automatically backs up the flags before
      the calibration tables are applied to the data
    \item The default flux standard in {\tt setjy} is now 'Perley-Butler 2010' with improved values

   \end{itemize}

\item {\bf Simulation}
  \begin{itemize}
  \item {\tt simdata} includes the ALMA early science Cycle 0 configurations and has updates on ALMA band 7 and 9 Tsys values
  \end{itemize} 


\item {\bf  Parallelization }
  \begin{itemize}
  \item Parallel flagging supported 

\end{itemize} 

\item {\bf  Data and Image Manipulation }
   \begin{itemize}
   \item new interpolation method {\it fftshift} in {\tt cvel}
    \item {\tt split} now copies arbitrary subtables 
    \item in {\tt split}: spw string now always means to <i>skip</i> n channels, never an
averaging width as it did previously (at least in the task)
   \end{itemize}

\item {\bf  Data im/export }
     \begin{itemize} 

    \item CASA does now comply with the FITS v3.0 standards for spectral WCS (wcslib 4.7 compatibility)
    \item CASA now supports the import and export of FITS images with spectral axes linear in optical velocity (ctype VOPT) and wavelength (ctype WAVE)
     \item In addition, spectral axes linear in air wavelength (ctype AWAV) can be imported
    \item Degenerate WCS axes in FITS images are now better handled
     \item {\tt importevla} fills now the flag\_cmd table, allows scan selection and has separate controls on online, zero, and shadow flags (note, this is implied by changing {\tt importevla2} to {\tt importevla}) 
      \end{itemize}

\item {\bf  Data Analysis }
    \begin{itemize} 
    \item {\tt imfit} has better error estimates
    \item Component lists can now be written to a table in {\tt imfit}
    \end{itemize}

\item {\bf  Viewer }
     \begin{itemize}
     \item The viewer can now display and convert to/from wavelengths and air wavelengths on the spectral axis
     \item The Spectral Profile Tool (SPT) now persists as you open more files in the {\tt Viewer}, and are correctly automatically added to list shown in SPT
     \item Zoom in the SPT window now persists when cursor is moved in {\tt Viewer} window
     \item If you open a single plane in SPT first, before cube(s), the spectrum is displayed (before if cube wasn't first in the stack, no spectrum could be displayed)
     \item FITS images created by other packages such as IDL without degenerate axes (e.g. Stokes) can be displayed in {\tt Viewer} and imported via importfits. 
     \item All extensions of a mulit-extension FITS image can now be displayed in the {\tt viewer}. 
     \item The cursor tracking, animation, and mouse button panels can be docked on any side of the viewer. The cursor tracking and animation panels can be tabbed or untabed. The dock state of these panels is retained from one execution of the viewer to the next. This feature can be used to avoid any shrinking of the display panel as more data is loaded.
...
\item {\bf  Single Dish }
  \begin{itemize}
    \item Cubic spline and sinusoidal fitting are now avaiable in sdbaseline 
    \item Polynomial fitting in {\tt sdbaseline} is now 30 times faster than in CASA 3.1
    \item {\tt sdplot} can now plot more than 16 panels at a time
    \item Tasks show a context sensitive help to the arguments rather than global value descriptions 
    \item Interactive flagging is now available in {\tt sdflag}
    \item Renaming of some parameters in {\tt sdbaseline} for clarification, and adding new parameters for improved clipping 
    \item Beams can now be selected for plotting in {\tt sdplot}
    \item Explicit specification of row panelling and stacking in {\tt sdplot} 
    \item {\tt sdbaseline} supports per IF baseline selection syntax
...
\item {\bf  Other }
  \begin{itemize} 
  \item ATM libraries are updated to ATM-0.5.0. to be in sync with ALMA 8.1 online software
  \item If the user has no .matplotlib/matplotlibrc file, CASA sets up the backend to TkAgg as the default
  \item Better data tiling algorithms for performance improvements 
  \item {\tt casapy --help} now displays the available startup options of CASA
  \item And, as usual, many bug fixes and updated documentation 
\end{itemize}
...
exportuvfits        imstat              flagautocorr        conjugatevis      
importaipscaltable  imval               flagcmd             cvel              
importasdm          listcal             flagdata            hanningsmooth     
importfits          listhistory         flagmanager         imhead            
importfitsidi       listobs             msview              msmoments         
importuvfits        listvis             plotms              plotms            
importvla           plotms              plotxy              plotxy            
(exportasdm)        plotxy              (flagdata2)         split             
(importevla)        vishead                                 testconcat        
(importgmrt)        visstat                                 vishead           
{importoldasdm}     (listsdm)                               (uvcontsub2)      
...
plotms                                  sdfit               find              
plotxy                                  sdflag              help par.parameter
viewer                                  sdflagmanager       help taskname     
                                        sdimaging           imview            
                                        sdimprocess         msview            
                                        sdlist              plotms            
                                        sdmath              rmtables          
...
\begin{itemize}
   ...
   \item {\tt uvcontsub/uvcontsub2} --- carry out uv-plane continuum subtraction
...
in each polarization in specified time solution invervals.  The {\tt 'T'}
...
desired order from each baseline, defined by line-free channels. {\tt
  uvcontsub2} performs this process across spectral windows in a broad band dataset.
