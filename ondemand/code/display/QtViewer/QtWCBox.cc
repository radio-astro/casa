//# QtWCBox.cc: Class to define a world coordinate box region in an image
...
//#
//# $Id: QtWCBox.cc 20567 2009-04-09 23:12:39Z gervandiepen $
...
//
		...
		if (itsBlc.nelements() != itsTrc.nelements()) {
			msg = String("QtWCBox - you gave more values for the blc than the trc");
			throw (AipsError (msg));
		}
		if (itsAbsRel.nelements() != 0 &&
		        itsAbsRel.nelements() != itsBlc.nelements()) {
			msg = String("QtWCBox - you must specify as many values for absRel as blc/trc");
			throw (AipsError (msg));
		}
...
//
		String msg;
		if (itsBlc.nelements() != itsTrc.nelements()) {
			msg = String("QtWCBox - you must specify as many blc as trc values");
			throw (AipsError (msg));
		}
		if (itsBlc.nelements() != itsPixelAxes.nelements()) {
			msg = String("QtWCBox - you must specify as many blc/trc values as pixel axes");
			throw (AipsError (msg));
		}
		if (itsAbsRel.nelements() != 0 &&
		        itsAbsRel.nelements() != itsBlc.nelements()) {
...
// in the order corresponding to the pixel axes.
		...
		if (!itsCSys.toWorld(wBlc, pixel)) {
			throw (AipsError ("QtWCBox - Cannot convert blc of LCBox because "+itsCSys.errorMessage()));
		}
		for (i=0; i<end.nelements(); i++) pixel(i) = end(i);
		if (!itsCSys.toWorld(wTrc, pixel)) {
			throw (AipsError ("QtWCBox - Cannot convert trc of LCBox because "+itsCSys.errorMessage()));
		}
...
// Create quanta vectors, and pixel axis vectors
		...
		for (i=0; i<itsCSys.nPixelAxes(); i++) {
			...
			if (worldAxis != -1) {
				...
				throw (AipsError ("QtWCBox - missing world axis in Coordinate System"));
			}
//
			itsPixelAxes(i) = i;
...
//
	...
	QtWCBox::QtWCBox (const QtWCBox& other)
//
...
//
	...
	QtWCBox& QtWCBox::operator= (const QtWCBox& other)
//
...
// Caste
		...
		const QtWCBox& that = (const QtWCBox&)other;
...
//
	...
	WCRegion* QtWCBox::cloneRegion() const {
		return new QtWCBox(*this);
	}

	QtWCBox QtWCBox::splitBox (const IPosition& axes) const {
...
// Define pixel units
		...
		if (error=="") {
			...
			pBox = new QtWCBox(blc,trc,pixelaxes,csys,absRel);
			return pBox;
		}
	...
	String QtWCBox::toBoxString() const {
// Define pixel units
	...
	TableRecord QtWCBox::toRecord(const String&) const
//
...
//
		for (uInt j=0; j<nAxes; j++) {
			...
			if (!h.toRecord(error, recT)) {
				throw (AipsError ("QtWCBox::toRecord - could not save blc because "+error));
			}
...
//
		if (!itsCSys.save(rec, "coordinates")) {
			throw (AipsError ("QtWCBox::toRecord - could not save Coordinate System"));
		}
		return rec;
	}
	...
	QtWCBox* QtWCBox::fromRecord (const TableRecord& rec,
	                              const String&) {
...
//
		...
		if (blcRec.nfields() != trcRec.nfields()) {
			throw (AipsError ("QtWCBox::fromRecord - blc and trc must be the same length"));
		}
//
		...
		if (nAbsRel == 0) {
			...
			if (nAbsRel != nFields) {
				throw (AipsError ("QtWCBox::fromRecord - absrel must be same length as blc/trc"));
			}
		}
//
		if (nFields > 0) {
...
//
			for (j=0; j<nFields; j++) {
				...
				if (!h.fromRecord(error, subRec1)) {
					throw (AipsError ("QtWCBox::fromRecord - could not recover blc because "+error));
				}
...
//
				...
				if (!h.fromRecord(error, subRec2)) {
					throw (AipsError ("QtWCBox::fromRecord - could not recover trc because "+error));
				}
...
//
	...
	Bool QtWCBox::getChanExt (Double& chanStart, Double& chanEnd) {
	...
	Bool QtWCBox::getPolExt (Double& polStart, Double& polEnd) {
	...
	LCRegion* QtWCBox::doToLCRegion (const CoordinateSystem& cSys,
...
// Make sure that we are not using a null QtWCBox

		if (itsNull) {
			throw (AipsError ("QtWCBox:toLCregion - this is a null QtWCBox object"));
		}
...
// Convert to pixels for all pixel axes of cSysTmp for blc and trc
		...
		if (!cSysTmp.toPixel(pBlc, wBlc)) {
			throw (AipsError ("QtWCBox:doToLCregion - conversion of blc to pixel coordinates failed"));
		}
//
		...
		if (!cSysTmp.toPixel(pTrc, wTrc)) {
			throw (AipsError ("QtWCBox:doToLCregion - conversion of trc to pixel coordinates failed"));
		}
...
// Create the LCBox.  It will throw an exception if blc > trc
	...
	String QtWCBox::className() {
		return "QtWCBox";
	}

	String QtWCBox::type() const {
		return className();
	}
...
//
	{
		if (pixelAxes.nelements() != values.nelements()) {
			throw (AipsError ("QtWCBox::checkUnits - internal error"));
		}
//
...
// Check units
			...
			if (worldAxis != -1) {
				...
				if (tmp.getUnit() != "pix" && tmp.getUnit() != "default" &&
					...
					if (tmp.getFullUnit() != Unit(units(worldAxis))) {
						String msg = String("QtWCBox::checkUnits - units of box blc (") + tmp.getUnit() +
				...
				throw (AipsError ("QtWCBox::checkUnits - missing world axis in Coordinate System"));
			}
		}
	}
	...
	void QtWCBox::convertPixel(Double& pixel,
