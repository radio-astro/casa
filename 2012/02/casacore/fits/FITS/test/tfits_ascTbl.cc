//# tfits_ascTbl.cc: FITS test program to create a ascii table by using
//# AsciiTableExtension::write_ascTbl_hdr().
...
//#
//# $Id: tfits_ascTbl.cc 20329 2008-06-06 07:59:22Z gervandiepen $
...
int main()
{
	...
	FitsOutput fout("tfits_ascTbl.dat",FITS::Disk);
   ...
   long tbcol = 0;    // byte offset in row to each column( default value will be calculated when passing in 0) 
	...
	if(!at.write_ascTbl_hdr(fout, naxis1,naxis2,tfields, ttype, &tbcol, tform, tunit, extname )){   
	   ...
	   cout << "Ascii Table HDU wrote header failed!" << endl;
	   exit( 0 );
	}
	cout << at;
