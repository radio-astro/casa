...
The new Calibration Table format can be handled by the TestFlagger tool in a
...
CASA <2>: tf.open('X7ef.tsys')
INFO	TestFlagger::open	Table type is Calibration
  Out[2]: True
...
\item Autoflagging (outliers detection) agents: clip, tfcrop, rflag:
...
tf.parseagentparameters(agentClip)
tf.init()
tf.run()
tf.done()
...
\item Manualflagging (meta-data selections like antenna/time-range):
...
tf.parseagentparameters(agentManual)
tf.init()
tf.run()
tf.done() 
...
tf.parseagentparameters(agentSummary)
tf.init()
summary = tf.run()
tf.done()
...
trough the selectdata() method of the TestFlagger tool:

\begin{verbatim}

tf.open('X7ef.tsys')
tf.selectdata(field='TW Hya')
...
tf.parseagentparameters(agentUnflag)
tf.parseagentparameters(agentClip)
tf.parseagentparameters(agentDisplay)
tf.init()
tf.run(writeflags=True)
tf.done()
...
tf.parseagentparameters(agentSummary)
tf.init()
summary = tf.run(writeflags=True)
tf.done() 
