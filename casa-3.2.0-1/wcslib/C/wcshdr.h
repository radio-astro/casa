/*============================================================================
    ...
    $Id: wcshdr.h,v 4.3 2007/12/27 05:41:36 cal103 Exp $
*=============================================================================
*
...
*   a FITS binary table (BINTABLE) extension that are located by PVi_ma
...
*
*
*   FITS WCS parser routine for image headers; wcspih()
*   ---------------------------------------------------
...
*   drafts of the WCS papers (see note 5 below).
*
*   Given a character array containing a FITS image header, wcspih()
*   identifies and reads all WCS keywords for the primary coordinate
*   representation and up to 26 alternate representations.  It returns this
*   information as an array of wcsprm structs.
*
...
*                        header from which to idenfify and construct the
...
*                        is also possible, see note 5 below.
...
*                        keywords are simply ignored (but see also note 5
*                        below).
...
*                        members to default values (see also note 8 below).
*                        Note that wcsset() is not invoked on these structs.
*
...
*
*
*   FITS WCS parser routine for binary table headers; wcsbth()
*   ----------------------------------------------------------
*   wcsbth() is a high-level FITS WCS routine that parses a binary table
*   header.  It handles image array and pixel list WCS keywords which may be
*   present together in one header.
*
...
*   default values via an inheritance mechanism discussed below in note 5 (see
...
*   handle primary image and image extension headers in addition to binary
*   table headers (it ignores NAXIS and does not rely on the presence of the
*   TFIELDS keyword).
*
*   All WCS keywords defined in Papers I, II, and III are recognized, and also
*   those used by the AIPS convention and certain other keywords that existed
*   in early drafts of the WCS papers (see note 5 below).
*
...
*   created.  See also note 9 concerning the number of separate images that
*   may be stored in a pixel list.
*
...
*                        which to idenfify and construct the coordinate
...
*                        complement of nkeyrec keyrecords and possibly not be
*                        null-terminated.
*      keysel   int      Given, a vector of flag bits that may be used to
*                        restrict the keyword types considered:
...
*                        WCSNna and TWCSna, see note 4 below), are selected by
*                        both WCSHDR_BIMGARR and WCSHDR_PIXLIST.  Thus if
*                        inheritance via WCSHDR_ALLIMG is enabled (see note 5
*                        below) and one of these shared keywords is present,
*                        then WCSHDR_IMGHEAD and WCSHDR_PIXLIST alone may be
*                        sufficient to cause the construction of coordinate
*                        descriptions for binary table image arrays.
*      colsel   int*     Pointer to an array of table column numbers used to
*                        restrict the keywords considered by wcsbth().
*
*                        A null pointer may be specified to indicate that
*                        there is no restriction.  Otherwise, the magnitude of
...
*                        TPCn_ka and TCDn_ka if WCSHDR_LONGKEY is enabled), 
*                        it is an error for one column to be selected but not
*                        the other.  This is unlike the situation with invalid
...
*      wcs      struct wcsprm**
*                        Pointer to an array of wcsprm structs containing up
*                        to 27027 coordinate representations (see note 6
*                        below).
*
...
*   is stored in PVi_ma and PSi_ma keywords in the image header.
*
...
*
*   Note that wcspih() and wcsbth() automatically invoke wcstab() on each of
*   the wcsprm structs that they return.
*
*   Given and returned:
*      wcs      struct wcsprm*
...
*
*
*   Index alternate coordinate representations; wcsidx()
*   ----------------------------------------------------
...
*                        alts['P'-'A'+1] would be set to -1.  Otherwise,
*                        the address of its wcsprm struct would be
*                        wcs + alts['P'-'A'+1].
*
*   Function return value:
*               int      Status return value:
...
*
*
*   Index alternate coordinate representions; wcsbdx()
*   --------------------------------------------------
*   wcsbdx() returns an array of 999 x 27 indices for the alternate coordinate
*   representions for binary table image arrays xor pixel lists in the array
*   of wcsprm structs returned by wcsbth().  Use wcsidx() for the unattached
...
*   Returned:
*      alts     short[1000][28]
*                        Index of each alternate coordinate represention in
*                        the array: alts[col][0] for the primary, alts[col][1]
*                        for 'A', to alts[col][26] for 'Z', where col is the
...
*                        column 13 then alts[13]['P'-'A'+1] would be set
*                        to -1.  Otherwise, the address of its wcsprm struct
*                        would be wcs + alts[13]['P'-'A'+1].
*
*   Function return value:
*               int      Status return value:
*                           0: Success.
...
*   --------------------------------------------
*   wcsvfree() frees the memory allocated by wcspih() or wcsbth() for the
*   array of wcsprm structs, first invoking wcsfree() on each of the array
*   members.
*
*   Given and returned:
...
*
*
*   Write out a wcsprm struct as a FITS header; wcshdo()
*   ----------------------------------------------------
*   wcshdo() translates a wcsprm struct into a FITS header.  If the colnum
*   member of the struct is non-zero then a binary table image array header
*   will be produced.  Otherwise, if the colax[] member of the struct is set
*   non-zero then a pixel list header will be produced.  Otherwise, a primary
*   image or image extension header will be produced.
*
...
*
*     1) The output header only contains WCS-related keywords.  In particular,
*        it does not contain syntactically-required keywords such as SIMPLE,
*        NAXIS, BITPIX, or END.
*
*     2) Deprecated (e.g. CROTAn) or non-standard usage will be translated to
*        standard (this is partially dependent on whether wcsfix() was
*        applied).
*
*     3) Quantities will be converted to the units used internally, basically
*        SI with the addition of degrees.
*
*     4) Floating-point quantities may be given to a different decimal
*        precision.
*
*     5) Elements of the PCi_j matrix will be written if and only if they
*        differ from the unit matrix.  Thus, if the matrix is unity then no
*        elements will be written.
*
*     6) Additional keywords such as WCSAXES, CUNITia, LONPOLEa and LATPOLEa
*        may appear.
*
*     7) The original keycomments will be lost, although wcshdo() tries hard
*        to write meaningful comments.
*
*     8) Keyword order may be changed.
*
*   Keywords can be translated between the image array, binary table, and
*   pixel lists forms by manipulating the colnum or colax[] members of the
*   wcsprm struct.
*
*   Given:
*      relax    int      Degree of permissiveness:
...
*                              standard.
*                        Fine-grained control of the degree of permissiveness
*                        is also possible, see note 10 below.
*
*   Given and returned:
*      wcs      struct wcsprm*
...
*                  WCSNAMEa   WCSNna  ... Or TWCSna (see below).
*                  ZSOURCEa   ZSOUna
*
*                  DATE-AVG   DAVGn
*                  DATE-OBS   DOBSn
...
*                  OBSGEO-Z   OBSGZn
*
*               where the image-header keywords in the left-hand column
*               provide default values for the column specific keywords on
*               the right.
*
...
*       wcsbth() does not check the the TFORM keyword for a pixel list column
*       to check that it is scalar.
*
...
*
*   10) wcshdo() interprets the "relax" argument as a vector of flag bits to
...
*               is very unlikely that any parser would reject keywords in the
...
*               time.
*
*         WCSHDO_WCSNna: In light of note 4 above, write WCSNna instead of
...
#define WCSHDR_VSOURCEa 0x00000080
...
#define WCSHDR_BINTAB   0x00080000
...
int wcsbth(char *, int, int, int, int, int *, int *, int*, struct wcsprm **);
int wcstab(struct wcsprm *);
int wcsidx(int, struct wcsprm **, int [27]);
int wcsbdx(int, struct wcsprm **, int, short [1000][28]);
int wcsvfree(int *, struct wcsprm **);
int wcshdo(int, struct wcsprm *, int *, char **);
