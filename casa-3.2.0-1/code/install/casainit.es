#!/bin/echo Usage: .
#-----------------------------------------------------------------------------
# casainit.es: Define the CASA environment for the "es" shell
#-----------------------------------------------------------------------------
#
...
# Usage: . casainit.es
#-----------------------------------------------------------------------------
# casainit.es bootstraps the CASA user environment for 'es', the
...
#
#    CASAPATH  A sequence of five character strings separated by spaces
...
# stripped off.  It also defines a shell function called 'casainit' which
# takes a value for "aips_ext" as an argument and reinvokes casainit.  This
# provides a convenient mechanism for interactively switching between
# architecture extensions.  Note that casainit always undefines "aips_ext".
#
...
#    Reset the architecture extension if required.
     if {test $^aips_ext} {
        ...
        if {~ $a_ext $a_arch} {
           ...
           if {test $aips_ext '=' '64b' -a `{uname -m} '!=' 'x86_64'} { a_arch='*' }
...
#    Is CASAPATH already defined?
     if {test $^CASAPATH} {
        a_och=`{echo $CASAPATH | awk '{print $2}'}
        a_old=`{echo $CASAPATH | awk '{printf("%s/%s",$1,$2)}'}
     } {
        a_och=$a_arch
        a_old=$a_root/$a_arch
     }
...
#    Function which invokes casainit with "aips_ext" as a command line argument.
     let (a_root = $a_root) {
        fn casainit {
           aips_ext=$* ;
           local (a_root = $a_root) {. $a_root/casainit.es}
        }
     }
  }
