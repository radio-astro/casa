...
/*   QtViewerBase* viewer() { return v_;  } */
  ...
  virtual QPixmap* getBackBuffer() { return pc_->getBackBuffer();  }
  ...
  void setRegionExtent(Int ext) { rgnExtent_ = max(0, min(2,  ext  ));  }
  
  // Return the last region created (via an image DD) with the mouse.
  ImageRegion lastRegion() { return lastRgn_;  }
  
  Bool hasRegion() { return hasRgn_;  } 
 ...
 protected slots:
  ...
  virtual void reorientColorBars_() { arrangeColorBars_(True);   }
  virtual void checkColorBars_()    { arrangeColorBars_(False);  }
  virtual void pcResizing_()        { arrangeColorBars_(False, True);  }
 ...
 public:
  ...
  void beginLabelAndAxisCaching( ) { pc_->beginLabelAndAxisCaching( );  }
