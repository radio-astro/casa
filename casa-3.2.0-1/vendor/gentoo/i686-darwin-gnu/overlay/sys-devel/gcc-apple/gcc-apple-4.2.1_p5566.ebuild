...
# $Id: gcc-apple-4.2.1_p5566.ebuild 36240 2008-12-31 22:58:06Z grobian $
...
SRC_URI="		http://www.opensource.apple.com/darwinsource/tarballs/other/gcc_42-${APPLE_VERS}.tar.gz
	...
	fortran? ( http://ftpmirror.gnu.org/gcc/gcc-${GFORTRAN_VERSION}/gcc-fortran-${GFORTRAN_VERSION}.tar.bz2 )"
...
IUSE="nls objc objc++ nocxx fortran"

RDEPEND=">=sys-libs/zlib-1.1.4
	>=sys-libs/ncurses-5.2-r2
	nls? ( sys-devel/gettext )
	>=sys-devel/gcc-config-1.3.12-r4
	fortran? ( dev-libs/gmp dev-libs/mpfr )"
DEPEND="${RDEPEND}
...
src_unpack() {
	...
	cd "${S}"
	use fortran && {
		mv ../gcc-${GFORTRAN_VERSION}/gcc/fortran ./gcc/fortran
		mv ../gcc-${GFORTRAN_VERSION}/libgfortran .
		rm -rf ../gcc-${GFORTRAN_VERSION}
	}
	...
	use fortran && \
		epatch "${FILESDIR}"/gfortran-${APPLE_VERS}.diff
