...
std::vector<std::string> QtDisplayPanel::worldAxisUnits( ) const {
    std::vector<std::string> result;
    QtDisplayData *dd = 0;
    // find controlling coordinate system...
    for ( ConstListIter<QtDisplayData*> qdds(qdds_);  ! qdds.atEnd(); qdds++ ) {
	QtDisplayData* qdd = qdds.getRight();
	if ( pd_->isCSmaster(qdd->dd( )) ) {
	    dd = qdd;
	    break;
	}
    }
    if ( dd == 0 ) {
	fprintf( stderr, "*** did not find controlling display data ***\n" );
	return result;
    }
    ...
    if ( image == 0 ) {
	fprintf( stderr, "*** did not retrieve image interface ***\n" );
	return result;
    }

    const CoordinateSystem &cs = image->coordinates( );
    cout << "<<<<<<<" << cs.nPixelAxes() << ">>>>>>>" << endl;
    return result;
    ...
    wcs.toStart( );
    if ( wcs.len( ) <= 0 ) return result;
    const WorldCanvas *wc = wcs.getRight( );
    Vector<String> units = wc->worldAxisUnits( );
    for ( int i=0; i < units.size( ); ++i )
	result.push_back(units[i].c_str( ));
    return result;
}

std::vector<std::string> QtDisplayPanel::worldAxisNames( ) const {
    std::vector<std::string> result;
    ConstListIter<WorldCanvas*>& wcs = *(pd_->myWCLI);
    wcs.toStart( );
    if ( wcs.len( ) <= 0 ) return result;
    const WorldCanvas *wc = wcs.getRight( );
    Vector<String> units = wc->worldAxisNames( );
    for ( int i=0; i < units.size( ); ++i )
	result.push_back(units[i].c_str( ));
    return result;
}
