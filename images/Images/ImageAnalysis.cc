...
//
{
	...
	if (model == Fit2D::GAUSSIAN || model == Fit2D::DISK) {
		...
		if (parameters.nelements() == 0) {
			...
			os << "Not enough parameters returned by fitter estimate"
					<< LogIO::EXCEPTION;
		}
	} else {
		// points, levels etc
		os << "Only Gaussian/Disk auto-single estimates are available"
				<< LogIO::EXCEPTION;
	}
	return parameters;
}
...
void ImageAnalysis::encodeSkyComponentError(LogIO& os, SkyComponent& sky,
