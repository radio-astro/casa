...
#    casalog.post('You are using flagcmd v3.6 Updated STM 2011-06-28')
    ...
    try:
        ...
        # Select the data
        #fglocal.setDataSelection(unionpars)
        ...
        if optype == 'apply':
            ...
            nap = applyFlags(fglocal,cmdlist,flagbackup,reset)
    ...
    try:
        ...
        mslocal.writehistory(message='taskname = flagcmd2',
                             origin='flagcmd2')
...
def backup_cmdflags(fglocal, mode):
    ...
    existing = fglocal.getflagversionlist(printflags=True)
    ...
    fglocal.saveflagversion(versionname=versionname,
                            comment='flagcmd2 autosave before ' + mode
...
def applyFlags(fglocal,cmdlist,flagbackup,reset):
    ''' for optype = apply '''
    ...
    for i in range(nrows):
        ...
        # Backup the flags
        if (flagbackup):
            backup_cmdflags(fglocal, 'flagcmd2_' + mode)
