...
C
      IF(YEAR .LE. 0) THEN
         WRITE(*,'(1X,A,I4,A)') 'TIMCDM:Year (',YEAR,
     &                          ') must be > 0 ...'
         CALL A2STOP('Program terminated with error in TIMCDM.')
      ENDIF
      IF((MONTH .LT. 1) .OR. (MONTH .GT. 12)) THEN
         WRITE(*,'(1X,A,I2,A)') 'TIMCDM:Month (',MONTH,
     &                          ') must be >= 1, <= 12 ...'
         CALL A2STOP('Program terminated with error in TIMCDM.')
      ENDIF
C
...
C
      IF(YEAR .LE. 0) THEN
         WRITE(*,'(1X,A,I4,A)') 'TIMLPY:Year (',YEAR,
     &                          ') must be > 0 ...'
         CALL A2STOP('Program terminated with error in TIMLPY.')
      ENDIF
C
...
C
      IF(YEAR .LE. 0) THEN
         WRITE(*,'(1X,A,I4,A)') 'TIMMDM:Year (',YEAR,
     &                          ') must be > 0 ...'
         CALL A2STOP('Program terminated with error in TIMMDM.')
      ENDIF
      IF((DOY .LT. 1) .OR. (DOY .GT. TIMCDM(YEAR,12))) THEN
         WRITE(*,'(1X,A,I3,A,I3,A)') 'TIMMDM:Day of year (',DOY,
     &                               ') must be >= 1, <= ',
     &                               TIMCDM(YEAR,12),' ...'
         CALL A2STOP('Program terminated with error in TIMMDM.')
      ENDIF
