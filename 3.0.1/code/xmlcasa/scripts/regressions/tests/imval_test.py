...
#
# cp CASA_REPOS/data/regression/ATST3/NGC4826/* WORKING_DIR
# cp CASA_REPOS/code/xmmlcasa/tasts/tests/imval_test.py WORKING_DIR
# casapy
...
def input_test( image_file, good_rgn_file ):
...
def single_point_test( image_file ):
    retValue = {'success': True, 'msgs': "", 'error_msgs': '' }
...
def description():
    return " Tests the correctness of the imval task in CASA including:"\
           + "\n\n"\
           + "1. Incorrect input is given to each paramter.  Incorrect input"\
           + "\n\tincludes incorrect data type, out-of-bounds (where"\
           + "\n\tapplicable, and correct data type, no input, and"\
           + "\n\tnon-sensical.\n"\
           + "2. Finding the value for a single point in the image, each"\
           + "\n\tof the corners are found as well as a couple of random"\
           + "\n\tpoints.\n"\
           + "3. Find an array of values: An array or two from on the"\
           + "\n\tsky only, one or two data cubes, and a 4D blob.\n\n"\
           + "Data used for this test includes: "\
           + "     1. n4826_bima.im"


def data():
    # The data is in <casa data regression dir>/ATST3/NGC4826
    return ['n4826_bima.im', 'n4826_bima_test.rgn' ]
...
def run():
    ...
    # we don't really need too.
    test_image_filename = 'n4826_bima.im'
    test_rgn_filename   =  'n4826_bima_test.rgn'
    test_list = [
        'input_test( '+test_image_filename+', '+ test_rgn_filename+' )',\
                  '_test()', \
                  '_test()' ]
    ...
    error_msgs = ""
    testResults=[]
    testResults.append( input_test( test_image_filename, test_rgn_filename ) )
    testResults.append( single_point_test( test_image_filename ) )
    testResults.append(array_test( test_image_filename ) )
