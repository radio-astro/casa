...
\begin{enumerate}
\item \htmladdnormallink{TestFlagger}{XXX} : The top-level Flagger class that connects all of the following together, 
and defines the C++ user-interface for the flagger.  This is the class used by the tool layer.
...
The following is a typical way of using the tflagger tool to perform flagging.

Open the MS or a calibration file and attach it to the tool. The function
...
FlagDataHandler::SUB_INTEGRATION iteration approach and 0.0 seconds as the time interval.
...
will use the MS Selection class. There are two functions to perform the
...
\item    The following are only possible in the tool.

Run multiple summary agents in a list. In order to do this, add as many summary
agents are desired when parsing the agent's parameters. You can do this by
calling the tf.parseagentparameters() several times before calling tf.init().

Determine to run the agents in sequential or in parallel. Set the 'sequential'
parameter of the run method to True or False to control this.

Flag CAL tables in currently only possible in the tool. Give a CAL table name
when calling the tf.open() method instead of a MS.
