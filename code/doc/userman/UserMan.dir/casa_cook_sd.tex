...
%Updated TN 2010-10-12 start editing for R3.1, added inputs from Kana and myself
...
{\bf For R3.1, the ASAP version included in CASA was updated to 3.0 
...
Note that this is specific issue for scantable, which is proper data format 
for ASAP. 
...
\end{verbatim}
    ...
    DESCRIPTION:
        ...
        *** Only apply to 'spectra' plottype ***\\
        ...
        This task adds an additional toolbar to Matplotlib plotter which
        has five buttons, 'spec value', 'statistics', 'notaion', '+ page'
        and 'Quit'.
        ...
        plotted. The results are printed on the casa logger.
        When 'notation' button is pressed and activated, you can
        graphically add a text on a position of plot. Click the
        LEFT-mouse on a position of plot to load notaion window, and
        input a text to the textbox, select an anchor, and press 'print'
        button to print it on the plot. Click RIGHT-mouse on a text and
        cascade menu will let you select either delete or modify it.
        When you select a panel parameter with more than 16 panels,
        '+ page' button is activated and let you go through pages.
...
{\bf R3.1 New Features:}\\
New parameters added to control the appearance of plots. 
A boolean parameter, {\tt header}, controls whether or not header is
printed on a plot and the subparameter, {\tt headsize}, specifies
font size. When a boolean parameter, {\tt plotsyle}, is True, users can
specify margins between subplots and legend location by the
subparameters, {\tt layout} and {\tt legendloc}. 

Default values changed for subparameters {\tt tweight = 'tintsys'} and
{\tt pweight = 'tsys'}.

Two new buttons, 'notaion' and '+ page', added to the toolbar on
plot.
...
to a MS first via {\tt importasdm} task. Then you can load this MS to ASAP. The
 ...
 antenna by the {\tt antenna} parameter (see 2.2.1). %% <= add correct TeX reference 
...
\item {\tt sd.plotter}
  ...
  There are known bugs when plotting with the tool-based plotting. 
  The spectral values won't be displayed on the plotter
  displayed with {\tt sd.plotter.plot()} even if 'spec value' button is
  pressed. 
