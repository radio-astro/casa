...
scales ('Perley-Butler 2010' is now standard by default),
...
can be obtained for the 'EVLA/VLA Baseline Corrections' web page:
...
or a combination. The format for the scan intents of your
observations are listed in the logger when you run {\tt
  listobs}. Minimum matching with wildcards will work, like
'*BANDPASS*'. This is especially useful when multiple intents are attached to scans. 
...
(after solving) the phase of the gain solutions for all antennas such
that the reference antenna's phase is constant at zero.  If the
selected antenna drops out, another antenna will be selected for
ongoing consistency in time (at its ``current'' value) until the
refant returns, usually at a new value (not zero), which will be kept
fixed thenceforth.  You can also run without a reference antenna, but
in this case the solutions will formally float with time; in practice,
the first antenna will be approximately constant near zero phase.
It is usually prudent to select an antenna
in the center of the array that is known to be particularly stable, as
any gain jumps or wanders in the {\tt refant} will be transferred to
the other antenna solutions.  Also, it is best to choose a reference
antenna that never drops out.
...
minblperant         =          2        #  Minimum baselines _per antenna_ required for solve
minsnr              =        0.0        #  Reject solutions below this SNR (only applies for bandtype = B)
...
gaintype            =        'G'        #  Type of gain solution (G, T, or GSPLINE,KCROSS)
...
for the general user in the 3.3 release.  The following examples
assume the circular basis.

the general user; it will 
...
achieved implicitly simply by running {\tt applycal} serially (to the
extent required to achieve different interpolation or other effects
for different fields and spectral windows) with all calibration tables
specified.  However, sometimes it is desirable to see the interpolated
calibration prior to application, and this section describes how this
can be done.
...
The inpust for {\tt clearcal} are simple:
\small
\begin{verbatim}
...
{\bf Alert:} For most applications you should now use {\tt
  uvcontsub2}. It has much more functionality than {\tt uvcontsub} and
better output options. In the future we will replace {\tt uvcontsub}
by {\tt uvcontsub2}.
...
The $uv$-plane continuum subtraction is performed by the {\tt
  uvcontsub} and {\tt uvcontsub2} tasks.  First, determine which
channels in your data cube do not have line emission, perhaps by
forming a preliminary image as described in the next chapter.  This
image will also help you decide whether or not you need to come back
and do $uv$-plane continuum subtraction at all. Note that {\tt
  uvcontsub} only accepts line free channels in the same spw where the
line resides. To fit continuum across many spws {\tt uvcontsub2}
should be used.


The inputs to {\tt uvcontsub2} are:
\small
\begin{verbatim}
#  uvcontsub2 :: Continuum fitting and subtraction in the uv plane CALIBRATION VERSION
vis                 =         ''        #  Name of input MS.  Output goes to vis
                                        #   + ".contsub"
field               =         ''        #  Select field(s) using id(s) or
                                        #   name(s)
fitspw              =         ''        #  Spectral window:channel selection for
                                        #   fitting the continuum
combine             =         ''        #  Data axes to combine for the
                                        #   continuum estimation (spw, scan,
                                        #   and/or field)
solint              =      'int'        #  Continuum fit timescale
fitorder            =          0        #  Polynomial order for the fits
spw                 =         ''        #  Spectral window selection for output
want_cont           =      False        #  Create vis + ".cont" to hold the
                                        #   continuum estimate.
...
quality of {\tt uvcontsub2} - although the continuum estimates of each baseline
...
Unlike {\tt uvcontsub}, there is no need to run split before running
{\tt uvcontsub2}.
...
\item Use {\tt uvcontsub2} with {\it fitorder = 0} or {\it 1} to estimate and subtract the
...
\item If a continuum image is desired, clean the line-free channels of the
  ...
  the 'continuum' model fitted by {\tt uvcontsub2}. The fitting
...
uvcontsub2(vis='ngc5921.usecase.msâ€™,
field='N5921',
