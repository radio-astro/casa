...
The CASA release 3.1.0 is the second release of CASA. The main feature
...
\subsection{What's New in Release 3.1.0}
...
\begin{itemize}
  ...
  \item Tasks {\tt gaincal} and {\tt polcal} now support specifying point
source Stokes parameters for calibration source model in a new {\tt 'smodel'}
parameter.  When specified, this overrides use of the {\tt MODEL\_DATA} column
and thereby saves time.

   \item The {\tt setjy} task now has a {\tt 'scalebychan'} parameter that
enables setting the model for flux density calibrators correctly for
every channel according to their spectral indices.

  \item The {\tt gaincal} task now supports a new gaintype, {\tt 'KCROSS'},
to solve for cross-hand delay.

   \item Scratch column creation/initialization has been
revised to improve performance in most cases.  Further improvements
in data tiling will be introduced in the near future to provide
