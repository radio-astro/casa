...
CASA 3.3.0 is now available and the main feature 
improvements are listed below. In addition to the new features, many bugs were 
addressed and fixed.
...
as well as CASA releases are available at \url{http://casa.nrao.edu}.
Releases will be announced via the 'casa-announce' mailing list. To
subscribe, please visit the \url{http://casa.nrao.edu}. For feedback,
and help please go to the \url{http://help.nrao.edu} NRAO
helpdesk; for ALMA questions please use the
\url{http://alma-help.nrao.edu} ALMA helpdesk.
...
interferometers can be imported from {\tt uvfits} or {\tt sdfits}
formats into measurements sets (ms) or scantables in CASA. Given the
variety of non-standard fits formats, we cannot guarantee that CASA
will fully support data from all telescopes. However, efforts are made
to support data formats from other facilities.
...
\subsection{What's New in Release 3.3.0}
...
\item  {\bf  Flagging }
    \begin{itemize}
       \item  {\tt testautoflag:} new, experimental task for autoflagging data based on piecewise fitting of the bandpass in the time and frequency domains
    \end{itemize}

\item  {\bf  Imaging }
    \begin{itemize}
      \item  {\tt clean} masks for main and outlier fields in {\tt clean} now 
use the same format please use the new CASA region format (see below), boxfile 
format support etc. will be removed in upcoming CASA releases
    \end{itemize}


\item  {\bf  Data examination/editing }
  \begin{itemize}
    \item  {\tt plotms:} also displays the antenna station name 
    \item  {\tt plotuv:} new task to plot the visibilities in the uv-plane
    \item  {\tt listvis:} now labels all entries with their units
    \item  {\tt ms.select()} is a new method to select visibilities on the tool level with a selection syntax similar to that in CASA tasks
    \item  {\tt listobs:} can now write the output to a text file
  \end{itemize}

\item  {\bf  Calibration }
  \begin{itemize}
    \item  {\tt setjy:} A spectral index is now supported as input for flux calibration
    \item  {\tt setjy:} Arbitrary fluxes, and spectral index can be applied to model images
    \item  {\tt setjy:} The parameter {\it listmodimages} will list all available calibrator models. Standard calibrator models don't need their full paths anymore
    \item  {\tt bandpass, gaincal, applycal:} The {\it opacity} parameter can now be a list of opacities, one entry for each spectral window
    \item  {\tt gaincal:} {\it gaintype='K'} is implemented to solve for antenna based delays
    \item  data selection based on {\it scan intents} and {\it observation id} is now supported in most calibration tasks
    \item  EVLA switched power calibration available in {\tt gencal}
      ({\it caltype='evlagain'})
    \item ALMA Tsys calibration available in {\tt gencal} ({\it caltype='tsys'}) 
    \item  previous {\tt uvcontsub2} has been renamed and is now replacing {\tt uvcontsub}
    \item  the reference antenna in calibration tasks can now be a list of antennas. If the first antenna is not available, the second entry will be used etc.
    \item  calibration tasks now use {\it minsnr=3} as default for the minimum signal-to-noise ration of data to be included in the solutions  
    \item Model for the EVLA calibrator 3C147 in L-band is now available   
  \end{itemize}

\item  {\bf  Simulation }
  \begin{itemize}
    \item  The {\tt simdata} task has been split into two tasks, 
{\tt sim\_observe} 
and
{\tt sim\_analyze}, to reduce the complexity for new users, and facilitate the
combination of multiple arrays. {\tt sim\_observe} is used to create
measurement sets from model images and component lists, and optionally
add thermal noise and cross-polarization.  {\tt sim\_analyze} is used to image
datasets and graphically assess their fidelity.  The original task
{\tt simdata} is retained in CASA 3.3 but will be removed in the future.
     \item  better handling of simulation from components and clarification of the 
units for reported statistics. 
    \item  More flexibility in simulating sources around transit
    \item  a {\it thermalnoise} {\it seed} parameter will now ensure randomness of noise 
    \item  the default velocity frame for simulated data is now LSRK rather than TOPO
  \end{itemize} 

\item  {\bf  Data and Image Manipulation }
   \begin{itemize}
    \item  {\tt fixvis:} new parameter {\it phasecenter} to change the direction of the phase center of the visibilities. Changes can be absolute or relative
    \item  {\tt concat} can now optionally omit copying (and concatenating) the POINTING table 
    \item  {\tt impbcor}, {\tt ia.pbcor()} new task and tool method for primary beam correction of images
    \item  {\tt immath:} a {\it stretch} parameter in the {\it mask} selection will extend a smaller mask to the full size of the data cube (e.g. a moment0 mask to all channels)
    \item  the default for channel interpolation during regridding processes is now 
{\it 'linear'} in most tasks  
   \end{itemize}
...
\item  {\bf  Data im/export }
     \begin{itemize}
        \item  full ICRS coordinate support
        \item  upgrades to {\tt importasdm} and {\tt exportasdm} to support ASDM v1.3
        \item  {\tt importasdm} can now optionally omit importing the POINTING table  
        \item  {\tt importevla:} EVLA switched power can now be filled from an SDM into an MS
     \end{itemize}
   

\item  {\bf  Data analysis }
     \begin{itemize}
      \item  {\tt imfit:} now optionally performs fits on a plane-by-plane basis if given multiple channels
      \item  {\tt imfit:}  now derives the integrals and associated errors of the fitted Gaussians 
      \item  {\tt imspec:} accepts initial values for Gaussian fitting
      \item  {\tt imstat:} now derives statistics in slices of data, e.g. across RA/DEC per channel map
     \end{itemize}
...
\item  {\bf  Viewer }
     \begin{itemize}
      \item  A distance measurement tool has been added
      \item  the spectral line tool can now plot errorbars on the spectrum
      \item  support of error cubes in viewer 
      \item  the spectrum of a cube can be derived as an average, sum,
        median, and weighted average of a selected region
      \item better support of spectral units
      \item elliptical region tool added
     \end{itemize}

\item  {\bf  Single Dish }
     \begin{itemize}
      \item  {\tt  sdbaseline:} Automated sinusoidal fitting based on FFTs
      \item  more uniformity in infile/outfile parameter naming throughout the single dish package
     \end{itemize}

\item  {\bf  Performance }
     \begin{itemize}
      \item  Copying the POINTING subtables is now 75-300 times faster       
      \item  {\tt  partition:} new task to split a MS into a multi-MS that is optimized for parallelized processing
      \item  tasks {\tt flagdata, clearcal, applycal} now work in parallelized mode, ie. they make use of multi-core multi-node processing
      \item  {\tt pcont} tool method for parallelized imaging and deconvolution of continuum data
     \end{itemize}


\item  {\bf  Other }
     \begin{itemize}
     \item New format for CASA regions. The new ascii format supports
       a large variety of shapes, offers flexibility for different
       coordinate systems and can be used as regions and
       annotations. See Chapter\,\ref{chapter:regionformat} for
       details. The new region format can be used in almost
       all tasks and we offer limited support to write out the new
       format in the current CASA viewer. Full support will be
...
(exportasdm)        plotxy              plotxy              plotxy            
(importevla)        vishead             (flagdata2)         split             
(importgmrt)        visstat             (testautoflag)      testconcat        
{importoldasdm}     (listsdm)                               uvcontsub         
                                                            vishead           
                                                            {uvcontsub2}      
...
listobs            : List data set summary in the logger or in a file
...
sim_analyze        : image and analyze simulated datasets
