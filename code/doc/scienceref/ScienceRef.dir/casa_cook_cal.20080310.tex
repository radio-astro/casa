...
% GvM 2007-02-22  add uvsub text
% STM 2007-02-25  add polcal text
...
tasks that are recently added to the Beta Release suite:
\begin{itemize}
...
Finally, there are also more advanced and experimental calibration tasks 
...
\begin{itemize}
   ...
   \item {\bf Polarization Calibration} --- solve for any unknown
      ...
      Calibration tasks are now available as of Beta Release Patch 1
      (\S~\ref{section:cal.solve.pol});
...
{\bf BETA ALERT:} The {\tt polcal} task is now available as
of Beta Patch 1.  It is still undergoing extensive testing,
and only basic capabilities are currently provided.
...
selectdata  =      False   #  Other data selection parameters
solint      =    86400.0   #  Solution interval (sec)
...
type {\tt 'D'}. {\bf BETA ALERT:} no table of type {\tt 'X'} will be
...
{\bf BETA ALERT:} {\tt 'X'} solutions are currently always
frequency-independent.

{\bf BETA ALERT:} {\tt polcal} will obtain a separate D-term solution for
...
\subsubsection{Polarization Calibration Heuristics}
...
polarization is known a priori.  Polarized sources must always be 
observed with sufficient parallactic angle coverage, where "sufficient"
is determined by SNR and the details of the solving mode.
...
in {\tt MODEL\_DATA}).  
{\bf BETA ALERT:} {\tt polmode='D+X'} will soon deliver this automatically.
