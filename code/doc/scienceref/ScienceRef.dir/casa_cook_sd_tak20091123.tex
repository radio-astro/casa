...
{\bf For R3.0, ASAP included in CASA was updated to Version 2.3.1 (which is 
...
installation.  Since we extended ASAP functionality, there
are certain functionality only available in the CASA version of ASAP.
In the following, we outline how to access ASAP
...
Before R3.0, all messages from ASAP were written out to the standard output
({\tt sys.stdout}), and they disappeared after you exited CASA. From R3.0, a
logging system of ASAP is integrated into CASA logging system. That is,
like CASA commands, all outputs from ASAP commands, except for GUI
related notifications, are sent to the file {\tt casapy.log} as well as they
...
All of the SDtasks, except for imaging related ones (
{\tt sdtpimaging}, {\tt sdimaging}, and {\tt sdimprocess}), work from a file on disk rather than from
...
images by either 'Basket-Weaving' or 'Pressed-out' methods.
\newpage 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
calibrated spectra to verify the calibration results if {\tt verify} is 'True'. 
  \item If an observation mode of input data are On-The-Fly, the direction
information will be considered for averaging, i.e., spectra will be
averaged only if the direction is in agreement with each other.
  \item Calibration of frequency switching data is now fully implemented
including the folding.
  \item Additional calibration algorithms are implemented. These are the
'Chopper-Wheel' calibration and the one adopted in APEX telescope (
an advanced version of classical 'Chopper-Wheel' method). Two load
calibration, which will be a standard calibration mode of ALMA total
power antennas, is formally implemented, but is still experimental.
\end{itemize}
...
\end{verbatim}
    ...
    {\bf R3.0 New Feature:}
     ...
     A new parameter {\tt verify} is added. The task plots both raw
...
\end{verbatim}
    ...
    *** Only apply to 'spectra' {\tt plottype} ***
    ...
    channel r        egion to calculate statistics for the all spectra
...
method (Sofue \& Reich 1979) can be used to remove the scanning 
        ...
        is to specify smoothing size directly. To do this, {\tt smoothsize} should 
        be specified as string that consists of a numerical value and an unit 
        (e.g. '10.0arcsec'). A value of {\tt beamsize} will be ignored in this case. 
        ...
        size. In this case, the {\tt beamsize} is interpreted as the observed beam 
...
{\bf R3.0 New Features:}
This is a newly created task and supposed to be merged to 
