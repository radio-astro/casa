...
//# RELEVANT (OR ALREADY-IMPLEMENTED) METHODS
  ...
  void beginLabelAndAxisCaching( ) { cache_label_and_axis = True; }
...
//# 			   Float xstretch = 1.0, Float ystretch = 1.0);
  ...
  virtual QPixmap* getBackBuffer() {return backBuffer_;  }
  ...
  virtual void setUpdateAllowed(Bool allowed=True) {
    if(allowed) {
      if(saveBuf_!=0) { delete saveBuf_; saveBuf_ = 0;  }  }
 ...
 private:
  ...
  Double q_(Double pY) { return QWidget::height()-1 - pY;  }
