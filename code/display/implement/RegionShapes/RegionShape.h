...
public:
    ...
    virtual bool isWorld() const { return m_isWorld; }
    
    // If this shape is in world coordinates, returns the system used.
    // Undefined for pixel coordinates.
    virtual MDirection::Types worldSystem() const { return m_worldSystem; }
    
    // Set world or pixel system.
    virtual void setIsWorld(bool isWorld) { m_isWorld = isWorld; }
    
    // Set world system.
    virtual void setWorldSystem(MDirection::Types sys) { m_worldSystem = sys; }
    
    // Returns this shape's type (color + type).
    virtual String title() const { return lineColor() + " " + type(); }
    ...
    // not.
    virtual bool hadDrawingError() const { return m_hadDrawingError; }
    
    // Returns the error that occurred during the last call to draw().
    const String& lastDrawingError() const { return m_drawingError; }
    
    // Returns this shape's properties as a Record.
    Record getProperties() const;
    
    // Synonym.
    Record toRecord() const { return getProperties(); }
