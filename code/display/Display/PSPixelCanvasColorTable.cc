...
namespace casa { //# NAMESPACE CASA - BEGIN
	...
	uInt PSPixelCanvasColorTable::nColors() const {
		return nColors_;
	}
...
// Returns the maximum # of colors per component.
	...
	uInt PSPixelCanvasColorTable::depth() const {
		return (uInt)bpc_;
	}
...
// map [0-1] float data to <[0-(n1_-1)], [0-(n2_-1)], [0-(n3_-1)]>
	void PSPixelCanvasColorTable::mapToColor3(Array<uLong> & outImage,
		...
		if(colorModel_ == Display::RGB)
			while (outq < endp) {
				t1 = (uLong)((*ch1q++)*MAXCOLOR);
				if (t1 > MAXCOLOR) t1 = MAXCOLOR;
				t2 = (uLong)((*ch2q++)*MAXCOLOR);
				if (t2 > MAXCOLOR) t2 = MAXCOLOR;
				t3 = (uLong)((*ch3q++)*MAXCOLOR);
				if (t3 > MAXCOLOR) t3 = MAXCOLOR;
				*outq++ = (t1 << RSHIFT) | (t2 << GSHIFT) | (t3 << BSHIFT);
			}
		else	// HSV
			while (outq < endp){
