...
// Constructor.
	PanelDisplay::PanelDisplay(PixelCanvas* pixelcanvas,
		...
		//itsWCLI(0),
		//itsWCHLI(0),
		itsMWCTools( CountedPtr<MultiWCTool>( ), uInt(10) ) {
...
// Option handling functions.
	...
	void PanelDisplay::addTool(const String& key, const CountedPtr<MultiWCTool> &value) {
	...
	void PanelDisplay::removeTool(const String& key) {
		...
		CountedPtr<MultiWCTool> tool = itsMWCTools(key);
		itsMWCTools.remove(key);
		if ( tool.get( ) == 0 ) return;
		tool->removeWorldCanvases(this);
	}

	const CountedPtr<MultiWCTool> PanelDisplay::getTool(const String& key) {
		if(!itsMWCTools.isDefined(key)) return CountedPtr<MultiWCTool>( );
