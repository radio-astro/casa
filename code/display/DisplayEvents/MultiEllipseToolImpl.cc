//# MultiEllipseToolImpl.cc: WorldCanvas event-based ellipse region drawer
...
namespace casa { //# NAMESPACE CASA - BEGIN
	...
	void MultiEllipseToolImpl::doubleInside() {
	...
	void MultiEllipseToolImpl::getLinearCoords(Vector<Double> &blc, Vector<Double> &trc) {
	...
	void MultiEllipseToolImpl::getWorldCoords(Vector<Double> &blc, Vector<Double> &trc) {
		if (!itsCurrentWC) return;
		// get the pixel coordinates of the elliptical region
		Int x1, y1, x2, y2;
		get(x1, y1, x2, y2);
	...
	void MultiEllipseToolImpl::handleEvent(DisplayEvent& ev) {
