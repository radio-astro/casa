...
// 		Double minor = beam.getMinor("rad");
	...
	SHARED_PTR<viewer::PVLine> MultiPVTool::allocate_region( WorldCanvas *wc, double x1, double y1, double x2, double y2, VOID * ) const {
		...
		// return SHARED_PTR<viewer::PVLine>(result);
	...
	bool MultiPVTool::create( viewer::region::RegionTypes /*region_type*/, WorldCanvas *wc,
		...
		SHARED_PTR<viewer::PVLine> result = allocate_region( wc, pts[0].first, pts[0].second, pts[1].first, pts[1].second, region_specific_state );
