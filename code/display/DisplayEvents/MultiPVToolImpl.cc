//# MultiPVToolImpl.cc: WorldCanvas event-based rectangle region drawer
...
namespace casa { //# NAMESPACE CASA - BEGIN
	...
	void MultiPVToolImpl::doubleInside() {
	...
	void MultiPVToolImpl::getWorldCoords(Vector<Double> &blc, Vector<Double> &trc) {
		if (!itsCurrentWC) return;
		// get the pixel coordinates of the rectangular region
		Int x1, y1, x2, y2;
		get(x1, y1, x2, y2);
	...
	void MultiPVToolImpl::handleEvent(DisplayEvent& ev) {
