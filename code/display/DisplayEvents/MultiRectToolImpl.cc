//# MultiRectToolImpl.cc: WorldCanvas event-based rectangle region drawer
...
namespace casa { //# NAMESPACE CASA - BEGIN
	...
	void MultiRectToolImpl::doubleInside() {
	...
	void MultiRectToolImpl::getLinearCoords(Vector<Double> &blc, Vector<Double> &trc) {
	...
	void MultiRectToolImpl::getWorldCoords(Vector<Double> &blc, Vector<Double> &trc) {
		if (!itsCurrentWC) return;
		// get the pixel coordinates of the rectangular region
		Int x1, y1, x2, y2;
		get(x1, y1, x2, y2);
	...
	void MultiRectToolImpl::handleEvent(DisplayEvent& ev) {
