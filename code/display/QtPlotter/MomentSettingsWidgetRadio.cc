...
namespace casa {
	...
	void MomentSettingsWidgetRadio::collapseImage() {
		...
		CountedPtr<const ImageInterface<float> > image = taskMonitor->getImage();
		...
		if ( imageAnalysis == NULL ) {
			...
			CountedPtr<ImageInterface<float> > image2(image->cloneII());
...
//#warning "Revert to THREADING"
	...
	void MomentSettingsWidgetRadio::collapseDone() {
		...
		if ( collapseThread != NULL && collapseThread->isSuccess()) {
			...
			for ( int i = 0; i < static_cast<int>(results.size()); i++ ) {
				...
				CountedPtr<ImageInterface<Float> > newImage = results[i].getImage();
	...
	void MomentSettingsWidgetRadio::convertChannelRanges( const QString& oldUnits, const QString& newUnits ) {
		...
		if ( imageAnalysis != NULL ){
			CountedPtr<const ImageInterface<Float> > imagePtr = imageAnalysis->getImage();
			Bool validCoord;
	...
	void MomentSettingsWidgetRadio::reset() {
		...
		collapseThread = NULL;
		if ( taskMonitor != NULL ) {
			CountedPtr<ImageInterface<Float> > img = const_pointer_cast<ImageInterface <Float> >(taskMonitor->getImage());
	...
	void MomentSettingsWidgetRadio::graphicalThreshold() {
		...
		CountedPtr<ImageInterface<Float> > image(const_pointer_cast<ImageInterface<Float> >(taskMonitor->getImage()));
