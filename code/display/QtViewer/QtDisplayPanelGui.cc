...
void QtDisplayPanelGui::construct_( QtDisplayPanel *newpanel, const std::list<std::string> &args ) {
	...
	CountedPtr<QtPolylineTool> pos = dynamic_pointer_cast<QtPolylineTool>(panelDisplay->getTool(QtMouseToolNames::POLYLINE));
	if (pos) {
		CountedPtr<viewer::QtRegionSourceKernel> qrs = dynamic_pointer_cast<viewer::QtRegionSourceKernel>(pos->getRegionSource( )->kernel( ));
		if ( qrs ) {
...
void QtDisplayPanelGui::generateHistogramRegionUpdates(){
	//Region updates
	PanelDisplay* panelDisplay = qdp_->panelDisplay();
	CountedPtr<QtCrossTool> pos = dynamic_pointer_cast<QtCrossTool>(panelDisplay->getTool(QtMouseToolNames::POINT));
	if (pos) {
		CountedPtr<viewer::QtRegionSourceKernel> qrs = dynamic_pointer_cast<viewer::QtRegionSourceKernel>(pos->getRegionSource( )->kernel( ));
		if ( qrs ) {
...
void QtDisplayPanelGui::resetListenerImage() {
	if (qdp_ != NULL ){
		...
		if ( mainImage != NULL ) {
			CountedPtr<ImageInterface<float> > img = mainImage->imageInterface();
			if ( sliceTool != NULL ) {
			...
			if ( histogrammer != NULL ) {
				histogrammer->setImage( CountedPtr<ImageInterface<Float> >() );
			}
			if ( fitTool != NULL ){
				fitTool->setImage( CountedPtr<ImageInterface<Float> >());
			}
		}
	}
}
...
void QtDisplayPanelGui::initFit2DTool() {
	if ( qdp_ != NULL ){
		...
		CountedPtr<QtRectTool> rect = dynamic_pointer_cast<QtRectTool>(panelDisplay->getTool(QtMouseToolNames::RECTANGLE));
		// one region source is shared among all of the tools...
		// so there is no need to connect these signals for all of the tools...
		if ( rect.get( ) != 0 ) {
			CountedPtr<viewer::QtRegionSourceKernel> qrs = dynamic_pointer_cast<viewer::QtRegionSourceKernel>(rect->getRegionSource( )->kernel( ));
			if ( qrs ) {
...
void QtDisplayPanelGui::createRGBImage( QtDisplayData* coordinateMaster,
	...
	CountedPtr<ImageInterface<Float> > imgPtr = coordinateMaster->imageInterface();
	/*********************************************************
...
void QtDisplayPanelGui::addDDSlot(String path, String dataType, String displayType,
	...
	//CountedPtr<ImageInterface<Float> > imgPtr(img);
...
QtDisplayData* QtDisplayPanelGui::addDD(String path, String dataType, String displayType, Bool autoRegister, Bool tmpData,
		CountedPtr<ImageInterface<Float> > img) {
	// create a new DD
...
void QtDisplayPanelGui::clearTools() {
	...
	if ( fitTool != NULL ) {
		CountedPtr<const ImageInterface<Float> > p;
		fitTool->setImage( p );
	}
	if ( histogrammer != NULL ) {
		CountedPtr< ImageInterface<Float> > p;
...
QtDisplayData* QtDisplayPanelGui::lookForExistingController() {
	...
	while( iter != qdp_->beginRegistered()) {
		...
		if ( pdd != 0 && pdd->isImage() ) {
			CountedPtr<ImageInterface<float> > img = pdd->imageInterface( );
...
void QtDisplayPanelGui::initializeProfile( ){
	if (profile_ == NULL) {
		CountedPtr<ImageInterface<float> > emptyImg;
		...
		connect(profile_, SIGNAL(showCollapsedImg(String, String, String, Bool, Bool, CountedPtr<ImageInterface<Float> > )),
				this, SLOT(addDDSlot(String, String, String, Bool, Bool, CountedPtr<ImageInterface<Float> >)));
...
void QtDisplayPanelGui::showImageProfile() {
	...
	if ( channelDDFound ){
		...
		PanelDisplay* ppd = qdp_->panelDisplay();
		CountedPtr<QtRectTool> rect = dynamic_pointer_cast<QtRectTool>(ppd->getTool(QtMouseToolNames::RECTANGLE));
...
void QtDisplayPanelGui::connectRegionSignals(PanelDisplay* ppd ) {
	CountedPtr<QtCrossTool> pos = dynamic_pointer_cast<QtCrossTool>(ppd->getTool(QtMouseToolNames::POINT));
	if ( pos.get( ) != 0 ) {
		...
		CountedPtr<QtOldCrossTool> pos = dynamic_pointer_cast<QtOldCrossTool>(ppd->getTool(QtMouseToolNames::POINT));
		if ( pos.get( ) != 0 ) {
			connect( pos.get( ), SIGNAL(wcNotify( const String, const Vector<Double>, const Vector<Double>,
					const Vector<Double>, const Vector<Double>, const ProfileType)),
					profile_, SLOT(wcChanged( const String, const Vector<Double>, const Vector<Double>,
	...
	if ( rect.get( ) != 0 ) {
		...
		CountedPtr<QtOldRectTool> rect = dynamic_pointer_cast<QtOldRectTool>(ppd->getTool(QtMouseToolNames::RECTANGLE));
		if ( rect.get( ) != 0 ) {
			connect( rect.get( ), SIGNAL(wcNotify( const String, const Vector<Double>, const Vector<Double>,
					const Vector<Double>, const Vector<Double>, const ProfileType)),
					profile_, SLOT(wcChanged( const String, const Vector<Double>, const Vector<Double>,
	...
	CountedPtr<QtEllipseTool> ellipse = dynamic_pointer_cast<QtEllipseTool>(ppd->getTool(QtMouseToolNames::ELLIPSE));
	if ( ellipse.get( ) != 0 ) {
		...
		CountedPtr<QtOldEllipseTool> ellipse = dynamic_pointer_cast<QtOldEllipseTool>(ppd->getTool(QtMouseToolNames::ELLIPSE));
		if ( ellipse.get( ) != 0 ) {
			connect( ellipse.get( ), SIGNAL(wcNotify( const String, const Vector<Double>, const Vector<Double>,
					const Vector<Double>, const Vector<Double>, const ProfileType )),
					profile_, SLOT(wcChanged( const String, const Vector<Double>, const Vector<Double>,
	...
	CountedPtr<QtPolyTool> poly = dynamic_pointer_cast<QtPolyTool>(ppd->getTool(QtMouseToolNames::POLYGON));
	if ( poly.get( ) != 0 ) {
		...
		CountedPtr<QtOldPolyTool> poly = dynamic_pointer_cast<QtOldPolyTool>(ppd->getTool(QtMouseToolNames::POLYGON));
		if ( poly.get( ) != 0 ) {
			connect( poly.get( ), SIGNAL(wcNotify( const String, const Vector<Double>, const Vector<Double>,
					const Vector<Double>, const Vector<Double>, const ProfileType )),
					profile_, SLOT(wcChanged( const String, const Vector<Double>, const Vector<Double>,
