...
namespace casa {
	...
	static const char *all_dec_   = "All Decs";
	static const char *this_dec_  = "This Dec";
	...
	QtCleanPanelGui2::QtCleanPanelGui2( QtViewer *v, QWidget *parent, const std::list<std::string> &args ) :
		...
		//		frame_2->setGeometry(QRect(300, 0, 111, 51));
		frame_2->setGeometry(QRect(0, 0, 111, 51));
...
// 	input_palette.setColor( backgroundRole(), QColor(122, 193, 66) );	// Qt Green
		...
		//		frame_4->setGeometry(QRect(410, 0, 161, 51));
		frame_4->setGeometry(QRect(110, 0, 161, 51));
		...
		thisPlaneRB_->setFont(smallFont);
		...
		//		frame_4H->setGeometry(QRect(570, 0, 161, 51));
		frame_4H->setGeometry(QRect(270, 0, 161, 51));
		...
		thisHiddenRB_->setFont(smallFont);
		...
		allHiddenRB_->setFont(smallFont);
		...
		//		frame_3->setGeometry(QRect(730, 0, 171, 51));
		frame_3->setGeometry(QRect(430, 0, 171, 51));
		...
		//		frame->setGeometry(QRect(0, 0, 300, 51));
		frame->setGeometry(QRect(0, 51, 600, 51));
		...
		label_3->setGeometry(QRect(310, 0, 120, 20));
		label_3->setFont(smallFont);
		label_3->setAlignment(Qt::AlignCenter);

		threshED_ = new QLineEdit(frame);
		threshED_->setObjectName(QString::fromUtf8("threshED_"));
		threshED_->setGeometry(QRect(310, 20, 120, 22));
		threshED_->setMinimumSize(QSize(120, 0));
		...
		label_4->setGeometry(QRect(460, 0, 120, 20));
		label_4->setFont(smallFont);
		label_4->setAlignment(Qt::AlignCenter);

		cyclethreshED_ = new QLineEdit(frame);
		cyclethreshED_->setObjectName(QString::fromUtf8("cyclethreshED_"));
		cyclethreshED_->setGeometry(QRect(460, 20, 120, 22));
		cyclethreshED_->setMinimumSize(QSize(120, 0));
		cyclethreshED_->setMaxLength(128);
		cyclethreshED_->setToolTip("Threshold at which to trigger next major cycle. Set to 'auto' for calculation as  ( peak residual ) * min[ max [ ( max psf sidelobe * cyclefactor ), minpsffraction ], maxpsffraction] ");
		...
		cyclethreshED_->setText(QApplication::translate("InteractiveClean", "auto", 0, QApplication::UnicodeUTF8));
		label_5->setText(QApplication::translate("InteractiveClean", "Next Action:", 0, QApplication::UnicodeUTF8));
		label_2->setText(QApplication::translate("InteractiveClean", "max cycleniter", 0, QApplication::UnicodeUTF8));
		label_3->setText(QApplication::translate("InteractiveClean", "threshold", 0, QApplication::UnicodeUTF8));
		label_4->setText(QApplication::translate("InteractiveClean", "cyclethreshold", 0, QApplication::UnicodeUTF8));
		label->setText(QApplication::translate("InteractiveClean", "iterations left", 0, QApplication::UnicodeUTF8));
		niterED_->setText(QApplication::translate("InteractiveClean", "100", 0, QApplication::UnicodeUTF8));
		cycleniterED_->setText(QApplication::translate("InteractiveClean", "60", 0, QApplication::UnicodeUTF8));
...
#define EXIT_FUNC(NAME,STR,EXTRA)						\
	...
	state["cycleniter"] = cycleniterED_->text().toInt();				\
	state["threshold"] = threshED_->text();					\
	state["cyclethreshold"] = cyclethreshED_->text();					\
	...
	QVariant QtCleanPanelGui2::setoptions(const QMap<QString,QVariant> &input, int /*id*/) {
		if(input.contains("niter"))
			niterED_->setText(input["niter"].toString());
		if(input.contains("cycleniter"))
			cycleniterED_->setText(input["cycleniter"].toString());
		if(input.contains("threshold"))
			threshED_->setText(input["threshold"].toString());
		if(input.contains("cyclethreshold"))
			cyclethreshED_->setText(input["cyclethreshold"].toString());
	...
	void QtCleanPanelGui2::addedData( QString type, QtDisplayData *dd ) {
		if ( type == "contour" ) {
			maskdd_ = dd;
			CountedPtr<ImageInterface<Float> > maskim=maskdd_->imageInterface();
...
void QtCleanPanelGui2::NAME( String x, String y, String z, std::vector<int> hidden ) {	\
										\
	...
	void QtCleanPanelGui2::changeMaskSelectionText( String x, String y, String z ) {
		if ( z == comp_freq_ ) {
	...
	void QtCleanPanelGui2::closeEvent(QCloseEvent *event) {
...
//	if ( ! v_->exiting( ) && ! isOverridedClose( ) ) {
	...
	void QtCleanPanelGui2::newMouseRegion(Record mouseRegion, WorldCanvasHolder* wch) {
	...
	void QtCleanPanelGui2::writeRegionText(const ImageRegion& imageReg, const String& filename, Float value) {
