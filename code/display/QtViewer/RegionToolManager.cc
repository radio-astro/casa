	...
	namespace viewer {
		...
		bool RegionToolManager::add_mark_select( RegionTool::State &state, region::RegionSelect select ) {
			...
			if ( state.count( region::PointInside ) > 0 ) {
				CountedPtr<viewer::region::region_list_type> new_marked_regions = state.regions(region::PointInside, select);
		...
		bool RegionToolManager::setup_moving_regions( RegionTool::State &state, region::RegionSelect select ) {
			CountedPtr<viewer::region::region_list_type> point_inside = state.regions( region::PointInside, select );
		...
		void RegionToolManager::operator()(const WCPositionEvent& ev) {
			...
			if ( ev.keystate() ) {
				...
				typedef std::map<Display::KeySym,CountedPtr<RegionTool> > keytools_t;
				...
				if ( region_buttons.find(ev.key( )) != region_buttons.end( ) ) {
					...
					if ( ev.modifiers( ) & Display::KM_Shift ) {
						...
						CountedPtr<viewer::region::region_list_type> handles = state.regions( region::PointHandle, selection );
						if ( handles->size( ) > 0 ) {
							moving_handle = true;
		...
		CountedPtr<RegionTool> RegionToolManager::tool( region::RegionTypes type ) {
			switch ( type ) {
