...
// </synopsis>
	...
	public:
		...
		// <group>
		LatticePADisplayData( CountedPtr<ImageInterface<T> > image, const uInt xAxis, const uInt yAxis, const uInt mAxis, const IPosition fixedPos, viewer::StatusSink *sink=0 );
		LatticePADisplayData(CountedPtr<ImageInterface<T> > image, const uInt xAxis,
		                     const uInt yAxis);
		// </group>
		...
		CountedPtr<ImageInterface<Float> > imageinterface( );
		...
		virtual CountedPtr<MaskedLattice<T> > maskedLattice() {
	...
	private:
		...
		CountedPtr<ImageInterface<T> > itsBaseImagePtr;
		...
		CountedPtr<MaskedLattice<T> > itsMaskedLatticePtr;
