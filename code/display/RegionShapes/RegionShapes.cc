...
// RSELLIPSE DEFINITIONS //
	...
	bool RSEllipse::drawAndUpdateBoundingBox(WorldCanvasHolder& wch, String* err) {
		...
		if(m_width > 0) {
			m_screenXMin -= m_width / 2;
			m_screenXMax += m_width / 2;
			m_screenYMin -= m_width / 2;
			m_screenYMax += m_width / 2;
		}
	...
	vector<double> RSEllipse::coordParameterValues() const {
		return m_spec;
	}
	...
	void RSEllipse::move(double dx, double dy, String system) {
		...
		if(system.empty() || (m_isWorld && system == MDirection::showType(
			...
			if(system == RSUtils::PIXEL) { // pixel to world
				...
				m_spec[0] = wx.getValue()(0);
				m_spec[1] = wy.getValue()(0);
	...
	void RSEllipse::updateMinMax() {
		m_xMin = m_spec[0] - m_spec[2];
		m_xMax = m_spec[0] + m_spec[2];
		m_yMin = m_spec[1] - m_spec[3];
		m_yMax = m_spec[1] + m_spec[3];
	}
...
// RSPOLYGON DEFINITIONS //
	...
	bool RSPolygon::drawAndUpdateBoundingBox(WorldCanvasHolder& wch, String* err) {
		...
		// update bounding box
		m_screenXMin = m_screenX(0);
		m_screenXMax = m_screenX(0);
		m_screenYMin = m_screenY(0);
		m_screenYMax = m_screenY(0);
...
// RSLINE DEFINITIONS //
	...
	bool RSLine::drawAndUpdateBoundingBox(WorldCanvasHolder& wch, String* err) {
		...
		if(m_arrow1 || m_arrow2) {
			...
			if(m_arrow1) {
				...
				polyX(0) = m_screenSpec[0];
				polyY(0) = m_screenSpec[1];
				polyX(1) = x1;
				polyY(1) = y1;
				polyX(2) = x2;
				polyY(2) = y2;
				...
				case FilledTriangle:
					...
					break;
				case Triangle:
					pc->drawPolygon(polyX, polyY);
					break;
				case V:
					pc->drawLine(polyX(0), polyY(0), polyX(1), polyY(1));
					pc->drawLine(polyX(0), polyY(0), polyX(2), polyY(2));
					break;
				case FilledDoubleV: {
			...
			if(m_arrow2) {
				arrowPoints(m_screenSpec[0], m_screenSpec[1],
				            ...
				            m_arrowLength, x1, y1, x2, y2);
				Vector<double> polyX(3), polyY(3);
				polyX(0) = m_screenSpec[2];
				polyY(0) = m_screenSpec[3];
				polyX(1) = x1;
				polyY(1) = y1;
				polyX(2) = x2;
				polyY(2) = y2;
		...
		// update bounding box
		m_screenXMin = m_screenSpec[0];
		m_screenXMax = m_screenSpec[0];
		m_screenYMin = m_screenSpec[1];
		m_screenYMax = m_screenSpec[1];
	...
	void RSLine::move(double dx, double dy, String system) {
		...
		if(system.empty() || (m_isWorld && system == MDirection::showType(
			...
			if(system == RSUtils::PIXEL) { // pixel to world
				...
				wx.getValue()(0) = m_spec[0];
				wy.getValue()(0) = m_spec[1];
				wx.getValue()(1) = m_spec[2];
				wy.getValue()(1) = m_spec[3];
				...
				m_spec[0] = wx.getValue()(0);
				m_spec[1] = wy.getValue()(0);
				m_spec[2] = wx.getValue()(1);
				m_spec[3] = wy.getValue()(1);
			} else if(m_isWorld) {         // world to world
				MDirection::Types sys;
				if(!MDirection::getType(sys, system)) return;
				Quantum<Vector<double> > q1(Vector<double>(2), UNIT),
				...
				m_spec[0] = q1.getValue()(0);
				m_spec[1] = q1.getValue()(1);
				q1.getValue()(0) = m_spec[2];
				q1.getValue()(1) = m_spec[3];
				if(!RSUtils::convertWCS(q1, m_worldSystem, q2, sys)) return;
				q2.getValue()(0) += dx;
				q2.getValue()(1) += dy;
				if(!RSUtils::convertWCS(q2, sys, q1, m_worldSystem)) return;
				m_spec[2] = q1.getValue()(0);
				m_spec[3] = q1.getValue()(1);
	...
	vector<vector<String> > RSLine::optionChoices() const {
		...
		v[2] = allArrowTypesStrings();
		v[3] = allArrowTypesStrings();
		return v;
	}
	...
	vector<RSOption> RSLine::optionValues() const {
		...
		v[0] = m_arrow1;
		v[1] = m_arrow2;
		v[2] = arrowType(m_aType1);
		v[3] = arrowType(m_aType2);
		return v;
	}
