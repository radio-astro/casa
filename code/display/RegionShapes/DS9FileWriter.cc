...
bool DS9FileWriter::writeShape(QTextStream& file, stringstream& errors,
    ...
    if(c != NULL) {
        ...
        double x, y, angle, length; int temp;
...
int QtDS9RFWOptions::getPrecision() const { return precisionSpinner->value(); }
...
bool DS9FileWriter::regionType(const RegionShape* shape,
    ...
    const RSMarker* m; const RSComposite* cp;
    if(dynamic_cast<const RSCircle*>(shape) != NULL) type = DS9::Circle;
        ...
        if(markers.size() == 0) {
            ...
            switch(markers[0]) {
            case Display::Cross:   type = DS9::CrossPoint; break;
            case Display::X:       type = DS9::XPoint; break;
            case Display::Diamond: type = DS9::DiamondPoint; break;
            case Display::Circle:  type = DS9::CirclePoint; break;
            case Display::Square:  type = DS9::BoxPoint; break;
        ...
        if(children.size() == 0) {
            ...
            if(isAnn) {
                ...
                for(unsigned int i = 1; isAnn && i < children.size(); i++) {
                    ...
                    if(x != x2 || y != y2) { isAnn = false; break; }
                }
                if(isAnn) {
                    type = DS9::Annulus;
                    return true;
