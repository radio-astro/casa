...
// RSUTILS DEFINITIONS //
	...
	bool RSUtils::worldToPixel(const Quantum<Vector<double> >& worldX,
		...
		for(unsigned int i = 0; i < worldX.getValue().size(); i++) {
			...
			t.setValue(qc.getValue()(0));
			world[0] = t.getValue(units(0));
			t.setValue(qc.getValue()(1));
			world[1] = t.getValue(units(1));

			try {
				if(!cs.toPixel(pixel, world)) wasError = true;
			} catch(...) {
				wasError = true;
			}
			if(wasError) {
	...
	bool RSUtils::pixelToWorld(const Vector<double>& pixelX,
		...
		for(unsigned int i = 0; i < pixelX.size(); i++) {
			...
			try {
				if(!cs.toWorld(world, pixel)) wasError = true;
			} catch(...) {
				wasError = true;
			}
			if(wasError) {
			...
			tx.setValue(world[0]);
			q.getValue()(0)= tx.getValue(RegionShape::UNIT);
			ty.setValue(world[1]);
			q.getValue()(1)= ty.getValue(RegionShape::UNIT);
	...
	bool RSUtils::worldToLinear(const Quantum<Vector<double> >& worldX,
		...
		for(unsigned int i = 0; i < worldX.getValue().size(); i++) {
			...
			usingPixel = false;
			try {
				if(!wch.worldToLin(linear, wcWorld)) wasError = true;
			} catch(...) {
				wasError = true;
			}
			if(wasError) {
				...
				// should work...
				wcWorld2(0) = wcWorld(0);
				wcWorld2(1) = wcWorld(1);
				try {
					if(!cs.toPixel(linear, wcWorld2)) wasError = true;
				} catch(...) {
					wasError = true;
				}
				if(wasError) {
	...
	bool RSUtils::pixelToLinear(const Vector<double>& pixelX,
		...
		for(unsigned int i = 0; i < pixelX.size(); i++) {
			...
			world2(1) = world(1);
			try {
				if(!wch.worldToLin(linear, world2)) wasError = true;
			} catch(...) {
				wasError = true;
			}
			if(wasError) {
...
// RSOPTION DEFINITIONS //
	...
	bool RSOption::isString() const {
		return m_isString;
	}
	bool RSOption::isBool() const {
		return m_isBool;
	}
	bool RSOption::isDouble() const {
		return m_isDouble;
	}
	bool RSOption::isStringArray() const {
		return m_isStringArray;
	}

	const String& RSOption::asString() const {
		return m_string;
	}
	bool RSOption::asBool() const {
		return m_bool;
	}
	double RSOption::asDouble() const {
		return m_double;
	}
	const vector<String>& RSOption::asStringArray() const {
	...
	RSOption& RSOption::operator=(const String& str) {
		m_isString = true;
		m_isBool = m_isDouble = m_isStringArray = false;
		m_string = str;
		return *this;
	}
	...
	RSOption& RSOption::operator=(bool b) {
		m_isBool = true;
		m_isString = m_isDouble = m_isStringArray = false;
		m_bool = b;
		return *this;
	}
	...
	RSOption& RSOption::operator=(double d) {
		m_isDouble = true;
		m_isBool = m_isString = m_isStringArray = false;
		m_double = d;
		return *this;
	}

	RSOption& RSOption::operator=(const vector<String>& v) {
		m_isStringArray = true;
		m_isBool = m_isDouble = m_isString = false;
		m_stringArray = v;
		return *this;
	}
...
//////////////////////////
	...
	void RSHandle::setMarkerType(Display::Marker type) {
		m_markerType = type;
	}
