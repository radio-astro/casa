...
namespace casa {
	...
	namespace viewer {
		...
		public:
			...
			operator CountedPtr<ImageRegion>( ) {
				return imageregion;
			}
		...
		private:
			...
			CountedPtr<ImageRegion> imageregion;
			size_t count_;
		};
		...
		public:
			...
			virtual std::list<CountedPtr<RegionInfo> > *statistics( ) {
				return generate_dds_statistics( );
			}

			// called when creating regions to allow suppression of corner-handle drawing...
			static CountedPtr<viewer::Region> creatingRegion( ) {
				return creating_region;
			}
			static void creatingRegionBegin( CountedPtr<viewer::Region> r ) {
				creating_region = r;
			}
...
// --------------------==================== from old QtRegion ====================--------------------
		...
		protected:
			void initHistogram();
			virtual std::list<CountedPtr<RegionInfo> > *generate_dds_statistics( );

			// hook to allow generate_dds_statistics( ) to generate statistics
			// for rectangular measurement set regions...
			virtual void generate_nonimage_statistics( DisplayData*, std::list<CountedPtr<RegionInfo> > * ) { }
			...
			virtual std::list<CountedPtr<RegionInfo> > *generate_dds_centers( ) = 0;
			...
			RegionInfo::center_t *getLayerCenter( PrincipalAxesDD *padd, CountedPtr<ImageInterface<Float> > image, ImageRegion& imgReg);
			...
			static CountedPtr<Region> creating_region;
