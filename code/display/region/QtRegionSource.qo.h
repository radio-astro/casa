...
//Used to change the position of the source.
		...
		protected:
			...
			/* CountedPtr<Rectangle> rectangle( int blc_x, int blc_y, int trc_x, int trc_y ); */
			CountedPtr<Rectangle> rectangle( RegionCreator *rc, WorldCanvas *wc, double blc_x, double blc_y, double trc_x, double trc_y );
			CountedPtr<Polygon> polygon( RegionCreator *rc, WorldCanvas *wc, double x1, double y1 );
			CountedPtr<Polygon> polygon( RegionCreator *rc, WorldCanvas *wc, const std::vector<std::pair<double,double> > &pts );
			CountedPtr<Polyline> polyline( RegionCreator *rc, WorldCanvas *wc, double x1, double y1 );
			CountedPtr<Polyline> polyline( RegionCreator *rc, WorldCanvas *wc, const std::vector<std::pair<double,double> > &pts );
			CountedPtr<PVLine> pvline( RegionCreator *rc, WorldCanvas *wc, double blc_x, double blc_y, double trc_x, double trc_y );

// ellipse is derived from rectangle... so while this should be "CountedPtr<Ellipse>" this would preclude
...
// generalized to "CountedPtr<Region>"...
			CountedPtr<Rectangle> ellipse( RegionCreator *rc, WorldCanvas *wc, double blc_x, double blc_y, double trc_x, double trc_y );
			CountedPtr<Rectangle> point( RegionCreator *rc, WorldCanvas *wc, double x, double y, QtMouseToolNames::PointRegionSymbols sym, int size );
