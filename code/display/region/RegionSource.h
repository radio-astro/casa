...
namespace casa {
	...
	namespace viewer {
		...
		public:
			typedef CountedPtr<RegionSourceKernel> shared_kernel_ptr_type;
		...
		protected:
			...
			virtual CountedPtr<Rectangle> rectangle( RegionCreator *rc, WorldCanvas *wc, double blc_x, double blc_y, double trc_x, double trc_y ) = 0;
			virtual CountedPtr<Polygon> polygon( RegionCreator *rc, WorldCanvas *wc, double x1, double y1 ) = 0;
			virtual CountedPtr<Polygon> polygon( RegionCreator *rc, WorldCanvas *wc, const std::vector<std::pair<double,double> > &pts ) = 0;
			virtual CountedPtr<Polyline> polyline( RegionCreator *rc, WorldCanvas *wc, double x1, double y1 ) = 0;
			virtual CountedPtr<Polyline> polyline( RegionCreator *rc, WorldCanvas *wc, const std::vector<std::pair<double,double> > &pts ) = 0;
			virtual CountedPtr<Rectangle> ellipse( RegionCreator *rc, WorldCanvas *wc, double blc_x, double blc_y, double trc_x, double trc_y ) = 0;
			virtual CountedPtr<Rectangle> point( RegionCreator *rc, WorldCanvas *wc, double x, double y, QtMouseToolNames::PointRegionSymbols sym, int size ) = 0;
			virtual CountedPtr<PVLine> pvline( RegionCreator *rc, WorldCanvas *wc, double blc_x, double blc_y, double trc_x, double trc_y ) = 0;
		...
		public:
			...
			CountedPtr<Rectangle> rectangle( WorldCanvas *wc, double blc_x, double blc_y, double trc_x, double trc_y ) {
				return kernel_->rectangle(region_creator,wc,blc_x,blc_y,trc_x,trc_y);
			}
			virtual CountedPtr<Polygon> polygon( WorldCanvas *wc, double x1, double y1 ) {
				return kernel_->polygon(region_creator,wc,x1,y1);
			}
			virtual CountedPtr<Polygon> polygon( WorldCanvas *wc, const std::vector<std::pair<double,double> > &pts ) {
				return kernel_->polygon(region_creator,wc,pts);
			}
			virtual CountedPtr<Polyline> polyline( WorldCanvas *wc, double x1, double y1 ) {
				return kernel_->polyline(region_creator,wc,x1,y1);
			}
			virtual CountedPtr<Polyline> polyline( WorldCanvas *wc, const std::vector<std::pair<double,double> > &pts ) {
				return kernel_->polyline(region_creator,wc,pts);
			}
			virtual CountedPtr<Rectangle> ellipse( WorldCanvas *wc, double blc_x, double blc_y, double trc_x, double trc_y ) {
				return kernel_->ellipse(region_creator,wc,blc_x,blc_y,trc_x,trc_y);
			}
			virtual CountedPtr<Rectangle> point( WorldCanvas *wc, double x, double y, QtMouseToolNames::PointRegionSymbols sym, int size ) {
				return kernel_->point(region_creator,wc,x,y,sym,size);
			}
			CountedPtr<PVLine> pvline( WorldCanvas *wc, double blc_x, double blc_y, double trc_x, double trc_y ) {
