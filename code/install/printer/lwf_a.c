...
/* Paper sizes, all measurements are in centimetres. */
...
#define LMARGIN     1.0  /* Left margin. */
#define TMARGIN     1.0  /* Top margin. */
#define RMARGIN     1.0  /* Right margin. */
#define BMARGIN     0.9  /* Bottom margin. */
#define PINDENT     2.5  /* Page numbering indent from RH edge. */
#define HGAP        1.0  /* Gap between header and first line. */

#define XORIGIN     0.1  /* Print origin with respect to */
#define YORIGIN     0.2  /* the BLH corner of the paper. */
...
int  checkrange(char *range);
void endpage(FILE *outfile);
int  flushpage(FILE *infile);
int  getline(FILE *iop);
int  inrange(int num, char *range);
void preamble(void);
void print(FILE *infile);
int  printpage(FILE *infile, FILE *outfile);
void PStext(FILE *outfile);
void reverse(FILE *outfile);

double  points_per_cm = 72.0/2.54;
double  xorigin, xspan, yorigin, yspan;
...
int main(int argc, char** argv)
{
   ...
   for (i = 1; i < argc && argv[i][0] == '-'; i++) {
      ...
      case 't':
         ...
         if (spacing < 1.0 || spacing > 3.0) {
            fprintf(stderr, "%s: Invalid line spacing (%f) specified.\n",
                    progname, spacing);
            return 1;
         }
         break;
   ...
   page_x   = (int) ((xspan - xorigin - PINDENT) * points_per_cm);
   header_y = (int) ((yspan - yorigin - TMARGIN) * points_per_cm) - HEAD_SIZE;
   ...
   start_x  = (int) ((offset - xorigin) * points_per_cm);
   start_y  = header_y - (int) (HGAP * points_per_cm) - fontSize;
   colsep   = (int) (((xspan - (offset - xorigin)) / ncols) * points_per_cm);
...
void preamble(void)
{
   ...
   if (doLandsc) {
      end_x = (int) ((yspan - yorigin) * points_per_cm);
      printf("   /Landscape {\n");
   ...
   if (doBanner == 1) {
      ...
      border_x0 = (int) ((LMARGIN - xorigin) * points_per_cm);
      border_y0 = (int) ((BMARGIN - yorigin) * points_per_cm);
      border_x1 = (int) ((xspan - xorigin - RMARGIN) * points_per_cm);
      border_y1 = (int) ((yspan - yorigin - TMARGIN) * points_per_cm);
...
void print(FILE *infile)
{
   ...
   if (doReverse) {
      ...
      if ((outfile = fopen(revfile, "w+")) == (FILE *) NULL) {
...
/*----------------------------------------------------------------------------
* A page has been written to the temp file; record the start of the next page.
*---------------------------------------------------------------------------*/
void endpage(FILE *outfile)
{
...
/*----------------------------------------------------------------------------
* Read the next line of ASCII text.
*---------------------------------------------------------------------------*/
int getline(FILE *iop)
{
   register int ch;
