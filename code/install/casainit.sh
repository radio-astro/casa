#!/bin/echo Usage: .
#-----------------------------------------------------------------------------
# casainit.sh: Define the CASA environment for Bourne-like shells
#-----------------------------------------------------------------------------
#
...
# Usage: . casainit.sh
#-----------------------------------------------------------------------------
# casainit.sh bootstraps the CASA user environment for Bourne-like shells.
# It sets or modifies the following environment variables based partly on
# information contained in the "aipshosts" file:
#
#    CASAPATH  A sequence of four character strings separated by spaces
#              comprised of the following:
#
#                 1) Root of the CASA directory tree.
#                 2) Host "architecture".
#                 3) Local site name.
#                 4) Name of the machine.
#
#    PATH      The PATH modified for CASA.  Any prior CASA path
...
#              qualified pathname.
#
#              If the "aips_bin" keyword is not present, the CASA bin
#              directory will be added to the start of PATH, following
#              "." if it comes first.
#
#    MANPATH   If defined, MANPATH will be modified for CASA in the same
#              manner as for PATH except using the "aips_doc" keyword.
#
# The host "architecture", a string of the form "<base>[_<ext>]", corresponds
# to a subdirectory of the CASA root directory in which an CASA system
# resides (object libraries, executables,...).  The base part of the name is a
# mnemonic predefined by CASA which describes the innate binary format of a
...
#
# casainit recognizes an environment variable "aips_ext" which, if defined,
...
# 'casainit' which takes a value for "aips_ext" as an argument and reinvokes
# casainit.  This provides a convenient mechanism for interactively switching
# between architecture extensions.  Note that casainit always undefines
# "aips_ext".
#
...
# signals that the particular host does not have an CASA installation and
# causes CASA initialization to be skipped.  Thus an "aipshosts" entry of
#
#    DEFAULT    NONE
#
# defeats CASA initialization for all hosts other than those specifically
# listed.
#
# Notes:
...
# $Id$
#=============================================================================
# The CASA root directory is hardcoded here.
...
# Look for this host in the CASA hosts database.
  a_temp=""
...
#    Reset the architecture extension if required.
     ...
     then
        ...
        then
           ...
           [ "$a_ext" == "64b" -a "`uname -m`" != "x86_64" ] && a_ext='*'
           [ "$a_ext" != "*" ] && a_arch=${a_arch}_$a_ext
        fi
     fi

#    Is CASAPATH already defined?
     if [ "$CASAPATH" != "" ]
     then
        a_och=`/bin/echo $CASAPATH | awk '{print $2}'`
        a_old=`/bin/echo $CASAPATH | awk '{printf("%s/%s",$1,$2)}'`
     else
        a_och=$a_arch
        a_old=$a_root/$a_arch
     fi
...
#    Define CASAPATH.
     CASAPATH="$a_root $a_arch $a_site $a_host"
     export CASAPATH
...
#    Ensure that some CASA bin area got into PATH.
     /bin/echo $a_new | grep " $a_root/$a_arch/bin " > /dev/null 2>&1
     if [ "$?" != 0 ]
     then
#       Leave "." first, and put the CASA areas next.
...
#       Ensure that some CASA lib area got into LD_LIBRARY_PATH.
...
#       Ensure that some CASA man area got into MANPATH.
...
#    Function which invokes casainit with "aips_ext" as a command line argument.
     if [ "${BASH_VERSION-}" != "" ]
     then
        a_temp='casainit () { local aips_ext="$*" ; local a_root='$a_root' ; . '$a_root'/casainit.sh; }'
        eval $a_temp
     elif [ "$a_shell" = "ksh" ]
     then
        a_temp='casainit () { aips_ext="$*" ; a_root='$a_root' ; . '$a_root'/casainit.sh; }'
        eval $a_temp
     fi
  fi
