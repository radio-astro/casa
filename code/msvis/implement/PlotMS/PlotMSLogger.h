...
public:
    ...
    PMS_ENUM2(Level, levels, levelStrings, level, "off", "low", "med", "high")
    // </group>
    ...
    // Calls the parent PlotMS's setLevel method.
    void setLevel(Level level);
    
    // Calls the parent PlotMS's setLogMeasurementEvents method which in turn
    // calls the protected setMeasurementEvents_ method.  The protected method
    // is hidden this way so that external objects can set the measurement
    // event flags on either PlotMS or on PlotMSLogger with the same result.
