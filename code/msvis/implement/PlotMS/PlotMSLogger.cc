...
void PlotMSLogger::setLevel(Level level) {
    if(itsParent_ != NULL) itsParent_->setLogLevel(level);
}

void PlotMSLogger::setMeasurementEvents(int flags) {
    if(itsParent_ != NULL) itsParent_->setLogMeasurementEvents(flags);
}
