//# PlotMS.cc: Main class, controller for plotms.
//# Copyright (C) 2008
...
PlotMS::PlotMS(PlotMSLogger::Level logLevel) : itsLogLevel_(logLevel),
        itsLogMeasurementFlag_(PlotMSLogger::levelToMeasurementFlag(logLevel)),
        itsPlotter_(NULL), itsVisSet_(NULL) {
...
bool PlotMS::optionsAreValid(const PlotMSOptions& opts) {
    // check rows, cols
    unsigned int rows = opts.nrows(), cols = opts.ncols();
    if(rows == 0 || cols == 0) {
        itsLastError_ = "Rows/columns cannot be zero!";
        return false;
    }
    
    // for each parameters, must be either be unset or have a proper filename
    fstream fin;
    bool open;
    String file;
    for(unsigned int r = 0; r < rows; r++) {
        for(unsigned int c = 0; c < cols; c++) {
            if(!opts.parameters(r, c).isSet()) continue;
            file = opts.parameters(r, c).getFilename();
            if(file.empty()) {
                itsLastError_ = "Filename cannot be empty for set parameters!";
                return false;
            }
            ...
            fin.close();
            if(!open) {
                itsLastError_ = "Cannot open file '" + file + "'!";
                return false;
            }
        }
    }
...
bool PlotMS::setOptions(const PlotMSOptions& opts) {
    if(!optionsAreValid(opts)) return false;
    ...
    vector<vector<bool> > updateData(rows), updateSettings(rows), wasSet(rows);
    for(unsigned int r = 0; r < rows; r++) {
        updateData[r].resize(cols, true);
        updateSettings[r].resize(cols, true);
        wasSet[r].resize(cols, false);
        for(unsigned int c = 0; c < cols; c++) {
            if(index < itsOptions_.nrows() * itsOptions_.ncols()) {
                updateData[r][c] = !opts.parameters(r, c).equalsData(
                                    itsOptions_.parameters(index));
                updateSettings[r][c] = !opts.parameters(r, c).equalsSettings(
                                        itsOptions_.parameters(index));
                index++;
            } else updateData[r][c] = opts.parameters(r, c).isSet();
        }
    }
    itsOptions_ = opts;
    ...
    for(unsigned int r = 0; r < rows; r++) {
        for(unsigned int c = 0; c < cols; c++) {
            if(!updateData[r][c] && !updateSettings[r][c]) continue;
            const PlotMSParameters& params = itsOptions_.parameters(r, c);
            set = params.isSet();
            
            if(updateSettings[r][c]) {
                ...
                itsPlotter_->showLegend(set && params.showLegend(),
                                        params.getLegendPosition(), r, c);
            }
            ...
            if(updateData[r][c]) {
                if(!set) {
                    ...
                    if(log != NULL)
                        log->markMeasurement(CLASS_NAME, LOG_LOAD_CACHE);
                    
                    if(!openMS(r, c)) {
                        itsLastError_ = "Could not open Measurement Set: " +
                                        itsLastError_;
                        return false;
                    }
                
                    if(!getData(r, c)) {
                        itsLastError_= "Could not get MS data: "+itsLastError_;
                        return false;
                    }
                    ...
                    if(updated) status << " ";
                    status << "Plotted " << itsData_.size(r, c) << " points";
                    if(rows > 1 || cols > 1)
                        status << " on (" << r << ", " << c << ')';
                    status << ".";
                    updated = true;
    ...
    itsPlotter_->updateOptions(itsOptions_);
...
PlotMSLogger::Level PlotMS::logLevel() const { return itsLogLevel_; }

void PlotMS::setLogLevel(PlotMSLogger::Level level) {
    itsLogLevel_ = level;
    setLogMeasurementEvents(PlotMSLogger::levelToMeasurementFlag(level));
}

int PlotMS::logMeasurementEvents() const { return itsLogMeasurementFlag_; }

void PlotMS::setLogMeasurementEvents(int flags) {
    itsLogMeasurementFlag_ = flags;
    itsLogger_.setMeasurementEvents_(flags);
}
...
void PlotMS::handleLocate(vector<PlotMSRect> rects, bool applyToAll) {
    PlotMSParameters* p;
    for(unsigned int i = 0; i < rects.size(); i++) {
        p = &itsOptions_.parameters(rects[i].canvasRow, rects[i].canvasColumn);
        cout << "Locate " << PMS::axis(p->getXAxis()) << " in ["
             << rects[i].xLeft << ' ' << rects[i].xRight << "], "
             << PMS::axis(p->getYAxis()) << " in [" << rects[i].yBottom << ' '
             << rects[i].yTop << "]";
        if(itsOptions_.nrows() > 1 || itsOptions_.ncols() > 1)
            cout << " on canvas (" << rects[i].canvasRow << ", "
                 << rects[i].canvasColumn << ")";
        cout << ":" << endl;
        itsData_.locate(rects[i].xLeft, rects[i].xRight, rects[i].yBottom,
                        rects[i].yTop, rects[i].canvasRow,
                        rects[i].canvasColumn);
    }
}
...
bool PlotMS::openMS(unsigned int row, unsigned int col) {
    ...
    try {
        ...
        itsSelectedMS_ = itsMS_;
        mssSetData(itsMS_, itsSelectedMS_, "","","","2","","","","","");
        ...
        itsLastError_ = err.getMesg();
        return false;
    }
}

bool PlotMS::getData(unsigned int row, unsigned int col) {
    const PlotMSParameters& params = itsOptions_.parameters(row, col);
    
    try {        
        itsData_.loadCache(*itsVisSet_, params.getXAxis(), params.getYAxis());
        return true;
...
Bool PlotMS::applyPlotOptions()
{
  ...
  else if(itsPlotOptions.fastaxis == PlotMSOptions::Horizontal){
    rownum = &PlotMS::rownum_horizontal;
    colnum = &PlotMS::colnum_horizontal;
  }
  else{
