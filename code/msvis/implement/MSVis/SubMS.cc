...
namespace casa {
  ...
  Bool SubMS::fillAllTables(const String& colname){
	...
	return False;
      ...
      else if(spw_p.nelements() > 1){
	os << LogIO::WARN
	   <<"No split-time-average of multiple spws yet : please do them \nseperately and then concatenate " << LogIO::POST;
	return False;
      }
      else{
  ...
  Bool SubMS::fillTimeAverData(Vector<Int>& ant1, Vector<Int>& ant2, 
	...
	if(!rowFlag(k) && dataDesc(row)==dataDescIn(k)){
	  ...
	  if(doChanAver_p){
	    ...
	    outFlag.xyPlane(row) = outFlag.xyPlane(row)* flag(k)(blc,trc);
	    outData.xyPlane(row) = outData.xyPlane(row) + data(k)(blc,trc)*max(inRowWeight(k));
	  }
	  else{
