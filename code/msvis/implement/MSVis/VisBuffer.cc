...
void VisBuffer::channelAve(const Matrix<Int>& chanavebounds)
{
    ...
    if ( chanavebounds.nelements() > 0 ) {
        ...
        if (visCubeOK_p) {
            chanAveVisCube(visCube(), nChanOut);
        }
        if (modelVisCubeOK_p) {
            chanAveVisCube(modelVisCube(), nChanOut);
        }
        if (correctedVisCubeOK_p) {
            chanAveVisCube(correctedVisCube(), nChanOut);
        }
        if (floatDataCubeOK_p) {
            chanAveVisCube(floatDataCube(), nChanOut);
        }
...
void
VisBuffer::checkVisIter (const char *, const char * file, int line) const
{
    if (visIter_p == NULL) {
...
Cube<Complex>&
VisBuffer::fillVisCube(VisibilityIterator::DataColumn whichOne)
{
    CheckVisIter ();
    switch (whichOne) {
    case VisibilityIterator::Model:
...
Vector<Float>
VisBuffer::feed_pa(Double time) const
{
    return visIter_p->feed_pa(time);
}

Float
VisBuffer::parang0(Double time) const
{
    return visIter_p->parang0(time);
}

Vector<Float>
VisBuffer::parang(Double time) const
{
    return visIter_p->parang(time);
}

Int
VisBuffer::numberAnt () const
{
    return msColumns().antenna().nrow(); /* for single (sub)array only.*/
}

MDirection
VisBuffer::azel0(Double time) const
{
    return visIter_p->azel0(time);
}

Vector<Double>&
VisBuffer::azel0Vec(Double time, Vector<Double>& azelVec) const
{
...
Vector<MDirection>
VisBuffer::azel(Double time) const
{
