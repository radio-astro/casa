...
namespace casa { //# NAMESPACE CASA - BEGIN
  ...
  public:
    ...
    virtual void initializeToVis(ImageInterface<Complex>& image,
				 const vi::VisBuffer2& vb, Array<Complex>& griddedVis,
    ...
    // present in ms.
    void get(vi::VisBuffer2& vb, Cube<Complex>& degrid, 
	     Array<Complex>& griddedVis, Vector<Double>& scale, 
	     Int row=-1);
    
    void get(vi::VisBuffer2& vb, Cube<Float>& pointingOffsets, Int row=-1,
	     FTMachine::Type whichVBColumn=FTMachine::MODEL,Int Conj=0)
    {
    ...
    virtual Int findPointingOffsets(const vi::VisBuffer2&, Array<Float>&, Array<Float>&,
				    Bool Evaluate=True);
    virtual Int findPointingOffsets(const vi::VisBuffer2&, Cube<Float>&,
    ...
    MDirection::Convert makeCoordinateMachine(const vi::VisBuffer2&,
    ...
    void makeConjPolMap(const vi::VisBuffer2& vb, const Vector<Int> cfPolMap, 
			Vector<Int>& conjPolMap);
    //    Vector<Int> makeConjPolMap(const VisBuffer& vb);
    void makeCFPolMap(const vi::VisBuffer2& vb, const Vector<Int>& cfstokes, 
		      Vector<Int>& polM);
  ...
  protected:
    ...
    virtual void resampleDataToGrid(Array<Complex>& griddedData, VBStore& vbs, 
				    const vi::VisBuffer2& vb, Bool& dopsf);
