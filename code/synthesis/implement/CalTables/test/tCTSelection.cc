...
void doTest1 (Bool verbose=False) {
  ...
  uInt nFld(20), nAnt(10), nSpw(1), nTime(10);
  Vector<Int> nChan(nSpw,1);
  Double refTime(4832568000.0); // 2012 Jan 06 @ noon
  Double tint(60.0);

  Bool disk(False);
  NewCalTable tnct("temp.ct","T",nFld,nAnt,nSpw,nChan,nTime,refTime,tint,disk,False); // verbose);
  ...
  AlwaysAssert( (selnct.nrow()==nAnt*nTime*nSpw*fldids.nelements()), AipsError);

  ROCTMainColumns ctmc(selnct);
  Vector<Int> selfieldcol;
