...
TEST_F( genericGatherAndSolveRealDataTests , RealData_G ) {
  ...
  // Calibrater-driven exercise with simulated data for genericGatherAndSolve

  // The Calibrater object
  
  //MeasurementSet ms("Four_ants_3C286.ms");

  Calibrater cal;
  cal.initialize(ms_,false,false,false);

  //  cal.selectvis("","11:20,12:20,13:20,15:20","30,31");
  cal.selectvis("","*:20~40","30,31");


  // the "SVC": a SolvableVisCal object (e.g., GJones) that is the solved-for
  //            term; ordinarily generated by Calibrater::setSolve
  Record solvePar;
  String caltablename("realdata_test.G.11.20-40"); solvePar.define("table",caltablename);
  ...
  String combine("spw,scan");         solvePar.define("combine",combine);
  cal.setsolve(String("G"),solvePar);
...
TEST_F( genericGatherAndSolveRealDataTests , RealData_B ) {
  ...
  cal.selectvis("","11,12","31");  // scan 31 only
  ...
  cal.setsolve(String("B"),solvePar);
...
TEST_F( genericGatherAndSolveRealDataTests , RealData_preB_G ) {
  ...
  Calibrater cal;
  cal.initialize(ms_,false,false,false);
  cout << "cal.ok() = " << cal.ok() << endl;

  cal.selectvis("","11,12","31");  // scan 31 only

  cal.setapply("",0.0,"realdata_test.B","","","nearest,nearest",true,Vector<Int>(),Vector<Double>());
