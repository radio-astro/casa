//#---------------------------------------------------------------------------
//# ASTEFXHeader.cc: Class for ASTE-FX data header.
//#---------------------------------------------------------------------------
...
// ASTEFXHeader
//
// Class for ASTE-FX data header.
//

// constructor 
ASTEFXHeader::ASTEFXHeader() 
{
  // memory allocation
  initialize() ;
}
...
// fill data header
int ASTEFXHeader::fill( string name ) 
{
  // open file
  FILE *fp ;
  ...
  if ( ( 0 < tmp ) && ( tmp <= ASTE_ARYMAX_FX ) ) {
    same = true ;
    cout << "ASTEFXHeader::fill()  same endian " << endl ;
  }
  else {
    same = false ;
    cout << "ASTEFXHeader::fill()  different endian " << endl ;
  }
  fseek( fp, 0, SEEK_SET ) ;
...
int ASTEFXHeader::fill( FILE *fp, bool sameEndian )
{
