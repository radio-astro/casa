...
bool
testflagger::setMS(const std::string& msname)
{
	try
	{
		if(ms_p) delete ms_p;
		...
		if(testflagger_p){
			return testflagger_p->attach(*ms_p);
		}
...
flagger::open(const std::string& msname)
{
	try
	{
		...
		if (!flagger2_p) {
			flagger2_p = new Flagger2();
		}
		if (flagger2_p) {
			return flagger2_p->attach(*ms_p);
		}
		return new ::casac::record();
	} catch (AipsError x) {
		*logger_p << LogIO::SEVERE << "Exception Reported: " << x.getMesg() << LogIO::POST;
...
void
flagger2::setDataSelection(const ::casac::record& parameters)
{
	try
	{
		...
		if(flagger2_p)
		{
			return flagger2_p->setDataSelection(params);
		}
		//		return false;
	} catch (AipsError x) {
		*logger_p << LogIO::SEVERE << "Exception Reported: " << x.getMesg() << LogIO::POST;
