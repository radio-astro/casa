...
#    myTauZ_custom, myPWV1 = Tau_K_Calc(mydew,mytemp,thisday,weights=(1-seasonal_weight,seasonal_weight))
...
#    tau_allF_c = (pl.array(coef0) + pl.array(coef1)*pl.mean(myPWV_custom)) *1e-1  
    ...
    pl.savefig(myMS+'.plotWX.png',dpi=150)
    casalog.post('wrote weather figure: '+myMS+'.plotWX.png')
    casalog.post(' SPW : Freq (GHz) : Zenith opacity (nepers)')
    for i in range(len(meanTau)):
        myStr = str(i).rjust(3) + '  :   ' 
        myStr2 = '%.3f'%(spwFreqs[i])
        myStr += myStr2.rjust(7) + '  :  ' +str(round(0.01*meanTau[i], 4))
        casalog.post(myStr)
    pl.close()
    return meanTau
