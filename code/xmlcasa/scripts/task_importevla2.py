...
...."""
    ...
    # Vers9.0 (3.4.0) SMC 2012-02-27 ported to use the new flagger tool (testflagger)
    #
    ...
    #
    try:
        casalog.origin('importevla2')
        casalog.post('You are using importevla2 v9.0 SMC Updated 2012-02-27'
                     )
        viso = ''
...
#                unionpars = fh.getUnion(mslocal, viso, flaglist)
                ...
                if flagbackup:
                    fh.backupFlags(tflocal, 'importevla2')
        ...
        if savecmds:
            ...
            casalog.post('Will not save flag commands to file (savecmds=False')
    ...
    mslocal.writehistory(message='taskname   = importevla2',
                         origin='importevla2')
    ...
    if shadow:
        ...
        mslocal.writehistory(message='tolerance   = "' + str(tolerance)
                             + '"', origin='importevla2')
        mslocal.writehistory(message='recalcuvw   = "' + str(recalcuvw)
                             + '"', origin='importevla2')
        mslocal.writehistory(message='antennafile   = "'
                             + str(antennafile) + '"',
                             origin='importevla2')
    if applyflags:
...
def sortflags(
    ...
    # Now loop over flags, break into sorted and unsorted groups
        ...
        unsortd = {}
        unsortdlist = []
        if myflagsort == 'antenna' or myflagsort == 'antspw' \
            or myflagsort == 'antpol':
        ...
        # work out number of selections for this flag command
                ...
                if myd.has_key('timerange'):
                    if myd['timerange'] != '':
                        nselect += 1
                if myflagsort != 'antspw' and myflagsort != 'antpol' \
                    and myd.has_key('spw'):
                    if myd['spw'] != '':
                        nselect += 1
        ...
        # can only sort mode manualflag together
            ...
            # the antennas
                ...
                # for antenna specifications split on ,
                        antlist = ant.split(',')
            # the spw
                    if (myflagsort == 'antspw' or myflagsort == 'antpol'
                        ) and myd.has_key('spw'):
                        spwsort = True
                        spw = myd['spw']
                        spwlist = spw.split(',')
