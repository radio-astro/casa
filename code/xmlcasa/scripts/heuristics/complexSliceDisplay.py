"""Module to plot slices through complex 2-d data.
"""
...
# find max and min of data
        ...
        angle_datamax = None
        if len(good_data) > 0:
            amp_datamin = min(abs(good_data))
            amp_datamax = max(abs(good_data))
            angle_datamin = min(angle(good_data, deg=True))
            angle_datamax = max(angle(good_data, deg=True))
            good_x = compress(
            ...
            if xtitle == 'TIME':
                ...
                datemin = min(good_x)
                datemax = max(good_x)
            y1_ax.plot(good_x, abs(good_data), marker='o', markerfacecolor='blue',
             linestyle='None') 
            y2_ax.plot(good_x, angle(good_data, deg=True), marker='o', 
             markerfacecolor='green', linestyle='None') 

        pylab.title(sub_title, fontsize='small')
        if plot_number == 1:
            if dataUnits != '':
                y1_ax.set_ylabel('%s amplitude (%s)' % (dataType, dataUnits),
                 color='blue')
                y2_ax.set_ylabel('%s phase (deg)' % (dataType), color='green')
            else:
                y1_ax.set_ylabel('%s amplitude' % dataType, color='blue')
                y2_ax.set_ylabel('%s phase (deg)' % dataType, color='green')

# mae axis colors match the plotted markers

        for t1 in y1_ax.get_yticklabels():
            t1.set_color('blue')
        for t2 in y2_ax.get_yticklabels():
            t2.set_color('green')
...
# suggest that the y value is unimportant.
        ...
        ymin, ymax = y1_ax.get_ylim()
        yflag = ymin + (ymax - ymin) / 10.0
        amp_datamin = min(yflag, amp_datamin)
        ...
        if len(bad_data) > 0:
            ...
            y1_ax.plot(bad_x, abs(bad_data), marker='^', color='blue',
             linestyle='None')
...
# overplot 'edge' channels in grey
        ...
        if xtitle.upper() == 'CHANNEL' and noisy_channels != None:
            ...
            if len(edge_data) > 0:
                ...
                y1_ax.plot(edge_x, abs(edge_data), marker='o', 
                 markerfacecolor='grey', markeredgecolor='blue', linestyle='None') 
                y2_ax.plot(edge_x, angle(edge_data, deg=True), marker='o', 
                 markerfacecolor='grey', markeredgecolor='green',
                 linestyle='None') 
...
# flag baselines
                    ...
                    for xval in flag_x:
                        ...
                        yf = array(xf, float)
                        yf[:] = yflag
                        y1_ax.plot(xf, yf, marker='^', markerfacecolor=colour,
        ...
        if flagVersions.count('StageEntry') > 0:
            ...
            if len(bad_data) > 0:
                ...
                if xtitle == 'TIME':
                    ...
                    bad_x = temp
                bad_data[:] = yflag
                y1_ax.plot(bad_x, bad_data, marker='^', markerfacecolor='violet',
                 markeredgecolor='violet', linestyle='None')
        ...
        if flagVersions.count('BeforeHeuristics') > 0:
            ...
            if len(bad_data) > 0:
                ...
                if xtitle == 'TIME':
                    ...
                    bad_x = temp
                bad_data[:] = yflag
                y1_ax.plot(bad_x, bad_data, marker='^', markerfacecolor='green',
        ...
        if flagVersions.count('NoData') > 0:
            ...
            if len(bad_data) > 0:
                ...
                if xtitle == 'TIME':
                    ...
                    bad_x = temp
                bad_data[:] = yflag
                y1_ax.plot(bad_x, bad_data, marker='^', markerfacecolor='indigo',
                 markeredgecolor='indigo', linestyle='None')

# set plot y scale to include all 'good' points - hmm, works for both axes,

        if amp_datamin != None:
            room = (amp_datamax - amp_datamin) / 10.0
            y1_ax.set_ylim(amp_datamin - room, amp_datamax + room)
            room = (angle_datamax - angle_datamin) / 10.0
            y2_ax.set_ylim(angle_datamin - room, angle_datamax + room)
...
# confusion
        ...
        y1_ax.yaxis.set_major_formatter(ticker.ScalarFormatter(
         useOffset=False))
        y2_ax.yaxis.set_major_formatter(ticker.ScalarFormatter(
         useOffset=False))
...
# following will have problems at very end of month
            ...
            y1_ax.set_xlim(datemin, datemax)

# set the major ticks to lie each hour

            hours = mdates.HourLocator(interval=1)
            minutes = mdates.MinuteLocator(interval=10)
            y1_ax.xaxis.set_major_locator(hours)
            y1_ax.xaxis.set_major_formatter(mdates.DateFormatter('%Hh%Mm'))
            y1_ax.xaxis.set_minor_locator(minutes)

# override default x-axis title

            y1_ax.set_xlabel('Time (hours after MJD start)')
...
# noisy channels
        ...
        if noisyChannelsPlotted:
            pylab.plot(xoff, yoff, marker='o', markerfacecolor='grey')
...
# iterate through collected items and display them in turn
                ...
                for index, result in enumerate(stages):
                    ...
                    self._plot_panel(self._layout, nstages+1, index+1,
                     description, 
...
# write HTML info that goes after the displays
    ...
    def writeDetailedHTMLDescription(self, stageName):
        ...
        self._htmlLogger.logHTML('''<h3>Display</h3>
         ...
         each position plotting as a graph a cut along the y-axis. 
         When the x-axis is 'channels', the y-axis is scaled to include
         unflagged data points that are not designated as 'noisy'; this
         means that some 'noisy' channels may not be plotted.''')
