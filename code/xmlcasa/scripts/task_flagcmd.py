...
def flagcmd(
    ...
    if os.path.exists(vis + '/table.dat'):
        ...
        msopened = getmstimes(vis, ms_starttime, ms_endtime, t1sdata,
                              t2sdata)
        if msopened:
            ...
            casalog.post('I thought that vis is a MS, but cannot open it'
                         , 'WARN')
            datatype = -1
    ...
    if msopened or flagmode != 'table' and (optype == 'list' or optype
            == 'plot'):
        try:
            myflagcmd = {}
            cmdlist = []
            ...
            # read from FLAG_CMD table in msfile
                ...
                if (type(flagtable) == str) & os.path.exists(flagtable):
                    ...
                    raise Exception, \
                        'Flag command file not found - please verify the name'
            #
            # Parse file
                try:
        ...
        # Now have list of commands - do something with them
            ...
            # Apply flags to data using flagger
                nappl = applyflagcmd(fglocal, vis, flagbackup, myflagd,
                        reset)
            # Save flags to file
                if nappl > 0:
                    if outfile == '':
                ...
                # Save to ascii file
                        ...
                        try:
                            ...
                            raise Exception, \
                                'Error writing lines to file ' + outfile
                        ffout.close()
                else:
            ...
            # List flags
                listflags(myflagcmd, myoutfile=outfile,
                          listmode=listmode)
            elif optype == 'save':
            ...
            # Clear flag commands from FLAG_CMD in vis
                msfile = vis
                if clearall:
                    print 'Warning: deleting all rows from FLAG_CMD in MS ' \
                        + msfile
                    casalog.post('Warning: deleting all rows from FLAG_CMD in MS '
                                  + msfile)
        ...
        # write history
    if msopened and not (optype == 'list' or optype == 'plot' or optype
                         == 'extract'):
        try:
            ...
            mslocal.writehistory(message='flagmode = "' + str(flagmode)
                                 + '"', origin='flagcmd')
            if flagmode == 'file':
                mslocal.writehistory(message='flagfile = "'
                        + str(flagfile) + '"', origin='flagcmd')
            elif flagmode == 'cmd':
                for cmd in command:
                    mslocal.writehistory(message='command  = "'
                            + str(cmd) + '"', origin='flagcmd')
            mslocal.close()
        except:
...
def applyflagcmd(
    ...
    + ' has unknown key'
            ...
            for mode in modelist:
                ...
                if nf > 0:
                    ...
                    for s in param_set[mode].keys():
                        ...
                        if mode == 'shadow':
                            if debug:
                                print 'Applying shadow with params: ', \
                                    param_i
        ...
        # Were any valid flagging commands executed?
            ...
            if ncmd > 0:
                print 'Applied ' + str(ncmd) \
                    + ' total flagging commands'
                casalog.post('Applied ' + str(ncmd)
                             + ' total flagging commands')
            else:
...
def parse_cmd(cmd):
    ...
    for x in mycmd:
        ...
        if debug:
            print xkey + ' has value ' + str(xval)
        # check to see if this one is a repeat
        if params.has_key(xkey):
            # a repeat
            print str(x) \
                + ' has duplicate key, using only first instance'
        else:
...
def backup_cmdflags(fglocal, mode):
    ...
    fglocal.saveflagversion(versionname=versionname,
                            comment='flagcmd autosave before ' + mode
...
def writeflagcmd(msfile, myflags, tag=''):
    ...
    # Save to FLAG_CMD table
        ...
        casalog.post('Saving ' + str(nadd)
                     + ' flags to FLAG_CMD MS table')
...
def readflagcmd(
    ...
    if nrows > 0:
        ...
        casalog.post('Read ' + str(nflagd)
                     + ' rows from FLAG_CMD table in MS')
    else:
...
def clearflagcmd(msfile, myrowlist=[]):
    ...
    if nrows > 0:
        ...
        try:
            ...
            print 'Deleted ' + str(rowlist.__len__()) \
...
def newplotflags(
    ...
    print '  Found ' + str(nreas) + ' reasons to plot for ' \
        + str(nants) + ' antennas'
    npf = ipf
    ...
    if nreas > ncolors:
        for i in range(nreas):
            ...
            if i < ncolors - 1:
                ...
                offs = 0.29999999999999999 - float(i) * 0.6 \
                    / float(ncolors - 1)
                ...
                readict[reas]['offset'] = -0.29999999999999999
        reakeys.append('Other')
        readict['Other'] = {}
        readict['Other']['color'] = colorlist[ncolors - 1]
        readict['Other']['index'] = ncolors - 1
        readict['Other']['offset'] = -0.29999999999999999
    else:
        for i in range(nreas):
            reas = myreas[i]
            reakeys.append(reas)
            colr = colorlist[i]
            offs = 0.29999999999999999 - float(i) * 0.6 / float(ncolors
                    - 1)
            readict[reas] = {}
            readict[reas]['color'] = colr
            readict[reas]['index'] = i
