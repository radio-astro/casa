...
for source in scriptlist:
	execute_script='DO_'+source
	if (eval(execute_script)):
		print 'source ',source
		#os.environ['CASAPATH']=pathname
		execfile(str.lower(source)+'_regression.py')
		scriptpass=regstate
		scriptlog=outfile
		scripttime=(endTime-startTime)
		scriptrate=(ratedict.get(source))/(endTime-startTime)
#		print >>regfile,source+'\t'+str(scriptpass)+'\t'+str(scripttime)+'\t'+str(scriptrate)+'\t'+str(scriptlog)
		print >>regfile,'%9s %6s %9s %9s %39s'%(source,scriptpass,scripttime,scriptrate,scriptlog)
