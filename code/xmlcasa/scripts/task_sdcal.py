...
def sdcal(sdfile, antenna, fluxunit, telescopeparm, specunit, frame, doppler, calmode, scanlist, field, iflist, pollist, channelrange, average, scanaverage, timeaverage, tweight, averageall, polaverage, pweight, tau, kernel, kwidth, masklist, maskmode, thresh, avg_limit, edge, blfunc, order, npiece, clipthresh, clipniter, verifycal, verifysm, verifybl, verbosebl, outfile, outform, overwrite, plotlevel):
...
#              sdbaseline(sdfile=tmpsdfile, maskmode=maskmode,order=order,interactive=interactive,masklist=masklist, thresh=thresh, avg_limit=avg_limit, edge=edge, outfile=tmpoutfile, outform=outform, overwrite=True, plotlevel=plotlevel)
              sdbaseline(sdfile=tmpsdfile,antenna=antenna,masklist=masklist,maskmode=maskmode,thresh=thresh,avg_limit=avg_limit,edge=edge,blfunc=blfunc,order=order,npiece=npiece,clipthresh=clipthresh,clipniter=clipniter,verify=verifybl,verbose=verbosebl,outfile=tmpoutfile,outform=outform,overwrite=True,plotlevel=plotlevel)
            else:
...
#        arg_names=['sdfile','fluxunit','telescopeparm','specunit','frame','doppler','calmode','scanlist','field','iflist','pollist', 'scanaverage','timeaverage','tweight', 'polaverage','pweight', 'kernel','kwidth','tau','maskmode','order','interactive','masklist','outfile','outform','overwrite','plotlevel','thresh','avg_limit','edge']
        arg_names=['sdfile','fluxunit','telescopeparm','specunit','frame','doppler','calmode','scanlist','field','iflist','pollist', 'scanaverage','timeaverage','tweight', 'polaverage','pweight', 'kernel','kwidth','tau','masklist','maskmode','thresh','avg_limit','edge','blfunc','order','npiece','clipthresh','clipniter','verify','verbose','outfile','outform','overwrite','plotlevel']
        a=inspect.stack()
        stacklevel=0
        for k in range(len(a)):
          if (string.find(a[k][1], 'ipython console') > 0):
