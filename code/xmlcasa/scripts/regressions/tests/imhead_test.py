...
# publish_summary.runTest( 'imhead_test' )
#
...
#
# cp -r CASA_REPOS/data/regression/ngc5921redux/reference/ngc5921_regression/ngc5921.clean.image WORKING_DIR
# cp CASA_REPOS/code/xmmlcasa/scripts/regression/tests/imhead_test.py WORKING_DIR
# casapy
# CASA <2>: cd WORKING_DIR
# CASA <3>: import imhead_test
# CASA <4>: imhead_test.run()
# </example>
#
# <motivation>
...
def values_restored( input_file='', input_file_copy='' ):
    retValue = {'success': True, 'msgs': "", 'error_msgs': '' }    
...
def description():
    return " Tests the correctness of the ihead task in CASA which includes:"\
           + "\n\n"\
           +""
...
def run():
    ...
    test_image_filename = 'ngc5921.clean.image'
    test_image_copy     = 'ngc5921.clean.image.copy'

    # First lets copy the file so we can see if we've restored it
    # back to its original form at the end of the tests.
    shutil.copytree( test_image_filename, test_image_copy )
    ...
    error_msgs = ""
    testResults=[]
    testResults.append( mode_test( test_image_filename ) )
    testResults.append( set_get_test( test_image_filename ) )
    testResults.append( add_del_test( test_image_filename ) )
    testResults.append( values_restored( test_image_filename, test_image_copy ) )
    ...
    if ( passed==False ):
        ...
        raise Exception, 'imhead test has failed!\n'+error_msgs
