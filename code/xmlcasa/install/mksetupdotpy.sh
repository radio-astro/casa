...
VARS="CPPSTD PYTHONLIBD PYTHONVER CCMTOOLSLIBD CCMTOOLSINCD CORELIBD COREINCD WCSLIBLIBD WCSLIBINCD QT4LIBD QT4LIB CFITSIOLIBD CFITSIOINCD XTRNLIBS_rpath"
...
echo "      ext_modules=[Extension(\"casac\"," >> setup.py
echo "               [" >>setup.py
...
if [ "$SETUPLIBS" != "" ]; then
   if [ "$QT4LIBD" != "" ]; then
      echo "               library_dirs=['$ARCHLIB', $SETUPLIBS, '$PYTHONLIBD' $CFITSIO_LIBPATH, '$WCSLIBLIBD', '$QT4LIBD']," >> setup.py
   else
      echo "               library_dirs=['$ARCHLIB', $SETUPLIBS, '$PYTHONLIBD' $CFITSIO_LIBPATH, '$WCSLIBLIBD']," >> setup.py
   fi
else
   if [ "$QT4LIBD" != "" ]; then
      echo "               library_dirs=['$ARCHLIB', '$PYTHONLIBD' $CFITSIO_LIBPATH, '$WCSLIBLIBD', '$QT4LIBD']," >> setup.py
   else
      echo "               library_dirs=['$ARCHLIB', '$PYTHONLIBD' $CFITSIO_LIBPATH, '$WCSLIBLIBD']," >> setup.py
   fi
fi
...
if [ "$tjunk" ]; then
   echo "                           'casa_scimath_f', 'casa_mirlib', 'cfitsio', 'wcs', " >> setup.py
else
   echo "                           'casa_scimath_f', 'lapack', 'blas', 'casa_mirlib', 'cfitsio', 'wcs', " >> setup.py
   if [ "$QT4LIB" != "" ]; then
   qtlibs=''
...
echo "                                     ]," >> setup.py
#_echo "                                     '-DAIPS_NO_TEMPLATE_SRC']," >> setup.py
if [ "$SETUPEXTRALINK" != "" ]; then
echo "               extra_link_args = [$SETUPEXTRALINK])], " >> setup.py
fi
echo "       include_dirs = ['$AIPSROOT/code/include', '$AIPSROOT/code/casa', '$COREINCD', '$WCSLIBINCD', '.', '..', '$CCMTOOLSINCD', '$CFITSIOINCD', '../impl'])" >> setup.py
