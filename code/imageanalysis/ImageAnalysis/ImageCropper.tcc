...
ImageCropper<T>::ImageCropper(
	const CountedPtr<const ImageInterface<T> > image,
...
template <class T>
CountedPtr<ImageInterface<T> > ImageCropper<T>::crop(
	...
	*this->_getLog() << LogOrigin(_class, __FUNCTION__, WHERE);
	...
	CountedPtr<ImageInterface<T> > outImage( this->_prepareOutputImage(cropped));
	if (! wantReturn) {
		outImage.reset();
	}
	return outImage;
