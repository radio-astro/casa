...
int main() {
	...
	try {
		...
		CountedPtr<casa::ImageInterface<float> > imgPtr( fits );
		ImageAnalysis ia(imgPtr );
        {
            // CAS-2533
            PagedImage<Float>* img = new PagedImage<Float>(datadir + "/CAS-2533.im");
            CountedPtr<casa::ImageInterface<float> > imgPtr2( img );
        ...
        }
        {
            // wavelength output
            PagedImage<Float>* img = new PagedImage<Float>(datadir + "/CAS-2533.im");
            CountedPtr<casa::ImageInterface<float> > imgPtr( img );
        ...
        }
        {
            ...
            CountedPtr<casa::ImageInterface<float> > imgPtr( image );
            ImageAnalysis ia( imgPtr);
            Record regionRec;
            String mask;
            Vector<Int> axes(0);
...

        {
        	...
        	data.set(0);
        	CountedPtr<casa::ImageInterface<float> > imgPtr( x );
        	ImageAnalysis ia(imgPtr);
        	Vector<Double> xy;
