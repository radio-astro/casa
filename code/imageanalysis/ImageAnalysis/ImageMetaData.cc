...
String ImageMetaData::_getBrightnessUnit() const {
	if (_bunit.empty()) {
		CountedPtr<const ImageInterface<Float> > imf = _getFloatImage();
		CountedPtr<const ImageInterface<Complex> > imc = _getComplexImage();
