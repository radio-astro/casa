...
void ImageFitter::_fitLoop(
	...
	CountedPtr<ArrayLattice<Bool> > initMask;
	CountedPtr<TempImage<Float> > tImage;
...
void ImageFitter::_doConverged(
	...
	CountedPtr<TempImage<Float> > fittedResid;
	if (outputImages) {
		if (hasSpectralAxis) {
			location[spectralAxisNumber] = _curChan - _chanVec[0];
		}
...
void ImageFitter::_setDeconvolvedSizes() {
	...
	for (uInt i=0; i<n; i++) {
		...
		CountedPtr<GaussianShape> gaussShape(
			static_cast<GaussianShape *>(
				_curConvolvedList.getShape(i)->clone()
			)
		);
		CountedPtr<PointShape> point;
