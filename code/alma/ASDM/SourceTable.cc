...
namespace asdm {
  ...
  SourceRow* SourceTable::add(SourceRow* x) {
    ...
    if (context.find(k) != context.end() && insertionId < context[k].size()) {

      for (unsigned int j = 0; j < context[k][insertionId].size(); j++){
	if (context[k][insertionId][j]->getTimeInterval().getStart().equals(startTime)) {
	  if(
    ...
    if (insertionId >=  context[k].size()) context[k].resize(insertionId+1);
