...
namespace asdm {
	...
	const vector<string> BeamTable::attributesNames = initAttributesNames();
 ...
 */
	 ...
	 */
	vector<string> BeamTable::initAttributesNames() {
	 ...
	 */
	const vector<string>& BeamTable::getAttributesNames() { return attributesNames; }
 ...
 */
	...
	string BeamTable::toXML()  {
		...
		buf.append("<BeamTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:beam=\"http://Alma/XASDM/BeamTable\" xsi:schemaLocation=\"http://Alma/XASDM/BeamTable http://almaobservatory.org/XML/XASDM/2/BeamTable.xsd\" schemaVersion=\"2\" schemaRevision=\"1.54\">\n");
	...
	string BeamTable::MIMEXMLPart(const asdm::ByteOrder* byteOrder) {
		...
		oss << "\n";
		oss << "<BeamTable xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:beam=\"http://Alma/XASDM/BeamTable\" xsi:schemaLocation=\"http://Alma/XASDM/BeamTable http://almaobservatory.org/XML/XASDM/2/BeamTable.xsd\" schemaVersion=\"2\" schemaRevision=\"1.54\">\n";
	...
	string BeamTable::toMIME(const asdm::ByteOrder* byteOrder) {
    ...
    if ( loc0 == string::npos) {
      throw ConversionException("Failed to detect the beginning of the XML header", "Beam");
    }
    ...
    if ( doc == NULL ) 
      throw ConversionException("Failed to parse the xmlHeader into a DOM structure.", "Beam");
   ...
   //
    ...
    attributesSeq.push_back("beamId") ; 
    ...
    else if (string("BeamTable").compare((const char*) root_element->name) == 0) {
      ...
      if ( bulkStoreRef == 0 || (bulkStoreRef->type != XML_ELEMENT_NODE)  || (string("BulkStoreRef").compare((const char*) bulkStoreRef->name) != 0))
      	throw ConversionException ("Could not find the element '/BeamTable/BulkStoreRef'. Invalid XML header '"+ xmlHeader + "'.", "Beam");
      ...
      if (byteOrderAttr == 0) 
	     throw ConversionException("Could not find the element '/BeamTable/BulkStoreRef/@byteOrder'. Invalid XML header '" + xmlHeader +"'.", "Beam");
      
      string byteOrderValue = string((const char*) byteOrderAttr->children->content);
      if (!(byteOrder = asdm::ByteOrder::fromString(byteOrderValue)))
		throw ConversionException("No valid value retrieved for the element '/BeamTable/BulkStoreRef/@byteOrder'. Invalid XML header '" + xmlHeader + "'.", "Beam");
     ...
     if ( attributes == 0 || (attributes->type != XML_ELEMENT_NODE)  || (string("Attributes").compare((const char*) attributes->name) != 0))	 
       	throw ConversionException ("Could not find the element '/BeamTable/Attributes'. Invalid XML header '"+ xmlHeader + "'.", "Beam");
    ...
    if (boost::filesystem::exists(boost::filesystem::path(uniqSlashes(directory + "/Beam.xml"))))
      setFromXMLFile(directory);
    else if (boost::filesystem::exists(boost::filesystem::path(uniqSlashes(directory + "/Beam.bin"))))
      setFromMIMEFile(directory);
    else
      throw ConversionException("No file found for the Beam table", "Beam");
	}			

	
  void BeamTable::setFromMIMEFile(const string& directory) {
    string tablePath ;
    
    tablePath = directory + "/Beam.bin";
    ifstream tablefile(tablePath.c_str(), ios::in|ios::binary);
    if (!tablefile.is_open()) { 
      throw ConversionException("Could not open file " + tablePath, "Beam");
    }
    // Read in a stringstream.
    stringstream ss; ss << tablefile.rdbuf();
    
    if (tablefile.rdstate() == istream::failbit || tablefile.rdstate() == istream::badbit) {
      throw ConversionException("Error reading file " + tablePath,"Beam");
    }
    
    // And close.
    tablefile.close();
    if (tablefile.rdstate() == istream::failbit)
      throw ConversionException("Could not close file " + tablePath,"Beam");
    
    setFromMIME(ss.str());
  }	

	
void BeamTable::setFromXMLFile(const string& directory) {
    string tablePath ;
    
    tablePath = directory + "/Beam.xml";
    ifstream tablefile(tablePath.c_str(), ios::in|ios::binary);
    if (!tablefile.is_open()) { 
      throw ConversionException("Could not open file " + tablePath, "Beam");
    }
