...
void PlotMSCacheIndexer::reportMeta(Double x, Double y,stringstream& ss) {
  ...
  if (netAxesMask_(1)) {
    ...
    /*
     // disabled until averaging_ available/avoidable in PlotMSCacheIndexer
    if (averaging_.channel() && averaging_.channelValue()>1) {
      Int lochan=Int(plotmscache_->getChan(currChunk_,irel_));
    ...
    */ 
    ss << Int(plotmscache_->getChan(currChunk_,getIndex0100(currChunk_,irel_))) << " ";
  }
  else
    ss << "*  ";
  ...
  ss << "Freq=";
  if (netAxesMask_(1))
    ss << plotmscache_->getFreq(currChunk_,getIndex0100(currChunk_,irel_)) << " ";
  else
    ss << "*        ";
