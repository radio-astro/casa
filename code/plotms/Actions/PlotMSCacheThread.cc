...
PlotMSCacheThread::PlotMSCacheThread(PlotMSPlot* plot, PlotMSCacheBase* cache,
  ...
  //  cout << "FILLING VIA **PLOTMSCACHEBASE** ***********************" << endl;
...
void PlotMSCacheThreadHelper::run() {
	...
	if(itsParent_.itsSetupPlot_) {
	  if (itsParent_.itsCache_)
	    itsParent_.itsCache_->setUpIndexer(PMS::NONE);
	  else
