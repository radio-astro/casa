...
void PlotMSVBAverager::finalizeAverage() {
    ...
    if(nBln > 0 && vbWtSum_p > 0.0) {
            ...
            if(blnOK_p(ibln)) {
                for(Int ichn = 0; ichn < nChan_p; ++ichn) {
                    for(Int icor = 0; icor < nCorr_p; ++icor) {
                        ...
                        if(doWC_p && thiswt>0.0) {
                            if(doVC_p)
                                avBuf_p.visCube()(icor, ichn, obln) =
                                    avBuf_p.visCube()(icor, ichn, ibln) /
                                    thiswt;
                            if(doMVC_p)
                                avBuf_p.modelVisCube()(icor, ichn, obln) =
                                    avBuf_p.modelVisCube()(icor, ichn, ibln) /
                                    thiswt;
                            if(doCVC_p)
                                avBuf_p.correctedVisCube()(icor, ichn, obln) =
                                    avBuf_p.correctedVisCube()(
                                        icor, ichn, ibln) / thiswt;
                        } // !flagCube & wt>0
                        else {
                            // make sure obln is zero
                            if(obln < ibln) {
                                if(doVC_p)
                                    avBuf_p.visCube()(icor, ichn, obln) = 0.0;
                                if(doMVC_p)
                                    avBuf_p.modelVisCube()(icor, ichn, obln) =
                                        0.0;
                                if(doCVC_p)
                        ...
                        if(obln < ibln) {
                            avBuf_p.flagCube()(icor, ichn, obln) =
                                avBuf_p.flagCube()(icor, ichn, ibln);
                            if(doWC_p)
                                avBuf_p.weightCube()(icor, ichn, obln) =
                                    thiswt;
                        }
                    } // icor
                } // ichn
        ...
        if(nBln < nBlnMax_p) {
            ...
            if(doCVC_p)
                avBuf_p.correctedVisCube().resize(nCorr_p, nChan_p, nBln,
                                                  True);
            if(doWC_p)
        ...
        // must be center of the interval!
        avBuf_p.time() = timeRef_p + (maxTime_p + minTime_p) / 2.0;
        ...
        for(Int chn = 0; chn < vb.nChannel(); chn++) {
            for(Int cor = 0; cor < nCorr_p; ++cor, ++wt) {
                ...
                if(!vb.flagCube()(cor, chn, ibln)) { // input UNflagged
                    ...
                    // input cell is flagged
                    // Only accumulate if output is also flagged
                    //  (yields average of flagged data if no unflagged
                    //  data ever found)
...
//    vb               const VisBuffer&     VisBuffer to accumulate
    ...
    for(Int ibln = 0; ibln < vb.nRow(); ++ibln) {
        ...
        // These antennas occur (even if flagged) in input,
        //  so preserve in output
        blnOK_p(obln_i) = True;
        blnOK_p(obln_j) = True;
        ...
        for(Int chn = 0; chn < vb.nChannel(); chn++) {
            for(Int cor = 0; cor < nCorr_p; ++cor, ++wt) {
                ...
                if(acc_j) {
                    ...
                    // NB: wt is implicitly indexed by cor,
                    //  so index incoming data w/ cor
                    ...
                    if(doCVC_p)
                        avBuf_p.correctedVisCube()(jcor, chn, obln_j) +=
                            (*wt)*conj(vb.correctedVisCube()(cor, chn, ibln));
                    if(doWC_p) avBuf_p.weightCube()(jcor, chn, obln_j) +=
                        (*wt);
                }
    ...
    if(antAve_p) {
        ...
        for(uInt icor = 0; icor < jcor_p.nelements(); ++icor) {
            if(corrs(icor)==Stokes::RL ||
               ...
               corrs(icor)==Stokes::YX)
                chids(ich++) = icor;
        }

        if(chids(0) > -1) {  // at least one ch detected
            if(chids(1) < 0) // second ch not detected, so abort
                throw(AipsError("Both cross-hands (or none) must be selected "
                                "and present when Per Antenna averaging is "
                                "selected."));
            else { // Both cross-hands detected
                // Cross-hand difference must be 1
                //  (indicates basis consistency)
                Int chdiff = abs(corrs(chids(1)) - corrs(chids(0)));
                if(chdiff != 1)
                    throw(AipsError("Cross-hand basis inconsistency "
                                    "detected."));
