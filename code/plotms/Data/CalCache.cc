...
void CalCache::setUpCalIter(const String& ctname,
    ...
    // TBD: is this ever needed?  (NCTI doesn't yet support it)
...
void CalCache::loadCalChunks(ROCTIter& ci,
    ...
    while(!ci.pastEnd()) {
        ...
        cout << " SPECTRAL_WINDOW? = " << boolalpha
             << thisct.keywordSet().isDefined("SPECTRAL_WINDOW") << endl;
...
void CalCache::loadCalAxis(ROCTIter& cti, Int chunk, PMS::Axis axis) {
    ...
    */
    ...
    case PMS::PARANG:
        // in degrees
        *parang_[chunk] = cti.feed_pa(cti.time()(0)) * (180.0 / C::pi);
        break;
    case PMS::ROW: {
        ...
        if(ichk != flchunks(order[iflag]) &&
           !ci_p->pastEnd()) {
            ...
            if(True) {
                ...
                ss << "Time diff:  " << time - cttime << " " << time
                   << " " << cttime << "\n";
                ss << "Spw diff:   " << spw - ctspw   << " " << spw
                   << " " << ctspw  << "\n";
                ss << "Field diff: " << field - ctfld << " " << field
                   << " " << ctfld  << "\n";
            }
            ...
            while(ifl < Int(nflag) && flchunks(order[ifl]) == ichk) {
                ...
                if(netAxesMask_(2) &&
                   ...
                   thisA1 > -1) {
                    ...
                    //   full baseline extension is OFF and the first antenna
                    //   in the selected point is > -1
                        ...
                        if(thisA2 > -1) {
                            ...
                            if(a1(irow) == thisA1 &&
                               a2(irow) == thisA2) {
                                ...
                                // found the one baseline, escape from for loop
                                break;
                            }
                        } else {
                    ...
                    //  is ordinarily implicit in baseline, we've turned on
                    //  baseline extension, or we've avaraged over all
                    //  baselines
                    bsln = Slice(0, nrow, 1);
