...
void PlotMSCacheBase::load(const vector<PMS::Axis>& axes,
    ...
    // Trap (currently) unsupported modes
    {
        ...
        if(nAM(2) && nAM(3))
            throw(AipsError("Plots of antenna-based vs. baseline-based "
                            "axes not supported (" + PMS::axis(currentX_) +
                            " and " + PMS::axis(currentY_)));

        // Can't plot averaged weights yet
        if(averaging_.anyAveraging()) {
            if(axes[0] == (PMS::WT) |
                axes[1] == (PMS::WT)) {
                throw(AipsError("Sorry, the Wt axes options do not yet "
                                "support averaging."));
            }
        }
    }
...
/*
    cout << VAR.size() << " " << j << " " << PMS::axis(axes[i]) << " "
         << VAR[j]->nrefs() << " " << VAR[j] << endl;
*/
...
void PlotMSCacheBase::release(const vector<PMS::Axis>& axes) {
...

    {
        for(unsigned int i = 0; i < axes.size(); i++) {
            ...
            if(dataLoaded_ && (currentX_ == axes[i] || currentY_ == axes[i])) {
                dataLoaded_ = false;
            }
        }
    }
    //  uInt postmem = HostInfo::memoryFree();
    //  cout << "memoryFree = " << premem << " " << postmem << " "
    //       << premem-postmem << endl;
...
void PlotMSCacheBase::setUpIndexer(PMS::Axis iteraxis, Bool globalXRange,
                                   Bool globalYRange) {
    logLoad("Setting up iteration indexing (if necessary), "
            "and calculating plot ranges.");

    //  cout << "############ PlotMSCacheBase::setUpIndexer: "
    //       << PMS::axis(iteraxis)
    ...
    case PMS::BASELINE: {
        ...
        */
        ...
        //  cout << "nIter = " << nIter << " iterValues = " << iterValues
        //       << endl;

        break;
    }
    case PMS::ANTENNA: {
        // Escape if(full) baseline averaging is on,
        // since we won't find any iterations
        if(averaging_.baseline())
            throw(AipsError("Iteration over baseline not supported "
                            "with full baseline averaging."));
    ...
    if(iteraxis!=PMS::NONE) {
        stringstream ss;
        ss << "Found " << nIter << " " << PMS::axis(iteraxis)
           << " iterations.";
        logLoad(ss.str());
    }

    //  cout << "********nIter = " << nIter << " iterValues = "
    //       << iterValues(IPosition(1, 0), IPosition(1, nIter - 1)) << endl;

    deleteIndexer();
    indexer_.resize(nIter);
    indexer_.set(NULL);
    for(Int iter = 0; iter < nIter; ++iter)
        indexer_[iter] = new PlotMSIndexer(this, currentX_, currentY_,
                                           iteraxis, iterValues(iter));
...

    {
        ...
        logLoad(ss.str());
        //  cout << "Use global ranges? : " << boolalpha << globalXRange << " "
        //       << globalYRange << endl;
    }
}
...
void PlotMSCacheBase::setPlotMask(Int chunk) {
    ...
    if(netAxesMask_(3) && !netAxesMask_(2)) {
        ...
        plmask_[chunk]->resize(nsh);
        (*plmask_[chunk]) = operator>(
            partialNFalse(*flag_[chunk], csh).reform(nsh), uInt(0));
    }
}
...
void PlotMSCacheBase::log(const String& method, const String& message,
			  int eventType) {
    plotms_->getLogger()->postMessage(PMS::LOG_ORIGIN,
                                      method, message, eventType);}
}
