                     ...
                     PlotMSCacheThread* thread) {
    {
        ...
        if(!corcolOk) {
            for(Int i = 0; i < loadData.size(); ++i) {
                ...
                case PMS::CORRECTED:
                case PMS::RESIDUAL: {
                    throw(AipsError("CORRECTED_DATA not present, "
                                    "please use DATA"));
                    break;
                }
                default:
...
void MSCache::setUpVisIter(const String& msname,
    ...
    // force scan boundaries
    if(!combscan) columns[i++] = MS::SCAN_NUMBER;
    // force field boundaries
    if(!combfld) columns[i++] = MS::FIELD_ID;
    // force spw boundaries
    if(!combspw) columns[i++] = MS::DATA_DESC_ID;
    columns[i++] = MS::TIME;
    // force per-timestamp chunks
    if(combspw || combfld) iterInterval = DBL_MIN;
    // effectively ignore field boundaries
    if(combfld) columns[i++] = MS::FIELD_ID;
    // effectively ignore spw boundaries
    if(combspw) columns[i++] = MS::DATA_DESC_ID;
...
void MSCache::countChunks(ROVisibilityIterator& vi, Vector<Int>& nIterPerAve,
    ...
    for(vi.originChunks(); vi.moreChunks(); vi.nextChunk(),chunk++) {
        ...
        for(vi.origin(); vi.more(); vi++, iter++) {
            ...
            if(// not combining field nor spw, OR
               //(!combfld && !combspw) ||
               // (combining field and/or spw) and solint exceeded, OR
               ((time1 - avetime1) > interval) ||
               // a negative time step occurs, OR
               ((time1 - avetime1) < 0.0) ||
               // not combining scans, and new scan encountered OR
               (!combscan && (thisscan != lastscan)) ||
               // not combining spws, and new spw encountered  OR
               (!combspw && (thisspw != lastspw)) ||
               // not combining fields, and new field encountered OR
...
void MSCache::loadAxis(VisBuffer& vb, Int vbnum, PMS::Axis axis,
        ...
        case PMS::DATA: {
            *amp_[vbnum] = amplitude(vb.visCube());
            ...
            if (False) {
                ...
                Slicer sl(Slice(), Slice(nch * (fact - 1) / 2, nch, 1),
                          Slice());
                ...
                cout << "tf.isWritable() = " << boolalpha << tf.isWritable()
                     << endl;
    ...
    case PMS::PHASE: {
        ...
        case PMS::RESIDUAL: {
            *pha_[vbnum] = phase(vb.correctedVisCube() -
                                 vb.modelVisCube()) * (180.0 / C::pi);
            break;
        }
        }
        break;
            ...
            if(ichk != flchunks(order[iflag])) {
                ...
                for(Int i = 0; i < nVBPerAve_(ichk); ++i) {
                    ...
                    /*{
                        ...
                        ss << "Time diff: " << time - vb.time()(0)
                           << " "  << time << " " << vb.time()(0) << "\n";
                        ss << "Spw diff:  " << spw - vb.spectralWindow()
                           << " " << spw << " " << vb.spectralWindow() << "\n";
                        ss << "Field diff:  " << field - vb.fieldId()
                           << " " << field << " " << vb.fieldId() << "\n";
                    }*/
                    ...
                    while(ifl < Int(nflag) && flchunks(order[ifl]) == ichk) {
                        ...
                        if(netAxesMask_(1) && !flagging.channel()) {
                            ...
                            if(averaging_.channel() &&
                               averaging_.channelValue() > 0) {
                                Int start = chanAveBounds_p(
                                    vb.spectralWindow())(ichan, 2);
                                Int n = chanAveBounds_p(
                                    vb.spectralWindow())(ichan, 3) - start + 1;
                        ...
                        Int thisA1 = Int(
                            getAnt1(currChunk, indexer->getIndex0010(
                                        currChunk, irel)));
                        Int thisA2 = Int(
                            getAnt2(currChunk, indexer->getIndex0010(
                                        currChunk, irel)));
                        if(netAxesMask_(2) &&
                           !flagging.antennaBaselinesBased() &&
                           thisA1 > -1) {
                            // i.e., if baseline is an explicit data axis,
                            //  full baseline extension is OFF and the first
                            //  antenna in the selected point is > -1

                            // Do some variety of detailed per-baseline
                            //  flagging
                                ...
                                if(thisA2 > -1) {
                                    ...
                                    if(a1(irow) == thisA1 &&
                                       a2(irow) == thisA2) {
                                        ...
                                        if(!flag) vbflagrow(irow) = False;
                                        // found the one baseline, escape
                                        //  from for loop
                                        break;
                                    }
                                } else {
                                    // either antenna matches the one specified
                                    //  antenna (don't break because there will
                                    //  be more than one)
                                    // TBD: this doesn't get cross-hands quite
                                    // right when averaging 'by antenna'...
                                    if(a1(irow) == thisA1 ||
                                       a2(irow) == thisA1) {
                                        vbflag(corr, chan,
                                               Slice(irow, 1, 1)) = flag;
                            ...
                            //  is ordinarily implicit in baseline, we've
                            //  turned on baseline extension, or we've avaraged
                            //  over all baselines
                            bsln = Slice(0, nrow, 1);
                            vbflag(corr, chan, bsln) = flag;
                            // unset flag_row when unflagging
                            if(!flag) vbflagrow(bsln) = False;
                        }
