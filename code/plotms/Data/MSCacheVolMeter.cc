...
MSCacheVolMeter::MSCacheVolMeter(const MeasurementSet& ms,
    ...
    for(Int iddid = 0; iddid < nDDID_; ++iddid) {
        ...
        Int nchanA = (
            chave ? Int(ceil(Double(nchan0) / ave.channelValue())) : nchan0);
        ...
        if(chansel.nelements() > 0 && chansel(ispw).nelements() > 0) {
            ...
            nchanA = Int(
                ceil(Double(maxchan - minchan + 1) / ave.channelValue()));
        }
...
String MSCacheVolMeter::evalVolume(map<PMS::Axis,Bool> axes,
    ...
    cout << "Sum     = " << sum(nRowsPerDDID_*nChanPerDDID_*nCorrPerDDID_)
         << endl;
    ...
    */
    ...
    for(map<PMS::Axis,Bool>::iterator pAi = axes.begin();
        ...
        if(pAi->second) {
            ...
            case PMS::REAL:
            case PMS::IMAG:
                axisVol = sizeof(Float) *
                    sum(nRowsPerDDID_ * nChanPerDDID_ * nCorrPerDDID_);
                break;
            case PMS::FLAG:
                axisVol = sizeof(Bool) *
                    sum(nRowsPerDDID_ * nChanPerDDID_ * nCorrPerDDID_);
            ...
            totalVol += axisVol;
            //  cout << " " << PMS::axis(pAi->first) << " volume = "
            //       << axisVol << " bytes." << endl;
        }
    } // for
    ...
    if(True) {  // ntrue(axesmask)<2) {
        ...
        //  cout << " Collapsed flag (plot mask) volume = " << plmaskVol
        //       << " bytes." << endl;
        totalVol += plmaskVol;
    }
    ...
    String arcpmsif("");
    Bool ignoreFree = (Aipsrc::find(arcpmsif,"plotms.ignorefree") &&
                       arcpmsif == "T");
    ...
    /*
    cout << "HostInfo::memoryTotal(false) = " << HostInfo::memoryTotal(false)
         << endl;
    ...
    */
    ...
    // Memory available to plotms is the min of user's casarc and free (in GB)
    Double hostMemGB = Double(min(hostMemTotalKB, hostMemFreeKB)) / 1.0e6;
    ...
    if(ignoreFree)
        ss << "Use of 'plotms.ignorefree: T' in the .casarc file may cause"
           << endl << "your machine to swap for very large plots." << endl;
    ...
    ss << endl<< "The plotms cache will require an estimated "
       << totalVolGB << " GB of memory (" << bytesPerPt << " bytes/point)."
       << endl << "This is " << fracMem << "% of the memory avail. to CASA ("
    ...
    if(totalPoints > UINT_MAX) {
        ss << endl << "Too many points!  CASA plotms cannot plot more than "
           << UINT_MAX << " points.";
        toomany = True;
    }
