...
double PlotMSIndexer::xAt(unsigned int i) const {
    ...
    return (plotmscache_->*getXFromCache_)(
        currChunk_, (self->*XIndexer_)(currChunk_, irel_));
}

double PlotMSIndexer::yAt(unsigned int i) const {
    setChunk(i);  // sets chunk and relative index in chunk
    return (plotmscache_->*getYFromCache_)(
        currChunk_, (self->*YIndexer_)(currChunk_, irel_));
}

void PlotMSIndexer::xAndYAt(unsigned int index,
			    double& x, double& y) const {
    setChunk(index);  // sets chunk and relative index in chunk
    x = (plotmscache_->*getXFromCache_)(
        currChunk_, (self->*XIndexer_)(currChunk_, irel_));
    y = (plotmscache_->*getYFromCache_)(
        currChunk_, (self->*YIndexer_)(currChunk_, irel_));
}
...
bool PlotMSIndexer::maskedMinsMaxes(double& xMin, double& xMax,
    ...
    //  cout << "Using masked range: " << xMin << " " << xMax << " "
    //       << yMin << " " << yMax << endl;
...
bool PlotMSIndexer::unmaskedMinsMaxes(double& xMin, double& xMax,
    ...
    //  cout << "Using unmasked range: " << xMin << " " << xMax << " "
    //       << yMin << " " << yMax << endl;
...
unsigned int PlotMSIndexer::binAt(unsigned int i) const {
    ...
    if(itsColorize_) {
        setChunk(i);
        unsigned int val = (unsigned int)(plotmscache_->*getColFromCache_)(
            currChunk_, (self->*ColIndexer_)(currChunk_, irel_));
        return val % numBins();
    }
    return 0;
}
...
bool PlotMSIndexer::colorize(bool doColorize, PMS::Axis colorizeAxis) {
    ...
    //  cout << "COLORIZE!! " << boolalpha << itsColorize_ << " "
    //       << PMS::axis(itsColorizeAxis_) << endl;
...
void PlotMSIndexer::setUpIndexing() {
    ...
    if (plotmscache_->netAxesMask_(2) && plotmscache_->netAxesMask_(3))
        throw(AipsError("Cannot yet support antenna-based and baseline-based "
                        "data in same plot."));
    ...
    */
    ...
    //  cout << "*>*>*>*>*>*> iterate_=" << boolalpha << iterate_
    //       << " Axis: " << PMS::axis(iterAxis_) << " iterValue_ = "
    //       << iterValue_ << endl;
        ...
        for(Int ich = 0; ich < nChunk(); ++ich)
            ...
            if(plotmscache_->goodChunk(ich)) {
                for(Int ibl = 0; ibl < chsh(2, ich); ++ibl) {
                    Int a1val = *(plotmscache_->antenna1_[ich]->data() + ibl);
                    Int a2val = *(plotmscache_->antenna2_[ich]->data() + ibl);
                    if((a1val == iterValue_) || (a2val == iterValue_)) {
                        ++nSegment_;
                    }
                }
            }
    ...
    //  cout << "nSegment_ = " << nSegment_ << " (nChunk=" << nChunk()
    //       << ")" << endl;
    ...
    for(Int ic = 0; ic < nChunk(); ++ic) {
        ...
        //  cout << "plotmscache_->plmask_[ic]->shape() = "
        //       << plotmscache_->plmask_[ic]->shape() << endl;
        switch(iterAxis_) {
        case PMS::NONE:
        case PMS::FIELD:
        case PMS::SPW:
        ...
        case PMS::BASELINE: {
            ...
            while(ibsln < nBsln &&
                  *(plotmscache_->baseline_[ic]->data() + ibsln) != iterValue_)
                ++ibsln;
            if(ibsln < nBsln) {
                // Found the baseline in this chunk
                ++iseg;
                cacheChunk_(iseg) = ic;
                // non-zero only if there is a baseline axis
        ...
        case PMS::ANTENNA: {
            ...
            for(Int ibsln = 0; ibsln < nBsln; ++ibsln) {
                Int a1val = *(plotmscache_->antenna1_[ic]->data() + ibsln);
                Int a2val = *(plotmscache_->antenna2_[ic]->data() + ibsln);
                if((a1val == iterValue_)  || (a2val == iterValue_)) {
        ...
        default:
            ...
            throw(AipsError("Unsupported iteration axis: " +
                            PMS::axis(iterAxis_)));
            break;
        }
    }
...
Record PlotMSIndexer::getPointMetaData(Int i) {
    ...
    Int icorr = Int(plotmscache_->getCorr(
                        currChunk_, getIndex1000(currChunk_, irel_)));
    String corr = plotmscache_->polname(icorr);

    Int offset = (
        currChunk_ > 0 ? (nCumulative_(currChunk_ - 1) + irel_) : irel_);
    // Collate meta data
    Record r;
    r.define("chan", chan);
    r.define("scan", scan);
...
PlotLogMessage* PlotMSIndexer::locateRange(
    const Vector<PlotRegion>& regions, Bool showUnflagged, Bool showFlagged) {
    Timer locatetimer;
    locatetimer.mark();
    ...
    if(showUnflagged) {
        ss << nFoundUnmasked << " unflagged" << (showFlagged ? ", " : "");
    }
    if(showFlagged) ss << nFoundMasked << " flagged";
    ss << ") among " << n << " in "
       << locatetimer.all_usec() / 1.0e6 << "s.";

    return new PlotLogMessage(PMS::LOG_ORIGIN,PMS::LOG_ORIGIN_LOCATE,
                              ss.str(), PMS::LOG_EVENT_LOCATE);
}

void PlotMSIndexer::reportMeta(Double x, Double y, Bool masked,
                               stringstream& ss) {
    ...
    ss << "Time=" << MVTime(plotmscache_->getTime(currChunk_, 0) /
                            C::day).string(MVTime::YMD,7) << " ";
    ss << "BL=";

    Int ant1 = Int(plotmscache_->getAnt1(
                       currChunk_, getIndex0010(currChunk_, irel_)) );
    Int ant2 = Int(plotmscache_->getAnt2(
                       currChunk_, getIndex0010(currChunk_, irel_)) );
    ...
    if(plotmscache_->netAxesMask_(0)) {
        ss << plotmscache_->polname(
            Int(plotmscache_->getCorr(currChunk_, getIndex1000(currChunk_,
                                                               irel_))));
    } else {
        ss << "*";
    }
...
PlotLogMessage* PlotMSIndexer::flagRange(const PlotMSFlagging& flagging,
            ...
            for(uInt j = 0; j < regions.size(); j++) {
                if(thisx > regions[j].left() && thisx < regions[j].right() &&
                   thisy > regions[j].bottom() && thisy < regions[j].top()) {
                    ...
                    // The following assumes currChunk_ and irel_
                    //  are properly set...
                    flagInCache(flagging, flag);

                    // Record this flags indices so we can apply to
                    //  MS (VisSet) below
    ...
    //  cout << "Found " << nFound << " points to "
    //       << (flag ? "flag." : "unflag.") << endl;
    ...
    return new PlotLogMessage(
        PMS::LOG_ORIGIN, flag ? PMS::LOG_ORIGIN_FLAG : PMS::LOG_ORIGIN_UNFLAG,
        ss.str(), flag ? PMS::LOG_EVENT_FLAG : PMS::LOG_EVENT_UNFLAG);
}
...
String PlotMSIndexer::iterLabel() {
    ...
    case PMS::FIELD:
        ...
        break;
    case PMS::BASELINE: {
        // sets currChunk_ and irel_ for first point so we can get ant indices
        maskedAt(0);
        Int ant1 = Int(plotmscache_->getAnt1(
                           currChunk_, getIndex0010(currChunk_, irel_)));
        Int ant2 = Int(plotmscache_->getAnt2(
...
void PlotMSIndexer::flagInCache(const PlotMSFlagging& flagging, Bool flag) {
    ...
    if(plotmscache_->netAxesMask_(0) && !flagging.corrAll()) {
        ...
        icorr = getIndex1000(currChunk_, irel_);
    ...
    if(plotmscache_->netAxesMask_(2)) {
        // specific correlation
        ibsln = getIndex0010(currChunk_,irel_);
    ...
    //  cout << "Bsln, chan, corr: " << ibsln << " " << ichan << " "
    //       << icorr << endl;
...
void PlotMSIndexer::computeRanges() {
    ...
    */
    ...
    /*
    cout << "sizedUnMasked (unflagged) = " << sizeUnMasked_ << " Ranges:"
         << xmin_ << "-" << xmax_ << " / " << ymin_ << "-" << ymax_ << endl;
    cout << "sizedMasked (flagged)     = " << sizeMasked_
         << " Ranges:" << xflmin_ << "-" << xflmax_ << " / "
         << yflmin_ << "-" << yflmax_ << endl;
    cout << "size() = " << size() << endl;
    */
...
void PlotMSIndexer::log(const String& method, const String& message,
    ...
    //  plotms_->getLogger()->postMessage(PMS::LOG_ORIGIN, method,
    //                                    message, eventType);
}
