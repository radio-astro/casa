 ...
 * in a single pass.  We use a 2x3 grid and a common right,bottom axis
 */

int main(int /*argc*/, char** /*argv[]*/) {

	String dataPath = tUtil::getFullPath( "pm_ngc5921.ms" );
    cout << "tExportRangeRightBottom using data from "<<dataPath.c_str()<<endl;
    ...
    axisParams->setAxes(X_BOTTOM, Y_RIGHT, 0 );
    ...
    String outFile = exportPath + "plotMSExportRangeRightBottom";
	...
	bool okOutput = tUtil::checkFile( outFile1, 110000, 155000, -1 );
	cout << "tExportRangeRightBottom:: Result of first save file check=" << okOutput << endl;
	bool okOutput2 = tUtil::checkFile( outFile2, 45000, 60000, -1 );
	cout << "tExportRangeRightBottom:  Result of second save file check=" << okOutput2 << endl;
