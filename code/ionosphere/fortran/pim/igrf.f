...
C-----------------------------------------------------------------------
	...
	INTEGER nmax,nmax1,nmax2, iyea, i,iu,is,j,l,m,n, numye, ier
	REAL umr, erad, aquad, bquad, dtemod,dte1,dte2
	REAL year,time, gh1,gh2,gha
...
C
	CALL GETSHC (IU, FIL1, pigrf, NMAX1, ERAD, GH1, IER)  
	    IF (IER .NE. 0) THEN
               WRITE (*,*) 'Error in 1st GETSHC'
	       write (*,*) iu, fil1, pigrf 
               CALL A2STOP('')
            END IF
	CALL GETSHC (IU, FIL2, pigrf, NMAX2, ERAD, GH2, IER)  
	    IF (IER .NE. 0) CALL A2STOP('')
...
C ===============================================================  
	...
	REAL erad, gh, g,h
        CHARACTER pigrf*80, filigrf*80, fspec*(*)
	DIMENSION	GH(*)                                        
