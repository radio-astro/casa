...
C  Common block
      ...
      REAL uthh, dbglat(100),dbglon(100)
...
C      dbgflg = .TRUE.
C      WRITE (*,'(2I,5F)') year,day,ut,obslat,obslon,saz,sel
...
C-----------------------------------
      ...
      IF (nalt .GT. maxexpalt) THEN
         WRITE (*,*) 'Too many Altitude samples in RTDAPPL'
         CALL A2STOP('')
      END IF
...
C==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==
      ...
      SUBROUTINE tecfr (dofr, nalt, az,el,lat,lon, z,rng,
...
C
       ...
       REAL saz,sel,slat,slon,latr,lonr
       REAL losx,losy,losz, bnorth,beast,bdown,babs, bx,by,bz
       REAL bpar(maxexpalt),bperp(maxexpalt), rotmeas(maxexpalt)
       REAL tec, rmi,rotmin
...
C------------------------
      ...
      IF (nalt .GT. maxexpalt) THEN
         WRITE (*,*) 'Too many altitude samples in TECFR'
         CALL A2STOP('')
      END IF
...
C
     ...
     2          COS(sel) * COS(saz) * SIN(slat) * SIN(slon)
         ...
         DO 100 i = 1, nalt
            CALL feldg (loslat(i),loslon(i), z(i),
     1                    bnorth,beast,bdown,babs)
        
            lonr = loslon(i) * SNGL(degrad)
            latr = loslat(i) * SNGL(degrad)
            bx = beast * SIN(lonr) +
     1           bnorth * SIN(latr) * COS(lonr) +
     2           bdown * COS(latr) * COS(lonr)
            bx = -bx
            by = beast * COS(lonr) +
     1           bnorth * SIN(latr) * SIN(lonr) - 
     2           bdown * COS(latr) * SIN(lonr)
            bz = bnorth * COS(latr) - bdown * SIN(latr)
  
            bpar(i) = losx*bx + losy*by + losz*bz
            bperp(i) = SQRT(babs*babs - bpar(i)*bpar(i))
 100     CONTINUE
