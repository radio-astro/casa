...
C
      IF(X12X23 .LE. 0.) CALL A2STOP(
     & 'PAR3PT: X-values are not monotonic.')
C
...
C
      IF(X12X23 .LE. 0.) CALL A2STOP(
     & 'LAG3PT: X-values are not monotonic.')
C
...
C
      ...
      DO J=1,N-1
       DO I=1,N-J
        ...
        W=C(I+1)-D(I)
        DEN=HO-HP
        IF(DEN .EQ. 0.) CALL A2STOP('POLINT:Identical points on grid.')
        DEN=W/DEN
        D(I)=HP*DEN
        C(I)=HO*DEN
       ENDDO
...
C
      CALL A2STOP('QROMB:Too many steps for me.')
C
      END

C==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==

      SUBROUTINE piksrt(n,arr,a)
      INTEGER n
      CHARACTER arr(n)*(*)
      INTEGER i,j
      CHARACTER a*(*)
...
C==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==*==
      ...
      SUBROUTINE piksrt3(n,dparr,arr,brr)
      INTEGER n
      REAL*8 dparr(n), brr(n)
      INTEGER i,j, arr(n), a
      REAL*8 dp,b
      DO 12 j=2,n
         dp=dparr(j)
