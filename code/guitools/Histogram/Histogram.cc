...
ImageHistograms<Float>* Histogram::filterByChannels( const CountedPtr<const ImageInterface<Float> > image ){
...
void Histogram::setImage(const CountedPtr<const ImageInterface<Float> > img ){
	image = img;
}
	...
	if ( image.get() != NULL ){
		...
		try {
			if ( region == NULL ){
				...
				CountedPtr<SubImage<Float> > subImage(new SubImage<Float>( *image, *region ));
