 ...
 *
 * "@(#) $Id: ATMSkyStatus.cpp,v 1.13 2011/11/22 18:08:07 jroche Exp $"
 *
 * who       when      what
 * --------  --------  ----------------------------------------------
...
SkyStatus::SkyStatus(RefractiveIndexProfile refractiveIndexProfile,
                     Temperature temperatureBackground) :
  RefractiveIndexProfile(refractiveIndexProfile), airMass_(1.0),
...
bool SkyStatus::updateProfilesAndRadiance(Length altitude,
                                          Pressure groundPressure,
                                          Temperature groundTemperature,
                                          double tropoLapseRate,
                                          Humidity relativeHumidity,
...
Length SkyStatus::getAverageNonDispersiveH2OPathLength(unsigned int spwid)
{
  ...
  for(unsigned int i = 0; i < getNumChan(spwid); i++) {
    av = av
        + getNonDispersiveH2OPathLength(v_transfertId_[spwid] + i).get("mm");
  }
  av = av / getNumChan(spwid);
  Length average(av, "mm");
  return average;
...
Length SkyStatus::WaterVaporRetrieval_fromFTS(unsigned int spwId,
                                              vector<double> v_transmission,
                                              Frequency f1,
                                              Frequency f2)
{
  if(f1.get() > f2.get()) {
    return Length(-999, "mm");
  }
...
Length SkyStatus::WaterVaporRetrieval_fromTEBB(unsigned int spwId, vector<
    Temperature> v_tebb, double airmass, double skycoupling, Temperature tspill)
{
  vector<double> spwId_filter;
  for(unsigned int n = 0; n < v_numChan_[spwId]; n++) {
    spwId_filter.push_back(1.0);
...
Length SkyStatus::WaterVaporRetrieval_fromTEBB(vector<unsigned int> spwId,
                                               vector<Percent> signalGain,
                                               vector<vector<Temperature> > vv_tebb,
                                               vector<vector<double> > spwId_filter,
                                               double airmass,
                                               vector<double> skycoupling,
                                               vector<Temperature> tspill)
{
...
double SkyStatus::SkyCouplingRetrieval_fromTEBB(unsigned int spwId, vector<
    Temperature> v_tebb, double skycoupling, Temperature tspill)
{
  vector<double> spwId_filter;
  for(unsigned int n = 0; n < v_numChan_[spwId]; n++) {
    spwId_filter.push_back(1.0);
...
Length SkyStatus::mkWaterVaporRetrieval_fromFTS(unsigned int spwId,
                                                vector<double> measuredSkyTransmission,
                                                double airm,
                                                Frequency fre1,
                                                Frequency fre2)
{
...
Length SkyStatus::mkWaterVaporRetrieval_fromTEBB(vector<unsigned int> spwId,
                                                 vector<Percent> signalGain,
                                                 vector<Temperature> measuredAverageSkyTEBB,
                                                 double airm,
                                                 vector<vector<double> > spwId_filter,
                                                 vector<double> skycoupling,
                                                 vector<Temperature> tspill)
...
Length SkyStatus::mkWaterVaporRetrieval_fromTEBB(vector<unsigned int> spwId,
                                                 vector<Percent> signalGain,
                                                 vector<vector<Temperature> > measuredSkyTEBB,
                                                 double airm,
                                                 vector<vector<double> > spwId_filter,
                                                 vector<double> skycoupling,
                                                 vector<Temperature> tspill)
...
double SkyStatus::mkSkyCouplingRetrieval_fromTEBB(unsigned int spwId,
                                                  Percent signalGain,
                                                  vector<Temperature> measuredSkyTEBB,
                                                  double airm,
                                                  vector<double> spwId_filter,
                                                  double skycoupling,
                                                  Temperature tspill)
...
Temperature SkyStatus::getWVRAverageSigmaTskyFit(vector<WVRMeasurement> RadiometerData,
                                                 unsigned int n,
                                                 unsigned int m)
{
  double sigma = 0.0;
