...
// calls to QtPixelCanvas methods...).
	...
	void QtPixelCanvas::copyBackBufferToFrontBuffer() {
		...
		update();
	}		// assure the widget gets the latest front buffer.
	...
	void QtPixelCanvas::copyFrontBufferToBackBuffer() {
		...
		p_begin_();
	}	// return to drawing on current buffer with p_.
	...
	void QtPixelCanvas::copyBackBufferToFrontBuffer(Int x1, Int y1,
		...
		Int qx,qy,w,h;
		qFromPC_(x1,y1,x2,y2, qx,qy,w,h);
...
// (But: doing that would make using QPC as a 'child Widget' a pain...).
	...
	void QtPixelCanvas::paintEvent(QPaintEvent* /*e*/) {
		...
		pw_.end();
		p_begin_();
	}		// reopen the painter used for draw commands.
	...
	void QtPixelCanvas::keyPressEvent (QKeyEvent* e ) {
		...
		switch ( qkey ) {
		case Qt::Key_Escape:
			ksym = Display::K_Escape;
			break;
		case Qt::Key_Space:
			ksym = Display::K_space;
			break;
		case Qt::Key_L:
			ksym = Display::K_l;
			break;
		...
		case Qt::Key_End:
			...
			break;
		case Qt::Key_PageUp:
			ksym = Display::K_Page_Up;
			break;
		case Qt::Key_PageDown:
			ksym = Display::K_Page_Down;
			break;
		default:
			if ( dlKeyMods_(e->modifiers()) == 0 ) {
				e->ignore();
				return;
			}
		}
...
//###########################
	...
	Float QtPixelCanvas::getLineWidth() const {
		return 1.;
	}
	...
	void QtPixelCanvas::drawText(QPainter& painter, const QPoint& point,
		...
		case Display::AlignLeft:
			br.moveTo(p.x(), p.y() - (br.height() / 2));
			break;
		case Display::AlignTop:
			br.moveTo(p.x() - (br.width() / 2), p.y());
			break;
		case Display::AlignRight:
			br.moveTo(p.x() - br.width(), p.y() - (br.height() / 2));
			break;
		case Display::AlignBottom:
			br.moveTo(p.x() - (br.width() / 2), p.y() - br.height());
			break;
	...
	void QtPixelCanvas::setLineStyle(Display::LineStyle style) {
		...
		case Display::LSSolid:
			itsPen.setStyle(Qt::SolidLine);
			break;
		case Display::LSDashed:
			itsPen.setStyle(Qt::DashLine);
			break;
	...
	uLong QtPixelCanvas::getForeground() const {
		return 0;
	}

	uLong QtPixelCanvas::getBackground() const {
		return 0;
	}
...
//#dk I _think_ we can no-op these and clear using deviceBackground...
	...
	uInt QtPixelCanvas::color() const {
		return 0;
	}
