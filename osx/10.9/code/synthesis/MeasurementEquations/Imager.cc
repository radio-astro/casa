...
Bool Imager::uvrange(const Double& uvmin, const Double& uvmax)
{
  ...
  try {
    ...
    if(mssel_p.null()){ os << "Please setdata first before using uvrange " << LogIO::POST; return False; }
     ...
     String parseString="select from $1 where (SQUARE(UVW[1]) + SQUARE(UVW[2]))*" + static_cast<std::string>(strInvLambda) + " > " + strUVmin.str( ) + " &&  (SQUARE(UVW[1]) + SQUARE(UVW[2]))*" + static_cast<std::string>(strInvLambda) + " < " + strUVmax.str( );
