...
#     jobs = [j for j,_ in jobs_and_wrappers]
    ...
    merged_results = merge_jobs(mergeable, casa_tasks.plotms, merge=(iteraxis,), 
                                ignore=('plotfile',))
    ...
    for merged_job, component_jobs in merged_results:
        ...
        if len(component_jobs) is 1:
            ...
            job_to_execute = casa_tasks.plotms(iteraxis=iteraxis, **merged_job.kw)
            LOG.trace('Component jobs:\n%s\nReplacement job:\n%s',
                      '\n'.join([str(j) for j in component_jobs]), job_to_execute)
        ...
        if not all([os.path.exists(dest) for dest in dest_filenames]):
            job_to_execute.execute(dry_run=False)
