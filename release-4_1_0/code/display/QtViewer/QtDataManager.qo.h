...
/*   infofield_list_t img_ifields; */
 ...
 private:
	...
	validation_msg( const validation_msg &o ) : path(o.path), output_format(o.output_format), valid(o.valid), msg(o.msg) { }
        validation_msg( ) : path(/* QString::Null */), output_format(CASA), valid(INVALID), msg(/* QString::Null */) { fprintf( stderr, "HERE<3>:::: " ); }
	validation_msg operator=(const validation_msg &o ) { path = o.path; output_format = o.output_format; valid = o.valid; msg = o.msg; return *this; }
	...
	public:
	    ...
	    const std::set<int> &filter( ) const { return filtered_types_; }
	    void updateDir( ) { if ( dir_entry_ ) { dir_entry_->setText(dir_->currentPath( )); } }
	    ...
	    QFrame *infoFrame( ) { return info_frame_; }
	    infofield_list_t *infoFields( ) { return info_fields_; }
	    
	    QTreeWidget *dtree( ) { return dtree_; }
	    QLineEdit *outFileLine( ) { return output_name_line_; }

	    std::tr1::shared_ptr<QDir> dir( ) { return dir_; }
	    QLineEdit *dirline( ) { return dir_entry_; }
	    std::string dirStr( ) const { return dir_entry_->text( ).toStdString( ); }
	private:
