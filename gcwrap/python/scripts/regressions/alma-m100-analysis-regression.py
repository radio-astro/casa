...
def timing():
    ...
    casalog.post('Step ' + str(mystep) + ': ' + step_title[mystep],
                 'WARN')
    ...
    casalog.post('Step  Time used (s)     Fraction of total time (percent) [description]'
                 , 'WARN')
    for i in range(0, len(steptime)):
        casalog.post('  ' + str(thesteps[i]) + '   ' + str(steptime[i])
                     + '  ' + str(steptime[i] / totaltime * 100.) + ' ['
...
if mystep in thesteps:
    ...
    importasdm(asdm='uid___A002_X2a5c2f_X54', vis='X54.ms',
               asis='Stati* Anten*', overwrite=True, lazy=True)
    importasdm(asdm='uid___A002_X2a5c2f_X220', vis='X220.ms',
               asis='Stati* Anten*', overwrite=True, lazy=True)
...
if mystep in thesteps:
	...
	gencal(vis = 'X54.ms',
	       ...
	       parameter = [-0.000379055076246,0.000910912511392,-0.000226045671848,7.8790821135e-05,0.000363811850548,-0.000224065035582,-0.000315962965482,
                        0.000397399838991,-0.000581170175089,-6.35427422822e-05,0.00129573699087,-0.000625061802566,-0.000167516991496,0.000174060463905,
                        -0.000417742878199,-0.00010875146836,0.000319179147482,-0.000588130671531,0.000142965465784,0.000455257482827,0.000168651808053,
                        -0.00150847900659,0.00357818510383,0.000811365433037])
	gencal(vis = 'X220.ms',
...
if mystep in thesteps:
    ...
    if cu.compare_version('>=', [3, 4, 0]):
        ...
        for name in basename:
            ...
            gencal(vis=name + '.ms', caltype='tsys',
        ...
        for spw in ['9', '11', '13', '15']:
            for name in basename:
                plotcal(
                    ...
                    plotsymbol='.',
                    figfile='cal-tsys_per_spw_' + spw + '_' + name
                        + '.png')
...
if mystep in thesteps:
    ...
    myflagcmd = ["mode='manual' antenna='CM01'",
                 ...
                 "mode='manual' intent='*ATMOSPHERE*'", "mode='shadow'"
...
#    os.system('/home/arcproc/opsw/alma/wvrgcal-stable/bin/wvrgcal --ms X220.ms --output cal-wvr_X220 --toffset -1 --wvrflag CM01 --segsource --tie "1224+213 Phase","M100" --statsource "1224+213 Phase"')
    wvrgcal(
        vis='X54.ms',
        caltable='cal-wvr_X54',
        toffset=-1,
        wvrflag=['CM01'],
        segsource=True,
        tie=['1224+213 Phase,M100'],
        statsource='1224+213 Phase',
        spw=[1, 3, 5, 7],
        wvrspw=[0],
        )
    wvrgcal(
        vis='X220.ms',
        caltable='cal-wvr_X220',
        toffset=-1,
        wvrflag=['CM01'],
        segsource=True,
        tie=['1224+213 Phase,M100'],
...
if mystep in thesteps:
    ...
    for myfield in ['3c273 - Bandpass', 'Titan', '3c273 - Phase',
                    '1224+213 Phase', 'M100']:
        print 'Field ', myfield
        for name in basename:
            applycal(
                ...
                spw='1,3,5,7',
                field=myfield,
                gainfield=[myfield, myfield, '', ''],
                interp=['nearest', tsysinterp, 'nearest', 'nearest'],
                spwmap=[[], tsysspwmap, [], []],
                gaintable=['cal-wvr_' + name, 'cal-tsys_' + name
                           + '.fdm', 'cal-delay_' + name + '.K',
                           'cal-antpos_' + name],flagbackup=False)
...
if mystep in thesteps:
    ...
    for name in basename:
        ...
        split(vis=name + '.ms', outputvis=name + '-line.ms',
              spw='1,3,5,7', datacolumn='corrected')
        flagmanager(vis=name + '-line.ms', mode='save',
                    versionname='apriori')
    timing()
...
if mystep in thesteps:
    ...
    for name in basename:
        ...
        flagdata(vis=name + '-line.ms', mode='list', inpfile=flagcmd,
                 flagbackup=False)
    ...
    flagdata(vis='X54-line.ms', antenna='PM01',
             timerange='19:03:35~19:03:42', mode='manual',
             flagbackup=False)

    flagdata(vis='X54-line.ms', antenna='DV04',
             timerange='19:38:45~19:38:55', mode='manual',
             flagbackup=False)
...
if mystep in thesteps:
    ...
    for name in basename:
        ...
        split(vis=name + '-line.ms', outputvis=name + '-line-vs.ms',
              datacolumn='data', width='8')

        flagmanager(vis=name + '-line-vs.ms', mode='save',
                    versionname='apriori')
    ...
    for name in basename:
        ...
        if makeplots:
            plotcal(
                ...
                iteration='spw',
                figfile='cal-' + name + '-phase_vs_time_XX.BPint.Gp.png'
                    ,
                subplot=221,
                )
            ...
            plotcal(
                ...
                plotrange=[0, 0, -180, 180],
                iteration='spw',
                figfile='cal-' + name + '-phase_vs_time_YY.BPint.Gp.png'
                    ,
                subplot=221,
                )
        ...
        if makeplots:
            for spw in ['0', '1', '2', '3']:
                plotcal(
                    ...
                    plotsymbol='.',
                    timerange='',
                    figfile='cal-' + name + '-phase.spw' + spw
                        + '.B1.png',
                    )
                ...
                plotcal(
                    ...
                    plotsymbol='.',
                    timerange='',
                    figfile='cal-' + name + '-amplitude.spw' + spw
                        + '.B1.png',
                    )
    timing()
...
if mystep in thesteps:
    ...
    for name in basename:
        flagmanager(vis=name + '-line-vs.ms', mode='restore',
                    versionname='apriori')
    ...
    setjy(vis=name + '-line-vs.ms', field='Titan',
          standard='Butler-JPL-Horizons 2012', scalebychan=False,
          spw='0,1,2,3')
...
if mystep in thesteps:
    ...
    if makeplots:
        plotcal(
            ...
            plotrange=[0, 0, -180, 180],
            iteration='spw',
            figfile='cal-' + name + '-phase_vs_time_XX.int.Gp.png',
            subplot=221,
            )
        ...
        plotcal(
            ...
            plotrange=[0, 0, -180, 180],
            iteration='spw',
            figfile='cal-' + name + '-phase_vs_time_YY.int.Gp.png',
            subplot=221,
            )
...
if mystep in thesteps:
    ...
    if makeplots:
        plotcal(
            ...
            plotrange=[0, 0, -180, 180],
            iteration='spw',
            figfile='cal-' + name + '-phase_vs_time_XX.scan.Gp.png',
            subplot=221,
            )
        ...
        plotcal(
            ...
            plotrange=[0, 0, -180, 180],
            iteration='spw',
            figfile='cal-' + name + '-phase_vs_time_YY.scan.Gp.png',
            subplot=221,
            )
...
if mystep in thesteps:
    ...
    if makeplots:
        plotcal(
            ...
            plotsymbol='o',
            iteration='spw',
            figfile='cal-' + name + '-amp_vs_time_XX.scan.Gap.png',
            subplot=221,
            )
        ...
        plotcal(
            ...
            plotsymbol='o',
            iteration='spw',
            figfile='cal-' + name + '-amp_vs_time_YY.scan.Gap.png',
            subplot=221,
            )
...
if mystep in thesteps:
    ...
    for name in basename:
        fluxscale(vis=name + '-line-vs.ms', caltable='cal-' + name
                  + '-scan.Gap', fluxtable='cal-' + name + '.flux',
                  reference='Titan', transfer='*Phase*,*Band*')
...
if mystep in thesteps:
    ...
    for name in basename:
        ...
        applycal(
            vis=name + '-line-vs.ms',
            field='3c273 - Phase',
            gaintable=['cal-' + name + '.B1', 'cal-' + name + '-scan.Gp'
                       , 'cal-' + name + '.flux'],
...
if mystep in thesteps:
    ...
    if makeplots:
        for name in basename:
            imview(raster={
                'file': 'test-' + name + '-sec_phasecal.image.tt0',
                'colorwedge': True,
                'range': [-0.02, 8.0],
                'scaling': -1.5,
                'colormap': 'Rainbow 2',
...
if mystep in thesteps:
    ...
    if makeplots:
        for name in basename:
            imview(raster={
                'file': 'test-' + name + '-prim_phasecal.image.tt0',
                'colorwedge': True,
                'range': [-0.005, 0.9],
                'scaling': -2.5,
                'colormap': 'Rainbow 2',
                }, out='test-' + name + '-prim_phasecal.png', zoom=1)
            calstat = imstat(imagename='test-' + name
                             + '-prim_phasecal.image.tt0', region='',
                             box='30,30,170,80')
            rms = calstat['rms'][0]
            ...
            print '>> Dynamic range in phase calibrator image: ' \
                + str(peak / rms)
...
if mystep in thesteps:
    ...
    for name in basename:
        ...
        split(vis=name + '-line-vs.ms', field='M100', outputvis=name
              + '-calibrated.ms', datacolumn='corrected',
              keepflags=False)
...
if mystep in thesteps:
    ...
    for name in basename:
        ...
        calstat = imstat(imagename='test-' + name
                         ...
                         box='30,30,170,80')
        rms = calstat['rms'][0]
        casalog.post(name + ': rms in phase calibrator image: '
                     + str(rms))
        calstat = imstat(imagename='test-' + name
                         + '-prim_phasecal.image.tt0', region='')
        peak = calstat['max'][0]
        casalog.post(name + ': Peak in phase calibrator image: '
                     + str(peak))
        casalog.post(name
                     + ': Dynamic range in phase calibrator image: '
                     + str(peak / rms))
    ...
    for i in [0, 1]:
        casalog.post(basename[i] + ',' + str(respeak[i]) + '('
                     + str(exppeak[i]) + ',' + str(exppeak33[i]) + '),'
                     + str(resrms[i]) + '(' + str(exprms[i]) + ','
                     + str(exprms33[i]) + ')')
    ...
    casalog.post(str(respeakm) + '(' + str(exppeakm) + ','
                 + str(exppeakm33) + '),' + str(resrmsm) + '('
                 + str(exprmsm) + ',' + str(exprmsm33) + ')')
    casalog.post('------------------------------------------------------------------------------------------'
                 )
    ...
    for i in [0, 1]:
        ...
        if peakdev > 0.5:
            casalog.post('ERROR: Peak in primary phase calibrator image '
                          + str(i) + ' deviates from expectation by '
                         + str(peakdev) + ' percent.', 'WARN')
            passed = False

        rmsdev = abs(resrms[i] - exprms[i]) / exprms[i] * 100.
        if rmsdev > 0.5:
            casalog.post('ERROR: RMS in primary phase calibrator image '
                          + str(i) + ' deviates from expectation by '
                         + str(rmsdev) + ' percent.', 'WARN')
            passed = False

    peakmdev = abs(respeakm - exppeakm) / exppeakm * 100.
    if peakmdev > 0.5:
        casalog.post('ERROR: Peak in M100 central field image '
                     + str(i) + ' deviates from expectation by '
                     + str(peakmdev) + ' percent.', 'WARN')
        passed = False

    rmsmdev = abs(resrmsm - exprmsm) / exprmsm * 100.
    if rmsmdev > 0.5:
        casalog.post('ERROR: RMS in M100 central field image ' + str(i)
                     + ' deviates from expectation by ' + str(rmsmdev)
                     + ' percent.', 'WARN')
        passed = False

    if not os.path.exists('M100-CO.mom0'):
        casalog.post('ERROR: M100 line cube moment 0 map was not created!'
                     , 'WARN')
        passed = False

    if not os.path.exists('M100-CO.mom1'):
        casalog.post('ERROR: M100 line cube moment 1 map was not created!'
                     , 'WARN')
        passed = False

    if not passed:
        raise Exception, \
            'Results are different from expectations by more than 0.5 percent.'
