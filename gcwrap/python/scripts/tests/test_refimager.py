...
class test_mask(testref_base):
     ...
     def test_mask_autobox_redraw(self):
         # changed to use threshold based automasking 
          ...
          self.prepData('refim_eptwochan.ms')
          ret = tclean(vis=self.msfile,imagename=self.img,imsize=100,cell='8.0arcsec',niter=10,deconvolver='hogbom',interactive=0,usemask='auto-thresh',maskthreshold=0.8)
          ret2 = tclean(vis=self.msfile,imagename=self.img+'2',imsize=100,cell='8.0arcsec',niter=20,cycleniter=10,deconvolver='hogbom',interactive=0,usemask='auto-thresh',maskthreshold=0.8)
          self.checkall(imexist=[self.img+'.mask'], imval=[(self.img+'.mask',1.0,[50,50,0,0]),(self.img+'.mask',0.0,[55,30,0,0]),(self.img+'2.mask',1.0,[55,30,0,0])])
...
class test_widefield(testref_base):
     ...
     def test_widefield_aproj_mtmfs(self):
          ...
          self.checkall(imexist=[self.img+'.image.tt0', self.img+'.psf.tt0', self.img+'.weight.tt0'],imval=[(self.img+'.image.tt0',1.0,[256,256,0,0]),(self.img+'.weight.tt0',0.49,[256,256,0,0]),(self.img+'.alpha',-1.4,[256,256,0,0]) ] )
...
class test_widefield_failing(testref_base):
     ...
     def test_widefield_mosaicft_mfs(self):
          ...
          self.checkall(imexist=[self.img+'.image', self.img+'.psf', self.img+'.weight'],imval=[(self.img+'.image',0.9471,[256,256,0,0]),(self.img+'.weight',0.3008,[256,256,0,0]) ] )
     ...
     def test_widefield_mosaicft_mtmfs(self):
          ...
          self.checkall(imexist=[self.img+'.image.tt0', self.img+'.psf.tt0', self.img+'.weight.tt0'],imval=[(self.img+'.image.tt0',0.922,[256,256,0,0]),(self.img+'.weight.tt0',0.299,[256,256,0,0]),(self.img+'.alpha',-0.04,[256,256,0,0]) ] )
