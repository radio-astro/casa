...
class sdflag_unittest_base:
    ...
    def _test_flag_result(self, name, ntbrow, rowlist, chanlist=None):
        """
        Test FLAGROW and FLAGTRA
        Arguments
            name     : name of scantable to test
            ntbrow   : expected row number of the scantable
        ...
        * Existance of the scantable
        * Number of rows in the scantable.
        * Compare FLAGROW with expected values defined by rowlist and chanlist
        * Compare FLAGTRA with expected values defined by rowlist and chanlist
        """
        ...
        nrow = tb.nrows()
        tb.close()
        self.assertEqual(nrow, ntbrow, "The number of rows in table changed.")
        idx_flagrow = rowlist if len(chanlist) == 0 else []
        # compare FLAGROW
        # create reference flagrow array
        ref_flagrow = [ 0 for idx in xrange(ntbrow) ]
...
class sdflag_selection(selection_syntax.SelectionSyntaxTest,
    ...
    def test_timerange_value_exact(self):
        ...
        ref_row = [1,2]
        self._run_flag_test(ref_row,timerange=timerange)
    ...
    # Helper functions
    ####################
    def _run_flag_test(self, flagged_row, flagged_chanlist=None,regular_test=True, **kwargs):
        """
        Run sdflag task with parameters given as key word arguments,
        and test the results by running _test_flag_result() with values
        given as the arguments, flagged_row amd flagged_chanlist.
        If regular_test is set to False, sdflag is invoked directly instead
        of calling self.run_task.
        """
        params = self.default_params.copy()
        for k,v in kwargs.items():
            params[k] = v
        if regular_test: self.res=self.run_task(**params)
        else: self.res = sdflag(**params)
        outfile = params['outfile'] if params['outfile'] != '' else params['infile']
        self._test_flag_result(outfile,self.ntbrow,flagged_row,flagged_chanlist)

    def _run_flag_failure_test(self, message,regular_test=True, **kwargs):
        """
        Run failure case of sdflag task with parameters given
        as key word arguments, and assert the error message is
        the one expected.
        If regular_test is set to False, sdflag is invoked directly instead
        of calling self.run_task.
        """
        ...
        try:
            ...
            pos=str(e).find(message)
            self.assertNotEqual(pos,-1,
                                msg='Unexpected exception was thrown: %s'%(str(e)))


class sdflag_storage_test(sdflag_unittest_base,unittest.TestCase):
    """
    Test outfile='' (overwriting input scantable) with
    disk and memory storage. (Tests for CAS-6601)

    testDisk --- overwrite flag of input file with disk storage
    testMem --- overwrite flag of input file with memory storage
    """
    rawfile = 'sd_analytic_type1-3.bl.asap'
    ntbrow = 4 # number of rows in rawfile
    outfile = ''
    params =  {'infile': rawfile, 'mode': 'manual',
               'outfile': outfile, 'outform': 'ASAP',
               'overwrite': True, 'scan': '16~17'}
    ref_row = [1,2,3]
