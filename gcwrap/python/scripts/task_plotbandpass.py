...
# ALMA data:
#    casa 3.3 tables
# cd /export/data_1/thunter/alma/data/SciVer/TWHya
...
# au.plotbandpass(caltable='bandpass_all_b_odd_dv04',xaxis='freq',yaxis='phase',chanrange='')
#
# EVLA dual-pol data:
...
#
PLOTBANDPASS_REVISION_STRING = "$Id: task_plotbandpass.py,v 1.4 2013/02/13 11:28:17 thunter Exp $" 
import pylab as pb
...
#                print "First OBSERVE_TARGET scan = ", myscan
    ...
    if (myscan == -1):
        ...
        myscan = 0
    dataColumnName = getDataColumnName(inputMs)
    if (debug): print "dataColumnName = ", dataColumnName
    mytb = createCasaTool(tbtool)
    mytb.open("%s" % inputMs)
    if (myscan == 0):
...
def plotbandpass(caltable='', antenna='', field='', spw='', yaxis='amp',
                 ...
                 lo1='', showimage=False, showatmPoints=False, parentms='', pdftk='pdftk'):
  """
  ...
  if (casadef.subversion_revision < '22652'):
      print "This version of casa is too old.  Use au.plotbandpass instead."
      return
  ...
  if (os.path.exists(msName)):
      ...
      try:
          ...
          mytimestamp = timeUtilities.time()
          if (debug): print "time = ", mytimestamp
          msAnt = mymsmd.antennanames(range(mymsmd.nantennas()))
          if (debug): print "msAnt = ", msAnt
...
#          print "************* 2) Set msName to ", msName
  ...
  if (debug): print "uniqueFields = ", uniqueFields
...
#        nPolarizations,
        ...
        polynomialAmplitude, timesBP, antennasBP, cal_desc_idBP, spwBP]) = openBpolyFile(caltable)
      bpoly = True
...
#                nPolarizations,
                frequencyLimits, increments, frequenciesGHz, polynomialPhase,
                polynomialAmplitude, timesBP, antennasBP, cal_desc_idBP, spwBP]) = openBpolyFile(caltable2)
...
#               nPolarizations2,
               ...
               polynomialAmplitude2, timesBP2, antennasBP2, cal_desc_idBP2, spwBP2]) = openBpolyFile(caltable3)
        except:
         ...
         if (msFound):
             ...
             for token in tokens:
                 if (token in mymsmd.antennanames(range(mymsmd.nantennas()))):
                     ...
                     antlist.append(mymsmd.antennaid(token))
                 elif (token[0] == '!'):
                     if (token[1:] in mymsmd.antennanames(range(mymsmd.nantennas()))):
                         antlist = range(mymsmd.nantennas())
                         removeAntenna.append(mymsmd.antennaid(token[1:]))
                     else:
                         print "Antenna %s is not in the ms. It contains: " % (token), range(mymsmd.nantennas())
                         return()
                 else:
                     print "Antenna %s is not in the ms. It contains: " % (token), range(mymsmd.nantennas())
                     return()
             ...
             for token in tokens:
                 ...
                 if (myloc > 0):
                     ...
                     if (token[1:] in range(mymsmd.nfields)):
                         ...
                         print "Field %s is not in the ms. It contains: " % (token), range(mymsmd.nfields)
                         return()
                 else:
                     print "Field %s is not in the ms. It contains: " % (token), range(mymsmd.nfields)
                     return()
...
#            print "Appending to ggx: ", gain2[g][0]
  ...
  computedAtmTime = -1
  computedAtmField = -1
  skippingSpwMessageSent = True
  atmString = ''
  if (showimage and lo1==''):
      # We only need to run this once per execution.
      getLOsReturnValue = getLOs(msName, verbose=False)
      if (getLOsReturnValue != []):
          lo1s = interpretLOs(msName,parentms)
            ...
            if ((ant[i]==xant) and (cal_desc_id[i]==ispw) and sm
                ...
                if (msFound):
                    if (len(chanFreqGHz[ispw]) == 1):
                        if (skippingSpwMessageSent == False):
...
#            print "A) uniqueFields[%d] = " % (fieldIndex), uniqueFields[fieldIndex]
            ...
            if (type(uFFI) == type(np.ndarray(0))):
                uFFI = uFFI[0]
                if (debug): print "converting uFFI from array to ", type(uFFI)
...
#                        return()
            ...
            if (LO1 != None):
                ...
                print "B) uniqueFields[%d] = " % (fieldIndex), uniqueFields[fieldIndex]
                uFFI = uniqueFields[fieldIndex]
                print "type(uFFI) = ", type(uFFI)
        ...
        if (sum(xflag)==nChannels and sum(yflag)==nChannels and showflagged==False):
                if (overlayTimes):
                    ...
                    if (sloppyMatch(timerangeListTimes[-1], uniqueTimes[mytime], solutionTimeThresholdSeconds,myprint=debugSloppyMatch)):
                        ...
                        if (debug):
                            print "###### set doneOverlayTime = ", doneOverlayTime
                else:
                ...
                for p in range(nPolarizations):
                    if (overlayAntennas or overlayTimes):
                        if (corr_type_string[p] in polsToPlot):
                              ...
                              if (overlayTimes): # try to support time,antenna
                                  if (debug):
                        ...
                        for p in range(nPolarizations):
                          if (corrTypeToString(corr_type[p]) in polsToPlot):
                            if (overlayAntennas or overlayTimes):
                              ...
                              if (overlayTimes):  # try to support antenna,time
                                  if (myUniqueTime != []):
...
#            print "amp: ylim, yrange = ",  ylim, yrange
            if (overlayAntennas == False and overlayTimes == False and bOverlay == False):
                ...
                if (debug):
                    print "------doneOverlayTime = ", doneOverlayTime
                if (doneOverlayTime):
...
#    print "refFreqInTable = ", refFreqInTable
    ...
    if (verbose): print "scantimes = ", scantimes
    ...
    for i in range(len(scantimes)):
        ...
        if (verbose): print "tdiff = ", tdiff
        if (tdiff < mindiff):
            bestscan = scans[i]
            if (verbose): print "bestscan = ", bestscan
            mindiff = tdiff
    if (verbose):
...
def showFDM(originalSpw, chanFreqGHz):
    ...
    sdebug = False
    if (sdebug): print "Showing FDM (%d)" % (len(originalSpw)), originalSpw
    fdmctr = -1
    x0,x1 = pb.xlim()
    y0,y1 = pb.ylim()
    yrange = y1 - y0
...
#        print "Calling getRADecForField"
        mydirection = getRADecForField(vis, myfieldId)
        if (verbose): print "mydirection= ", mydirection
        if (len(telescopeName) < 1):
            telescopeName = 'ALMA'
        myazel = computeAzElFromRADecMJD(mydirection, myscantime/86400., telescopeName)
        conditions['elevation'] = myazel[1] * 180/math.pi
