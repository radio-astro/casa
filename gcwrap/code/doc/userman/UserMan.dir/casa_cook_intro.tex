...
CASA 3.4.0 is now available and the main feature improvements are
...
\subsection{What's New in Release 3.4.0}
...
\item {\bf  Flagging}
  \begin{itemize}
   \item  New flagging infrastructure via new tasks {\tt tflagdata} and {\tt flagcmd} with substantially improved performance and extended capabilities (see below). 

   \item   The previous version of the {\tt flagcmd} task was renamed to {\tt oldflagcmd}. The interface has been re-designed and several parameters have been renamed.

   \item The tasks {\tt flagdata} and {\tt flagdata2} are still available in CASA 3.4 but they will both be replaced by the new {\tt tflagdata} task along with the CASA 3.5 release. Please consider this for scripting as some parameters have changed in {\tt tflagdata}. All improvements below refer to the new tasks.

   \item Time-baseline auto flagging algorithm ({\tt mode='tfcrop'}) in {\tt tflagdata} with improved plotting capabilities to visualize flagged visibility points per baselines, and navigate through fields, spws and scans (formely part of {\tt testautoflag}).

   \item Threshold-based flagging ({\tt mode='rflag'}) in {\tt tflagdata} (originally implemented by E.Greisen in AIPS Dec2011]).

   \item Added {\tt mode='list'} to {\tt tflagdata} to combine any number of flag commands in one single run.

   \item Support for water vapor radiometer (WVR)-based expressions (e.g. {\tt 'ABS\_WVR'}) in auto-flagging algorithms ({\tt mode=clip,tfcrop,rflag}).

   \item  Added flexible iteration capabilities for the time dimension in the auto-flagging algorithms ({\tt mode=tfcrop,rflag}).

   \item In {\tt mode='clip'}, automatically flag {\it NaNs} by default, and optionally exact zeros with the new sub-parameter clipzeros.

   \item Shadowing can be calculated for antennas that are in the array but are not present in the MS.

   \item Support at the tool level to handle flagging of the new CalTablecalibration table format (all modes except for elevation and shadow).

   \item  New {\tt cmdreason} sub-parameter to allow saving reasons to the outfile (or to the {\tt FLAG\_CMD} table). 

   \item The task {\tt flagcmd} now supports all modes that exist in {\tt tflagdata} (except for {\tt mode=summary}).

   \item The flag commands are {\it NOT} automatically saved to the {\tt FLAG\_CMD} sub-table in the {\tt flagcmd} task. The user needs to set {\tt savepars} to {\it True} in order to achieve this.

   \item  New selection by reason from an input text file in both tasks.

   \item  Tasks {\tt importevla} and {\tt importasdm} now have flag-command-import options. 

   \item  The flagversions names have changed. The backup names do not take a mode as part of their names anymore. They only take the task name along with an index. Example: {\it flags.tflagdata\_1} or {\it flags.flagcmd\_1}.

   \item  The vector mode of the old {\tt flagdata} is gone. The user can achieve the same results using the {\tt inpmode='cmd'} of the {\tt flagcmd} task. See the task inline help for more details.
...
\item  {\bf Imaging}
  \begin{itemize}
    ...
    \item  multi-field support for multi-term {\tt clean}

    \item  {\tt clean} with {\tt nterms>1} will output a spectral index error map

    \item  experimental parallelized clean task ({\tt pclean})  
...
\item  {\bf Data examination/editing}
   ...
   \item  new {\tt imstat} output formatting, including an output file option

    \item  {\tt listobs} output can be written to a file

   \item  in concatenated data sets, it is possible that an antenna is on different antenna stations at different times. We introduced the "antenna@station" syntax for data selection to accommodate such situations

   \item  new task {\tt listfits} to display fits headers (images and uv data)

   \item  data selection in {\tt listobs}

   \item  {\tt plotms}: UVW coordinates can now be displayed in units of wavelengths (enabling the full uv-coverage view of multi-frequency syntesis data)
   \item  new task {\tt asdmsummary} displays a summary of the content of an ASDM. 
...
\begin{itemize}
  ...
  \item the creation of the {\tt MODEL column} is now optional, calibrator models are computed on the fly when needed. This increases the performance of CASA and reduces disk space requirements. A consequence is that the {\tt calready} parameter is obsolete in {\tt clean}.

  \item  {\bf new calibration table format} (not compatible with the old format)

  \item  calibration tables are now independent from the original MS

   \item  frequency windows can be adjusted for calibration solutions

   \item  conversion script {\tt caltabconvert} to convert old (CASA 3.3 and earlier) to new calibration tables (CASA 3.4 and later) - note: we recommend to regenerate the calibration tables with calibration tasks if possible as some information will be not availbale by simply converting old to new caltables. 

   \item  improved solar system models in {\tt setjy}

   \item  MS data selection fully supports observation ID

   \item  {\tt wvrgcal} to generate a calibration table from water vapor radiometer ALMA data

   \item  automatic lookup of EVLA antenna position changes in {\tt gencal caltype=antpos}
 
   \item  new task {\tt plotweather} to display the weather conditions. {\tt plotweather} also calculates the atmospheric opacities for each spw using actual and historical weather data for the EVLA 

  \item  {\tt uvcontsub} supports fitorder $>1$ for continuum subtraction

   \item  {\tt scalebychan=True} is the new default in {\tt setjy}
...
\begin{itemize}
   ...
   \item  simulations are now in LSRK
   \item  ALMA cycle 1 array configurations are included
...
\begin{itemize}
  ...
  \item  improved {\tt feather} task, including new parameters for weighting data, single dish amplitude scaling, and plotting

  \item  {\tt concat} can now apply additional, user supplied weights to the input data sets 

  \item  {\tt imtrans} has a new option to invert image axes

   \item  new task {\tt statwt} to derive visibility data weights based on their scatter
\end{itemize}

\item  {\bf Data im/export}
\begin{itemize}

  \item  performance improvements in {\tt importasdm}, incl. full support for large data files

  \item  ALMA ASDMv3 as default in {\tt importasdm}

  \item  The ASDM Pointing table has a new optional column atmosphericCorrection to reflect resulting atmospheric refraction
...
\begin{itemize}
  ...
  \item  zero level fitting in {\tt imfit}

  \item  support for constrained Gaussian multiplet fitting in {\tt specfit} (e.g. for hyperfine line fitting)

  \item  logfile support in {\tt specfit}

  \item  Lorentzian profile fitting in {\tt specfit}

  \item  {\tt imstat} now includes a {\tt mask} parameter

   \item  spectra line search tool can now return a python dictionary

  \item  support of masks with degenerate axes in all image analysis tasks
...
\begin{itemize}
   \item  new CASA region syntax is now partially supported in the {\tt viewer}, incl. loading/saving regions, plotting, creation and modification of regions

   \item  image statistics within regions in the {\tt viewer} are now displayed in a separate dock. The values are updated as the region is dragged across an image

   \item  Gaussian and polynomial fitting of spectra in the {\tt viewer} spectral profile tool
 
  \item  plotting of errors in the spectral profile viewer

  \item  mean, median, and sum image construction across the spectral axis ({\tt imcollapse} functionality) from within the {\tt viewer} spectral profile tool

  \item  selecting a position in the spectral profile tool with CNTRL-click jumps to the corresponding plane in the cube 

  \item  saving of images from within the {\tt viewer} (in CASA or FITS format)
...
\begin{itemize}
  ...
  \item  automatic import of single dish tasks at CASA startup

  \item  renamed {\tt sdcal} to {\tt sdreduce} and {\tt sdaverage} to {\tt sdcal}

   \item  new task {\tt sdgrid} to convolve single dish map data onto a regularly spaced grid

  \item  channel binning in {\tt sdsmooth}
...
\begin{itemize}
  \item  unified, optimized visibility access for some tasks
  \item  substantial performance improvements in {\tt importasdm} (including {\tt importevla}), {\tt tflagdata}, and {\tt flagcmd}, experimental parallelized clean {\tt pclean} 
  \item  The new flagger has successfully deployed asynchronous-I/O for flagging modes that read visibility values.
...
\begin{itemize}
  ...
  \item  when installing Linux rpms, it is now possible to have more
    than a single version of CASA on disk. Versions can be invoked
    like {\tt casapy --release 3.x.x}

  \item  improved support for special characters in MS field names

  \item  MWA, EVN, LOFAR, Effelsberg are now recognized observatories

  \item  pyfits is now included in CASA
...
\begin{itemize}
   ...
   \item {\tt tflagdata} --- flagging (and unflagging)
         of specified data (\S~\ref{section:edit.tflagdata});
...
The {\tt tflagdata} task (\S~\ref{section:edit.tflagdata})
...
needed to specify the flagging scope. {\tt tflagdata} also contains
