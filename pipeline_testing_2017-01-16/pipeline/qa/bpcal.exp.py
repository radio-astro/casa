...
def bpcal_calc( in_table, logger='' ):
	...
	full_numchanList = caLoc.numchannel()
        ...
        print "SPW STUFF 2 ", spwList, numchanList
	...
	try:
		...
		for s in range( len(spwList) ):
			...
			bpcal_stats['AMPLITUDE_FIT'][str(s)] = f
                        print "ENTRY ", str(s), f['spw'], f['chanRange']
                print "GOT AMP FIT " 
	...
	try:
            ...
            for s in range( len(spwList) ):
                ...
                # already. Note chanRange is used and not defined ?
                #spw = spwList[s]
                ...
                print "ENTRY ", str(s), int (spwList[s]), chanRange
                # Amplitude
...
def bpcal_score( bpcal_stats ):
	...
	for index in indices:
		...
		spwNum = str(amp[index]['spw']); keys.remove( 'spw' )
		chanRange = amp[index]['chanRange']; keys.remove( 'chanRange' )

		if len(keys) == 0: continue

		bpcal_scores['AMPLITUDE_SCORE_FLAG'][spwNum] = dict()
		bpcal_scores['AMPLITUDE_SCORE_RMS'][spwNum] = dict()
		bpcal_scores['AMPLITUDE_SCORE_SNR'][spwNum] = dict()
		bpcal_scores['AMPLITUDE_SCORE_FN'][spwNum] = dict()
		bpcal_scores['AMPLITUDE_SCORE_DD'][spwNum] = dict()
		bpcal_scores['AMPLITUDE_SCORE_TOTAL'][spwNum] = dict()

		for k in keys:

			if len( amp[index][k]['pars'] ) == 0: continue

			bpcal_scores['AMPLITUDE_SCORE_FLAG'][spwNum][k] = \
			    bpcal_score_flag( amp[index][k]['flag'], chanRange )

			bpcal_scores['AMPLITUDE_SCORE_RMS'][spwNum][k] = \
			    bpcal_score_RMS( math.sqrt(amp[index][k]['resVar']),
			    0.1 )

                        bpcal_scores['AMPLITUDE_SCORE_SNR'][spwNum][k] = \
                            bpcal_score_SNR( bpcal_stats['AMPLITUDE_SNR'][index][k] )

                        bpcal_scores['AMPLITUDE_SCORE_FN'][spwNum][k] = \
                            bpcal_score_flatness( bpcal_stats['AMPLITUDE'][index][k] )

                        bpcal_scores['AMPLITUDE_SCORE_DD'][spwNum][k] = \
                            bpcal_score_derivative_deviation( bpcal_stats['AMPLITUDE'][index][k] )

			bpcal_scores['AMPLITUDE_SCORE_TOTAL'][spwNum][k] = \
			    bpcal_scores['AMPLITUDE_SCORE_FLAG'][spwNum][k] \
			    * bpcal_scores['AMPLITUDE_SCORE_RMS'][spwNum][k]
	...
	for index in indices:
		...
		keys = phase[index].keys()

		spwNum = str(phase[index]['spw']); keys.remove( 'spw' )
		chanRange = phase[index]['chanRange']; keys.remove( 'chanRange' )

		if len(keys) == 0: continue

		bpcal_scores['PHASE_SCORE_FLAG'][spwNum] = dict()
		bpcal_scores['PHASE_SCORE_RMS'][spwNum] = dict()
		bpcal_scores['PHASE_SCORE_FN'][spwNum] = dict()
		bpcal_scores['PHASE_SCORE_DD'][spwNum] = dict()
		bpcal_scores['PHASE_SCORE_DELAY'][spwNum] = dict()
		bpcal_scores['PHASE_SCORE_TOTAL'][spwNum] = dict()

		for k in keys:

			if len( phase[index][k]['pars'] ) == 0: continue

			bpcal_scores['PHASE_SCORE_FLAG'][spwNum][k] = \
			    bpcal_score_flag( phase[index][k]['flag'], chanRange )

			bpcal_scores['PHASE_SCORE_RMS'][spwNum][k] = \
			    bpcal_score_RMS( math.sqrt(phase[index][k]['resVar']),
			    0.05 )

                        bpcal_scores['PHASE_SCORE_FN'][spwNum][k] = \
                            bpcal_score_flatness( bpcal_stats['PHASE'][index][k]+45./180.*math.pi )

                        bpcal_scores['PHASE_SCORE_DD'][spwNum][k] = \
                            bpcal_score_derivative_deviation( bpcal_stats['PHASE'][index][k] )

			bpcal_scores['PHASE_SCORE_DELAY'][spwNum][k] = \
			    bpcal_score_delay(
			    phase[index][k]['pars'][1]/(2.0*math.pi),
			    phase[index][k]['vars'][1]/(2.0*math.pi),
			    phase[index][k]['frequency'], phase[index][k]['value'],
			    phase[index][k]['flag'], chanRange )

			bpcal_scores['PHASE_SCORE_TOTAL'][spwNum][k] = \
			    bpcal_scores['PHASE_SCORE_FLAG'][spwNum][k] \
			    * bpcal_scores['PHASE_SCORE_RMS'][spwNum][k] \
			    * bpcal_scores['PHASE_SCORE_DELAY'][spwNum][k]
