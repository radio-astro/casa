...
#  with options to overlay them in various combinations.  It relies on
...
# au.plotbandpass('bandpass.bcal',showtsky=F,xaxis='freq',yaxis='amp',overlay='antenna',spw='',field='',showfdm=True)
...
#
PLOTBANDPASS_REVISION_STRING = "$Id: plotbandpass.py,v 1.1.2.1 2012/09/06 15:03:47 swilliam Exp $"
import pylab as pb
...
def makeplot(figfile,msFound,msAnt,overlayAntennas,pages,pagectr,
             density,interactive,antennasToPlot,spwsToPlot,overlayTimes):
  if (type(figfile) == str):
    if (figfile.find('/')>=0):
  ...
  if (msFound):
    ...
    elif (overlayTimes and overlayAntennas):
        plotfilename = figfile+'.overlay_time_antenna'
    else:
...
def openBpolyFile(caltable):
   ...
   if ('CAL_DESC_ID' not in names):
       print "WARNING: This appears to be a new-format cal table from casa 3.4.  Using plotbandpass2."
       return(3.4)
   desc = tb.getdesc()
...
def plotbandpass(caltable='', antenna='', field='', spw='', yaxis='amp',
  ...
  For more detailed help, run help(au.plotbandpass) or see examples at:
  ...
  if (help):
      ...
      print " overlay: 'antenna','time' or 'antena,time', make 1 plot with items in colors"
      ...
      print " yaxis: 'tsys','amp','phase', or 'both' == 'ap'; append 'db' for dB"
  ...
  if (yaxis.find('both')<0 and yaxis.find('ap')<0 and yaxis.find('amp')<0
      and yaxis.find('phase')<0 and yaxis.find('tsys') < 0):
      print "Invalid yaxis.  Must be 'tsys', 'amp', 'phase' or 'both'."
      return(vm)
...
#  print "Using markersize = ", markersize
  ...
  try:
      ...
      return(vm)
  names = tb.colnames()
  caltableSubType = (tb.info())['subType']
  ...
  cal_desc_id = tb.getcol('CAL_DESC_ID')
  uniqueSpws = np.sort(np.unique(cal_desc_id))
...
#              if ((times[row] in timelist) == False):
...
#  print "shape(uniqueTimesPerFieldPerSpw) = ", np.shape(np.array(uniqueTimesPerFieldPerSpw))
#  displayTimesArray(uniqueTimesPerFieldPerSpw)
  displayTimesArray([[uniqueTimes]])

  # Parse the spws to plot from the command line
  if (spw==''):
     spwsToPlot = uniqueSpws
  else:
     if (type(spw) == str):
           if (spw.find('!')>=0):
  ...
  if (len(uniqueSpws) > 1):
      print "%d spws in the solution = " % (len(uniqueSpws)), uniqueSpws
  else:
      print "%d spw in the solution = " % (len(uniqueSpws)), uniqueSpws
  for myspw in spwsToPlot:
      if (myspw not in uniqueSpws):
          print "spw %d is not in the solution" % (myspw)
          calDesc = tb.open(caltable+'/CAL_DESC')
          originalSpws = tb.getcol('SPECTRAL_WINDOW_ID')
          if (myspw in originalSpws):
              mylist = list(originalSpws[0])
              print "You might mean spw %d, which was %d in the original data." % (mylist.index(myspw),myspw)
          return
  ...
  try:
      ...
      if (len(caltable2) > 0):
          try:
              ...
              print "Sorry, but for overlays, caltable must be a non-BPOLY solution, while caltable2 and 3 can be either type."
          return(vm)
  except:
      print "This is a %s solution." % (caltableSubType)
      ...
      if (len(caltable2) > 0):
        ...
        try:
          ...
          if (len(caltable3) > 0):
            ...
            print "Overlay another B solution"
            bOverlay = True
  ...
  calDesc = tb.open(caltable+'/CAL_DESC')
  msName = tb.getcol('MS_NAME')
  print "The associated ms = %s" % np.unique(msName)
  originalSpws = tb.getcol('SPECTRAL_WINDOW_ID')
  ...
  if (bpoly == False):
    ...
    # only present in B solutions.  It is blank in BPOLY solutions
    try:
        originalChannelRange = tb.getcol('CHAN_RANGE')
        originalChannelStart = originalChannelRange[0][0][:][0]
    except:
        originalChannelStart = np.zeros(len(msName))
      
  originalSpw = originalSpws[0]
  print "original unique spws in the dataset = ", np.unique(originalSpw)
  tb.close()
  msName = msName[0]
#  print "************* 1) Set msName to ", msName
  
  if (bOverlay):        
        calDesc2 = tb.open(caltable2+'/CAL_DESC')
        msName2 = tb.getcol('MS_NAME')
        originalSpws2 = tb.getcol('SPECTRAL_WINDOW_ID')
        originalChannelRange2 = tb.getcol('CHAN_RANGE')
        originalChannelStart2 = originalChannelRange2[0][0][:][0]
        originalSpw2 = originalSpws2[0]
        print "original unique spws in the second dataset = ", np.unique(originalSpw2)
        tb.close()
        # Now open the Bandpass solution table
        try:
              tb.open(caltable2)
              ...
              return(vm)
        caltable2SubType = (tb.info())['subType']
...
#        uniqueTimes2 = np.unique(np.unique(times2))
...
#        print "Found %d solutions in time: MJD seconds = " % (nUniqueTimes2), uniqueTimes2
        print "Found %d solutions in time." % (nUniqueTimes2)
...
#                          if ((times2[row] in timelist2) == False):
        ...
        tb.close()
        if (len(uniqueSpws2) > 1):
              print "%d spws in the second solution = " % (len(uniqueSpws2)), uniqueSpws2
        else:
              print "%d spw in the second solution = " % (len(uniqueSpws2)), uniqueSpws2
  ...
  print "Time ranges to plot (1-based) = ", timerangeList
...
#          print "************* 2) Set msName to ", msName
          try:
              ...
              msFields = vm.fieldNamesForFieldIds
              print "msAnt = ", msAnt
#              print "len(vm.spwInfo) = ", len(vm.spwInfo)
#              print "shape(vm.spwInfo) = ", np.shape(vm.spwInfo)
          except:
              print "1) Could not open the associated measurement set tables (%s). Will not translate antenna names or frequencies." % (msName)
  else:
      if (vm.getInputMs().split('/')[-1] != msName.split('/')[-1]):
          print "There is a mismatch between the ms name in the ValueMapping structure provided"
          print "and the one associated with this bandpass solution:"
...
#  pb.figure()
  ...
  if (bpoly):
    ...
    (corr_type, corr_type_string) = getCorrType(msName)
    if (corr_type_string == []):
        return(vm)
...
#                timeString = ',  t%d/%d  %s UT' % (mytime+1,nUniqueTimes,utstring(uniqueTimes[mytime],xframeStart))
                ...
                if ((yaxis.find('amp')>=0 or yaxis.find('tsys')>=0 or amplitudeWithPhase) and myap==0):
                  xframe += 1
                  myUniqueColor = []
                  if (debug):
                  ...
                  if (xframe == firstFrame):
                      if (debug):
                          print "printing title 1 ------------------------------------"
                      DrawBottomLegendPageCoords(msName, uniqueTimes[mytime], mysize)
                      pb.text(xstartTitle, ystartTitle,'%s (degamp=%d, degphase=%d)'%(caltable,nPolyAmp[index]-1,
                                nPolyPhase[index]-1),size=mysize,  transform=pb.gcf().transFigure) 
...
#                  print "w) incrementing xframe to %d" % xframe
                  ...
                  if (xframe == firstFrame):
                      if (debug):
                          print "printing title 2 ------------------------------------"
                      pb.text(xstartTitle, ystartTitle,'%s (degamp=%d, degphase=%d)'%(caltable,
         ...
         if (xframe == lastFrame):
            if (len(figfile) > 0):
                plotfiles.append(makeplot(figfile,msFound,msAnt,
                           overlayAntennas,pages,pagectr,density,
...
#               myinput = raw_input("(%.1f sec) Press return for next page (b for backwards, q to quit): "%(donetime-mytimestamp))
    ...
    if (len(figfile) > 0 and pagectr<len(pages)):
        if (debug):
            print "pagectr=%d, len(pages)=%d" % (pagectr,len(pages))
...
#      return
  ...
  nChannels = len(gain[0])
  nRows = len(gain[0][0])
  if (bOverlay):
        tb.open(caltable2)
        caltable2SubType = (tb.info())['subType']
        gain2 = tb.getcol('GAIN')
        ggx2 = gain2[0][:][:]
        tb.close()
        nPolarizations2 = len(gain2)
        if (nPolarizations == 2):
              ggy2 = gain2[1][:][:]
        nChannels2 = len(gain2[0])
...
#              print "***********  Warning: there was an extra split"
              i = ictr
          else:
              i = originalSpw[ictr]
      ...
      if (bOverlay):
            for ictr in range(len(originalSpw2)):
              i = originalSpw2[ictr]
#              print "spw %d = TDM spw %d = FDM spw %d" % (ictr,i,originalSpw[ictr])
              nchan = len(vm.spwInfo[i]["chanFreqs"])
              if (nchan > 1):
                  missingFrequencyWidth = originalChannelStart[ictr]*(vm.spwInfo[i]["chanFreqs"][-1]-vm.spwInfo[i]["chanFreqs"][0])/(nchan-1)
              else:
              ...
              if (missingFrequencyWidth > 0):
                    print "Correcting for channels flagged prior to running bandpass"
              newfreqs = 1e-9*(vm.spwInfo[i]["chanFreqs"])  + missingFrequencyWidth*1e-9
              chanFreqGHz2.append(newfreqs)
#              print "using spwInfo[%d], #chan=%d, %f-%f" % (i, len(newfreqs), newfreqs[0], newfreqs[-1])
  ...
  if (bOverlay):
        ...
        print "nChannels2 = ", nChannels2
        print "nFields2 = %d = " % (nFields2), uniqueFields2
        print "nRows2 = ", nRows2
  uniqueAntennaIds = np.sort(np.unique(ant))

  if (caltableSubType == 'B TSYS'):
      yAmplitudeLabel = "Tsys (K)"
#  if (yaxis.find('tsys') >= 0):
#      yAmplitudeLabel = "Tsys (K)"
#  elif (caltable.lower()[-5:] == '.tsys' or
#        caltable.lower()[-9:] == '.tsys.fdm' or
#        caltable.lower()[-9:] == '.tsys_fdm'):
#      yAmplitudeLabel = "Tsys (K)"
  else:
      if (yaxis.lower().find('db') >= 0):
  ...
  while (xctr < len(antennasToPlot)):
    ...
    while (spwctr < len(spwsToPlot)):
      ...
      if (overlayAntennas):
          xctr = -1
      firstTimeMatch = -1
#      computedAtm = -1  # only compute once per source  (obsolete)
      while (mytime < nUniqueTimes):
        if (debug):
...
# This is the way it was:
                    ...
                    for j in range(0,nChannels):
                        channels.append(j)
                        if (msFound):
#                            print "ispw=%d, j=%d, shape(chanFreqGHz) = " % (ispw,j), np.shape(chanFreqGHz)
...
#                else:
#                    print "field %d is not in " % (fields[i]), fieldsToPlot
#            else:
#                if (ant[i]==xant and cal_desc_id[i]==ispw):
#                    print "no match: %d?%d  %d?%d %d? %.0f?" % (ant[i],xant,cal_desc_id[i],ispw,mytime,uniqueTimes[mytime]), sm, timerangeList, timerangeListTimes
        # end 'for i'
        myspw = originalSpw[ispw]
        if (msFound):
          if (myspw >= len(refFreq)):
#            print "Warning: extra split"
            myspw = ispw
...
#            print "CAF, CAS, CAT = ", computedAtmField, computedAtmSpw, computedAtmTime
            ...
            (atmfreq,atmchan,transmission,pwvmean,atmairmass,TebbSky,missingCalWVRErrorPrinted)= \
               CalcAtmTransmission(pchannels, pfrequencies, xaxis, pwv,
                       ...
                       interval, uniqueFields[fieldIndex], refFreq[originalSpw[ispw]],
...
#            print "len(TebbSky)=%d, len(transmission)=%d" % (len(TebbSky),len(transmission))
            if (overlayTimes):
                ...
                atmString = 'PWV %.2fmm, airmass %.2f' % (pwvmean,atmairmass)
            if (showimage):
...
#                    print "ispw=%d" % (ispw)
#                    print "originalSpw[%d]=" % (ispw), originalSpw[ispw]
#                    print "spws = ", spws
#                    print "originalSpw = ", originalSpw
#                    print "spws.index(originalSpw[ispw]) = ", spws.index(originalSpw[ispw])
#                    print "LOs = ", LOs
...
#              print "Computing image frequencies (LO1=%f) ***********************************"%(LO1)
              ...
              (atmfreqImage,atmchanImage,transmissionImage,pwvmeanImage,atmairmassImage,TebbSkyImage,missingCalWVRErrorPrintedImage) = \
                CalcAtmTransmission(pchannels, pfrequenciesImage, xaxis, pwv,
                       vm, msName, asdm, xant, uniqueTimes[mytime],
                       interval, uniqueFields[fieldIndex], refFreq[originalSpw[ispw]],
                       net_sideband[originalSpw[ispw]], mytime,
                       missingCalWVRErrorPrinted, verbose=DEBUG)
...
#            computedAtm = uniqueFields[fieldIndex]
        if (bOverlay):
...
#                else:
#                    print "%s is not in " % (fields2[i]), fieldsToPlot
#                    print "The two bandpass solutions differ in their field ID."
#                 Need to not return here because Tsys solutions have multiple fields!
#                    return(vm)
          # end 'for i'
...
#            matchFound = False
        ...
        if (multiFieldsWithOverlayTime):
            ...
            uTPFPS = []
            for f in fieldIndicesToPlot:
                for t in uniqueTimesPerFieldPerSpw[ispw][f]:
...
#                print "No match for %.1f in "%(t), uTPFPS
          ...
          for t in uniqueTimesPerFieldPerSpw[ispw][fieldIndex]:
...
######### Here is the amplitude plotting ############
        if (yaxis.find('amp')>=0 or yaxis.find('tsys')>=0 or
            yaxis.find('both')>=0 or yaxis.find('ap')>=0):

          if (debug):
              print "amp: xant=%d, myap=%d, mytime=%d, firstTimeMatch=%d, bOverlay=" % (xant, myap, mytime, firstTimeMatch), bOverlay
          if (myap==1):
...
#                    print "amp: Resetting xaxis channel range to counteract flagged data"
                ...
                if (bOverlay):
                      ...
                      if (np.abs(xrange/xrange2 - 1) > 0.05):  # 0.0666 is 2000/1875-1
...
#                            print "amp: Resetting xaxis frequency range to counteract flagged data"
            ...
            if (overlayAntennas == False and overlayTimes == False and bOverlay == False):
                ...
                if (xframe == firstFrame):
                  ...
                  and overlayTimes==False):  # try to support antenna,time
                ...
                doneTimeOverlay = True
                for f in fieldIndicesToPlot:
                  if (len(uniqueTimesPerFieldPerSpw[ispw][f]) > 0):
                    if (uniqueTimes[mytime] < uniqueTimesPerFieldPerSpw[ispw][f][-1]-solutionTimeThresholdSeconds and
                        uniqueTimes[mytime] < timerangeListTimes[-1]):
#                        print "Not done because %.0f < %.0f for fieldIndex=%d" % (uniqueTimes[mytime], uniqueTimesPerFieldPerSpw[ispw][f][-1],f)
...
#                            print "Drawing %s" % (legendString)
                            ...
                            for a in range(len(uniqueTimesPerFieldPerSpw[ispw][fieldIndex])):
                              legendString = utstring(uniqueTimesPerFieldPerSpw[ispw][fieldIndex][a],220)
                              x0 = xstartTitle + (a*timeHorizontalSpacing)
                              y0 = ystartOverlayLegend
                              if (a==0):
                                  pb.text(x0-0.02,y0, 'UT',color='k',fontsize=mysize, transform=pb.gcf().transFigure)
...
#                        print "Checking if %f >= %f" % (xrange2, xrange)
            ...
            if ((overlayAntennas==False and overlayTimes==False) or
                (overlayAntennas==True and xant==antennasToPlot[-1] and overlayTimes==False) or
                (overlayTimes==True and doneTimeOverlay and overlayAntennas==False)):
                ...
                if (xaxis.find('freq')>=0 and showfdm and nChannels <= 256):
                    showFDM(vm.spwInfo) # (originalSpw, chanFreqGHz)
...
#            if (xframe == 111 and amplitudeWithPhase):
            if ((xframe == 111 and amplitudeWithPhase) or
                # Following case is needed to make subplot=11 to work for overlay='antenna,time'
...
#                            print "phase frame %d: Resetting xaxis frequency range to counteract flagged data" % (xframe)
            ...
            if (overlayAntennas == False and overlayTimes == False and bOverlay == False):
                ...
                if (xframe == firstFrame):
                      ...
                      pb.text(xstartTitle, ystartTitle, caltableList, size=titlesize,
                              color='k', transform=pb.gcf().transFigure)
            elif (overlayAntennas==True and overlayTimes==False and xant==antennasToPlot[-1] and bOverlay==False):
                # We do this last, because by then, the limits will be stable.
                x0 = xstartPolLabel
                y0 = ystartPolLabel
                if (corrTypeToString(corr_type[0]) in polsToPlot):
                ...
                if (xframe == firstFrame):
                    ...
                    pb.text(xstartTitle, ystartTitle, caltable, size=titlesize, color='k',
                            transform=pb.gcf().transFigure)
                    DrawAntennaNames(msAnt, antennasToPlot, msFound, mysize)
            elif (overlayTimes==True and overlayAntennas==False and bOverlay == False):
                doneTimeOverlay = True
                for f in fieldIndicesToPlot:
                    if (uniqueTimes[mytime] < uniqueTimesPerFieldPerSpw[ispw][f][-1]-solutionTimeThresholdSeconds and
                        uniqueTimes[mytime] < timerangeListTimes[-1]):
...
#          print "*** mytime+1 = %d,  nUniqueTimes = %d........" % (mytime+1, nUniqueTimes)
          ...
          if ((overlayAntennas==False and overlayTimes==False)
              ...
              or (doneTimeOverlay and overlayTimes==True and overlayAntennas==False)):
#              or (overlayTimes==True and sloppyMatch(uniqueTimesPerFieldPerSpw[ispw][fieldIndex][-1],uniqueTimes[mytime],solutionTimeThresholdSeconds,myprint=debugSloppyMatch) and fieldIndex==fieldIndicesToPlot[-1])):  # and (xant==antennasToPlot[-1]))):
            ...
            if (len(figfile) > 0):
                  plotfiles.append(makeplot(figfile,msFound,msAnt,
                                   overlayAntennas,pages,pagectr,density,
                                   interactive,antennasToPlot,spwsToPlot,
                                   overlayTimes))
            myinput = ''
            donetime = timeUtilities.time()
            if (interactive):
...
#                print "Returning to [%d,%d,%d,%d]" % (xctr,spwctr,mytime,myap)
                ...
                if (pagectr >= len(pages)):
                    ...
                    newpage = 0
            if (overlayTimes==True and sloppyMatch(uniqueTimesPerFieldPerSpw[ispw][fieldIndex][-1],uniqueTimes[mytime],solutionTimeThresholdSeconds,myprint=debugSloppyMatch)):
                # be sure to avoid any more loops through mytime which will cause 'b' button to fail
                mytime = nUniqueTimes
#          else:
#              print "Not going to new page, uniqueTimes[mytime]=%.1f, uniqueTimesPerFieldPerSpw[ispw][fieldIndex][-1]=%.1f" % (uniqueTimes[mytime],uniqueTimesPerFieldPerSpw[ispw][fieldIndex][-1])
        if (redisplay == False):
    ...
    if (xant >= antennasToPlot[-1] and xframe != xframeStart):
        ...
        if (len(figfile) > 0):
              plotfiles.append(makeplot(figfile,msFound,msAnt,overlayAntennas,
                                      ...
                                      antennasToPlot,spwsToPlot,overlayTimes))
    if (redisplay == False):
        xctr += 1
...
def CalcAtmTransmission(pchan,pfreq,xaxis,pwv,vm,vis,asdm,antenna,timestamp,
                        ...
                        mytime, missingCalWVRErrorPrinted, verbose=False):
    ...
    for i in range(len(scantimes)):
          ...
          if (tdiff < mindiff):
              ...
              mindiff = tdiff
    if (verbose):
          print "For timestamp=%.1f, got closest scan = " %timestamp, bestscan
    ...
    if (verbose):
          print "CalcAtm: found elevation=%f for scan:" % (conditions['elevation']), bestscan
...
#       print "Getting Tsky, channel by channel"
    ...
    if (len(wet) != len(dry)):
        print "Warning: inconsistent opacity array shapes returned by atm"
        print "n=%d, chans = " %(n), chans
        print "len(wet)=%d, np.shape(wet) = " % (len(wet)), np.shape(wet)
        print "len(dry)=%d, np.shape(dry) = " % (len(dry)), np.shape(dry)
    elif ((len(wet) != len(TebbSky)) or (len(dry) != len(TebbSky))):
        print "Warning: inconsistent array shapes returned by atm (wet,TebbSky)"
        print "n=%d, chans = " % (n), chans
        print "len(wet)=%d, np.shape(wet) = " % (len(wet)), np.shape(wet)
        print "len(dry)=%d, np.shape(dry) = " % (len(dry)), np.shape(dry)
        print "len(TebbSky)=%d, np.shape(TebbSky)" % (len(TebbSky)), np.shape(TebbSky)
    else:
...
def RescaleTrans(trans, lim, subplotRows, lo1=None, xframe=0):
    ...
    # Returns: arrays of the rescaled transmission values and the zero point values.
    ...
    if (transmissionRange < 0.05):
          # force there to be a minimum range of transm. displayed so as to not
          # overemphasize tiny ozone lines
          transmissionRange = 0.05

    if (transmissionRange > 1 and transmissionRange < 10):
          # force there to be a minimum range of Tebbsky to display
    ...
    # compute the transmission at the bottom of the plot
...
def recalcYlimitsFreq(chanrange, ylimits, amp, sideband,plotrange,xchannels):
  ...
  if (chanrange[0]==0 and chanrange[1] == 0 and plotrange[2] == 0 and plotrange[3]==0):
      ...
      if (ylim_debug):
        ...
        print "Examining values: ", amp[mylist]
...
#    print "Entered SetNewXLimits with range = %.3f" % (np.max(newxlimits)-np.min(newxlimits))
    ...
    if (newxlimits[0] < newxlimits[1]):
        pb.xlim([newxlimits[0]-xrange*0.01,newxlimits[1]+xrange*0.01] )
    else:
#        print "Swapping xlimits order"
        pb.xlim(newxlimits[1]-xrange*0.01, newxlimits[0]+xrange*0.01)
...
def showFDM(spwInfo): # originalSpw, chanFreqGHz):
    """
    ...
    for i in spwInfo.keys():
        if (len(spwInfo[i]["chanFreqs"]) > 256):
            chanFreqGHz = spwInfo[i]["chanFreqs"] * 1e-9
            # Then it must be an FDM spw
            fdmctr += 1
            verticalOffset = fdmctr*0.04*yrange
            y1a = y0 + 0.04*yrange + verticalOffset
            y2 = y1a + 0.01*yrange
#            print "chan=%d: Drawing line at y=%f (y0=%f) from x=%f to %f" % (len(chanFreqGHz),
#                                              y1a,y0,chanFreqGHz[0], chanFreqGHz[-1])
            f0 = chanFreqGHz[0]
            f1 = chanFreqGHz[-1]
            if (f1 < f0):
                swap = f1
...
def DrawAtmosphere(showatm, showtsky, subplotRows, atmString, mysize,
    ...
    if (showatm or showtsky):
        if (showatm):
            ...
            atmcolor = tskyColor
        pb.text(0.2, ystartPolLabel, atmString, color=atmcolor, size=mysize, transform=pb.gca().transAxes)
        if (showtsky):
            ...
            if (len(rescaledY) != len(transmission)):
                print "We have a problem in RescaleTrans() !!!!!!!!!!!"
        if (xaxis.find('chan')>=0):
#            rescaledX = RescaleX(atmchan, xlim, plotrange)
            rescaledX = atmchan
            if (len(rescaledX) != len(rescaledY)):
                print "len(rescaledY)=%d,  len(rescaledX)=%d" % (len(rescaledY),len(rescaledX)) 
            pb.plot(rescaledX, rescaledY,'%s%s'%(atmcolor,atmline))
            tindex = -1
        elif (xaxis.find('freq')>=0):
            if (len(rescaledY) != len(atmfreq)):
                print "len(rescaledY)=%d, len(atmfreq)=%d" % (len(rescaledY),len(atmfreq)) 
